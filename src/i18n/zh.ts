...
export const zh: Messages = {
  ...
  v2: {
    home: {
      ...
      heroLede: '把论文目标、资源约束和优先目标变成一次可共享的研究任务；候选模型按约束和证据逐步筛选，比较结果与来源保留在同一个任务里。',
      heroCta: '开始设计实验',
      exampleKicker: '研究任务示例',
      exampleTitle: '自进化代码智能体 · LoRA · 24GB',
      exampleBody: '示例约束是“权重更新 = LoRA、单卡 24GB、开放权重”；18 个模型按这些条件筛成 5 个候选，其中 3 个证据链较完整。',
      exampleModels: '个模型',
      ...
      overviewBody: '同一模型目录按发布时间、推理硬件、参数规模和证据状态展示；论文采用情况作为第二层证据叠加。',
      statModels: '模型总数',
      statFamilies: '家族数',
  ...
  workspace: {
    ...
    lede: '设定研究任务后，工作台按资源与实验约束筛选候选模型，并把每个候选的来源、比较结果和待核验项一起保存。',
  ...
  research: {
    ...
    memo: {
      ...
      copied: '已复制',
      dataRevision: '数据版本',
      empty: '还没有可生成的决策备忘录：先填写研究任务，并加入至少一个候选模型。',
      sectionTask: '研究任务',
