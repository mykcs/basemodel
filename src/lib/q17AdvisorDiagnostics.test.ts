...
describe('Q17 advisor diagnostics publication', () => {
  ...
  it('keeps the result and boundary visible in both language routes', () => {
    ...
    expect(component).toContain('第一阶段（Stage 1，做题与任务后复盘）由 Qwen 完成 WebShop 网页购物任务');
    expect(component).toContain('第二阶段（Stage 2，持续学习）会更新文字记忆、可复用技能、行为规则和 SD-LoRA 参数');
    expect(component).toContain('DirectApply 表示候选参数检查通过后下一轮直接使用');
    expect(component).toContain('No-GDR 表示不再先用一小组题筛选新旧参数');
    expect(component).toContain('下文 R127 / R128 指进入第 127 / 128 轮时加载的两个相邻参数状态');
