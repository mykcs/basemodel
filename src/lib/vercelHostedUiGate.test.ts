...
describe('Vercel hosted UI gate planner', () => {
  it('maps concrete Astro page files to their deployed routes', () => {
    ...
    expect(pageFileToRoute('docs/archive/site-en/src/pages/en/index.astro.archive')).toBeUndefined();
    expect(pageFileToRoute('src/pages/research/seed-openevo/study/results.astro')).toBe('/research/seed-openevo/study/results/');
    expect(pageFileToRoute('docs/archive/site-en/src/pages/en/research/seed-openevo/study/results.astro.archive')).toBeUndefined();
  ...
  it('keeps a metadata-only Results release on the active Chinese route', () => {
    ...
    expect(plan.routes).toEqual(['/research/seed-openevo/study/results/']);
  ...
  it('keeps the briefing owner local to its active page entrypoint', () => {
  ...
  it('keeps the mechanism map local to its active page entrypoint', () => {
    const src = new URL('../', import.meta.url);
    const importers = readdirSync(src, { recursive: true, encoding: 'utf8' })
      .filter((file) => /\.(?:astro|[cm]?[jt]sx?)$/.test(file) && !/\.(?:test|spec)\./.test(file))
  ...
  it('keeps shared research primitives, global styling, and unknown companions full', () => {
    ...
    expect(planHostedUi([
      owner,
      ...Array.from({ length: 8 }, (_, index) => `src/pages/unrelated-${index}.astro`),
    ]).mode).toBe('full');
  });
