...
describe('experiment-first Study index', () => {
  ...
  it('keeps DirectApply attached to its full run, same-task check, SD-LoRA history, Text Memory, and D1 diagnostics', () => {
    ...
    expect(directApply?.childLinks).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ role: 'analysis', label: expect.objectContaining({ zh: '后继三组对比：普通 / Bounded State / + GDR' }), href: '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/' }),
  ...
  it('keeps the six experiment parents and key lineage/analysis children', () => {
    ...
    expect(titles).toEqual([
      ...
      '1.7B · 普通 OpenEVO / Bounded State / + GDR',
