...
describe('reader-first public copy hierarchy', () => {
  ...
  it('makes the action-wrapper incident concrete, responsibility-first, and useful for the next experiment', () => {
    ...
    expect(incident).toContain('Qwen2.5-7B-Instruct（7B，不是 3B）');
    expect(incident).toContain('这里的 BASE 只是“未加载 OpenEvo adapter”的实验臂名，不是 Qwen 的 Base checkpoint');
    expect(incident).toContain('我们没有再用 OpenEvo 对它做参数训练');
    expect(incident).toContain('同一个 7B Instruct + H1.38B final SD-LoRA');
    expect(incident).toContain('公开 checkpoint 只包含 adapter，不是另一份完整 7B 权重');
    expect(incident).toContain('https://huggingface.co/Qwen/Qwen2.5-7B-Instruct');
    expect(incident).toContain('https://huggingface.co/miyuki17/openevo-qwen25-7b-webshop-sd-lora/tree/7c836f259f32e6775f02f90e6a9880e8ef82f40c');
    expect(incident).toContain('128 × 2 = 256');
    expect(incident).toContain('评测时两边参数都不再更新');
    expect(incident).not.toContain('两边都是冻结模型');
    expect(incident).toContain('16 条成功 rollout，覆盖 8 个任务 × 每个任务 2 条独立成功轨迹');
    expect(incident).toContain('累计 optimizer-step 上限 ≤128');
