...
describe('content-first server operations pilot', () => {
  it('makes the dated operational state the first subject without hiding deletion authority', () => {
    expect(server).toContain('非实时快照 · 保存时刻 2026-09-15 22:10（UTC+8）');
    expect(server).toContain('实验文件磁盘：约 0.99 TiB 可用');
    expect(server).toContain('NOT_AUTHORIZED');
    expect(server).toContain('实验文件磁盘：约 0.99 TiB 可用');
    expect(server).toContain('这份快照里，2.0 TiB 持久文件系统已用约 51%');
    ...
    expect(server).toContain('远端已有副本也不能自动删服务器文件');
    expect(server).toContain('删除服务器文件和把私有归档改成公开必须分别批准');
    ...
    expect(server).toContain('还没合并的草稿不能当正式规则');
