...
describe('SEED WebShop reproduction and measurement boundaries', () => {
  ...
  it('explains why parser and task-identity audits consumed time without duplicating the technical wrapper trace', () => {
    ...
    expect(wrapperAttribution).toContain('我们想知道 OpenEvo 的“让模型从自己做成功的 WebShop 任务里再学一点”到底有没有用');
    expect(wrapperAttribution).toContain('PRIMARY-v1 就是这条流程中的一次正式评测，不是一次新的训练');
    ...
    expect(wrapperAttribution).toContain('128 × 2 = 256 个 episode');
    expect(wrapperAttribution).toContain('16 条成功 rollout 记录');
    expect(wrapperAttribution).toContain('覆盖 8 个不同任务、每个任务 2 条独立成功轨迹');
