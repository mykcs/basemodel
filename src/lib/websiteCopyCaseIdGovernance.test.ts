...
describe('website copy case ID governance', () => {
  it('keeps CASE identities unique in the integrated tree', () => {
    const ids = [...cases.matchAll(/^### CASE-(\d{3})\b/gm)].map((match) => `CASE-${requiredCapture(match, 1)}`);
    expect(ids.length).toBeGreaterThan(0);
    expect(duplicates(ids), 'duplicate CASE IDs must be renumbered at integration time').toEqual([]);
  });

  it('keeps explicit case anchors unique', () => {
    const anchors = [...cases.matchAll(/<a id="(case-[^"]+)"/g)].map((match) => requiredCapture(match, 1));
    expect(duplicates(anchors), 'duplicate case anchors break stable references').toEqual([]);
  });
});
