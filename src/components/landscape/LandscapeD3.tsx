...
import { buildLandscapePoints, LANDSCAPE_HARDWARE_ORDER, type LandscapePoint } from '../../lib/landscape';
import type { AtlasModel } from '../../lib/types';

export default function LandscapeD3({ models, locale = 'zh' }: { models: AtlasModel[]; locale?: Locale }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const tiers = [...LANDSCAPE_HARDWARE_ORDER, 'unknown'];
      ...
      const labels = new Map(tiers.map((tier, index) => [tier, index === tiers.length - 1 ? (locale === 'zh' ? '待核验' : 'Unknown') : points.find((point) => point.hardwareIndex === index)?.hardwareLabel ?? tier]));
      ...
      plot.selectAll<SVGPathElement, LandscapePoint>('path.landscape-point').data(points, (point) => point.id).join('path').attr('class', 'landscape-point').attr('transform', (point) => `translate(${x(new Date(point.releaseTimestamp))},${y(tiers[point.hardwareIndex]) ?? 0})`).attr('d', (point) => symbolPath(point.architecture, point.parameterB === null ? 6 : size(point.parameterB))).attr('fill', (point) => vendorColor(point.vendor)).attr('stroke', (point) => statusColor(point.dataStatus)).attr('stroke-width', 2).attr('stroke-dasharray', (point) => point.dataStatus === 'verified' ? null : point.dataStatus === 'partial' ? '5 3' : '2 3').attr('tabindex', 0).on('mouseenter focus', (_, point) => { tooltip.style('opacity', 1).html(`<strong>${point.name}</strong><br/>${point.vendor} · ${point.releaseDate}<br/>${point.hardwareLabel} · ${point.dataStatusLabel}`); }).on('mousemove', (event) => { tooltip.style('left', `${event.offsetX + 12}px`).style('top', `${event.offsetY + 12}px`); }).on('mouseleave blur', () => tooltip.style('opacity', 0)).on('click keydown', (event, point) => { if (event.type === 'click' || (event as KeyboardEvent).key === 'Enter') window.location.href = localePath(locale, `/models/${point.id}/`); });
    };
    render();
    const observer = new ResizeObserver(render);
    observer.observe(root);
    return () => observer.disconnect();
  }, [locale, models, m.landscape.chartAria]);
