...
export default function ModelExplorer({ models, papers, paperModelIds, locale = 'zh' }: { models: AtlasModel[]; papers: AtlasPaper[]; paperModelIds: string[]; locale?: Locale }) {
  ...
  return <section className="explorer-shell">
    ...
    <div className="quick-filter-row" aria-label={m.explorer.filter}>{quickFilters.map(({ key, label }) => <button className={`quick-filter ${filters[key] === true ? 'is-active' : ''}`} type="button" aria-pressed={filters[key] === true} onClick={() => toggleQuickFilter(key)} key={key}>{label}</button>)}</div>
