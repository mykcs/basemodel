...
export default function ModelExplorer({ models, papers, paperModelIds, locale = 'zh' }: { models: AtlasModel[]; papers: AtlasPaper[]; paperModelIds: string[]; locale?: Locale }) {
  ...
  const paperIds = useMemo(() => new Set(paperModelIds), [paperModelIds]);
  const result = useMemo(() => sortModels(filterModels(models, filters, paperIds), sort), [models, filters, paperIds, sort]);
  ...
  const quickFilters: Array<{ key: BooleanFilterKey; label: string }> = [
    { key: 'openWeights', label: m.explorer.quickOpenWeights }, { key: 'baseCheckpoint', label: m.explorer.baseCheckpoint }, { key: 'lora', label: m.explorer.quickLora }, { key: 'rl', label: m.explorer.quickRl },
  ...
  useEffect(() => {
    ...
    if (sort !== 'release') next.set('sort', sort); if (filterDepth !== 'core') next.set('depth', filterDepth); if (view !== 'decision') next.set('view', view);
  ...
  const facetCount = (key: BooleanFilterKey) => filterModels(models, { ...filters, [key]: true }, paperIds).length;
  ...
  const isDefaultDecision = !hasActiveTask && activeChips.length === 0 && sort === 'release';
  const cardFor = (model: AtlasModel) => <ModelDecisionCard key={model.id} model={model} fit={taskFits.get(model.id)} locale={locale} m={m} inCandidate={activeCandidates.includes(model.id)} inCompare={activeCompare.includes(model.id)} onToggleCandidate={() => activeCandidates.includes(model.id) ? removeCandidate(model.id) : addCandidate(model.id)} onToggleCompare={() => activeCompare.includes(model.id) ? removeFromCompare(model.id) : addToCompare(model.id)} onQuickView={() => openQuickView(model.id)} paperAdopted={paperIds.has(model.id)} />;
  const decisionSections = useMemo(() => {
    if (!isDefaultDecision) return [];
    const used = new Set<string>();
    const take = (predicate: (model: AtlasModel) => boolean) => result.filter((model) => !used.has(model.id) && predicate(model)).filter((model) => { used.add(model.id); return true; });
    const currentOpen = take((model) => isCurrentModel(model, familyCoverageRecords, models) && model.openness.weights_available === true);
    const currentApi = take((model) => isCurrentModel(model, familyCoverageRecords, models) && model.access?.api_status === 'available');
    const paperUsed = take((model) => paperIds.has(model.id));
    const remaining = result.filter((model) => !used.has(model.id));
    return [
      { id: 'open', title: locale === 'zh' ? '当前开放权重研究候选' : 'Current open-weight research candidates', hint: locale === 'zh' ? '优先看可下载、可进入本地研究流程的当前模型。' : 'Start with current models that can enter a local research workflow.', models: currentOpen },
      { id: 'api', title: locale === 'zh' ? '当前 API / 托管模型' : 'Current API / hosted models', hint: locale === 'zh' ? '适合 API 推理或现代化重跑；不要与开放权重能力混为一谈。' : 'Useful for API inference or modern reruns; do not conflate these with open-weight access.', models: currentApi },
      { id: 'paper', title: locale === 'zh' ? '论文中已有采用证据' : 'Models already adopted in papers', hint: locale === 'zh' ? '历史采用不等于当前最佳，但对论文可比性很重要。' : 'Historical adoption is not the same as current best, but it matters for comparability.', models: paperUsed },
      { id: 'catalog', title: locale === 'zh' ? '其余完整目录' : 'Remaining catalog', hint: locale === 'zh' ? '需要数据库级浏览时再展开这部分。' : 'Use this section when you need catalog-level exploration.', models: remaining },
    ].filter((section) => section.models.length > 0);
  }, [isDefaultDecision, result, models, paperIds, locale]);
  ...
  return <section className="explorer-shell">
    ...
    {showFilters && <div className="filter-panel"><div className="filter-depth-toggle" role="group" aria-label={locale === 'zh' ? '筛选层级' : 'Filter depth'}><button type="button" className={filterDepth === 'core' ? 'is-active' : ''} onClick={() => setFilterDepth('core')}>{locale === 'zh' ? '研究约束' : 'Research constraints'}</button><button type="button" className={filterDepth === 'advanced' ? 'is-active' : ''} onClick={() => setFilterDepth('advanced')}>{locale === 'zh' ? '目录属性' : 'Catalog attributes'}</button></div>{filterDepth === 'core' ? <><Select label={m.explorer.openWeights} value={filters.openWeights === undefined ? '' : String(filters.openWeights)} onChange={(value) => update('openWeights', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.baseCheckpoint} value={filters.baseCheckpoint === undefined ? '' : String(filters.baseCheckpoint)} onChange={(value) => update('baseCheckpoint', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.lora} value={filters.lora === undefined ? '' : String(filters.lora)} onChange={(value) => update('lora', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.rl} value={filters.rl === undefined ? '' : String(filters.rl)} onChange={(value) => update('rl', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.current} value={filters.current === undefined ? '' : String(filters.current)} onChange={(value) => update('current', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.singleGpu} value={filters.singleGpu === undefined ? '' : String(filters.singleGpu)} onChange={(value) => update('singleGpu', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.paperUse} value={filters.paperUse === undefined ? '' : String(filters.paperUse)} onChange={(value) => update('paperUse', value === '' ? undefined : value === 'true')} options={haveNot} /><Select label={m.explorer.hardwareTier} value={filters.resource ?? ''} onChange={(value) => update('resource', value)} options={resourceOptions} /><Select label={m.explorer.checkpoint} value={filters.checkpoint ?? ''} onChange={(value) => update('checkpoint', value)} options={[['', m.explorer.allCheckpoints], ['base', checkpointLabel('base', locale)], ['instruct', checkpointLabel('instruct', locale)], ['thinking', checkpointLabel('thinking', locale)], ['coder', checkpointLabel('coder', locale)], ['vision', checkpointLabel('vision', locale)]]} /></> : <><Select label={m.explorer.vendor} value={filters.vendor ?? ''} onChange={(value) => update('vendor', value)} options={[['', m.explorer.allVendors], ...vendors.map((value) => [value, value])]} /><Select label={m.explorer.family} value={filters.family ?? ''} onChange={(value) => update('family', value)} options={[['', m.explorer.allFamilies], ...families.map((value) => [value, value])]} /><Select label={m.explorer.generation} value={filters.generation ?? ''} onChange={(value) => update('generation', value)} options={[['', m.explorer.allGenerations], ...generations.map((value) => [value, value])]} /><Select label={m.explorer.architecture} value={filters.architecture ?? ''} onChange={(value) => update('architecture', value)} options={[['', m.explorer.allArchitectures], ['dense', architectureLabel('dense', locale)], ['moe', architectureLabel('moe', locale)], ['other', architectureLabel('other', locale)]]} /><Select label={m.explorer.modality} value={filters.modality ?? ''} onChange={(value) => update('modality', value)} options={modalityOptions} /><Select label={m.explorer.minParams} value={filters.minParams === undefined ? '' : String(filters.minParams)} onChange={(value) => update('minParams', value === '' ? undefined : Number(value))} options={paramOptions} /><Select label={m.explorer.specialization} value={filters.specialization ?? ''} onChange={(value) => update('specialization', value)} options={[['', m.explorer.allSpecializations], ...specializations.map((value) => [value, specializationLabel(value, locale)])]} /><Select label={m.explorer.finetuning} value={filters.finetuning === undefined ? '' : String(filters.finetuning)} onChange={(value) => update('finetuning', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.toolUse} value={filters.toolUse === undefined ? '' : String(filters.toolUse)} onChange={(value) => update('toolUse', value === '' ? undefined : value === 'true')} options={yesNo} /><Select label={m.explorer.coding} value={filters.coding === undefined ? '' : String(filters.coding)} onChange={(value) => update('coding', value === '' ? undefined : value === 'true')} options={yesNo} /></>}</div>}
    ...
    {view === 'decision' && (decisionSections.length ? <div className="decision-sections">{decisionSections.map((section) => <section className={`decision-section decision-section-${section.id}`} key={section.id}><div className="decision-section-heading"><h2>{section.title}</h2><p>{section.hint}</p></div><div className="model-grid">{section.models.map(cardFor)}</div></section>)}</div> : <div className="model-grid">{result.map(cardFor)}</div>)}
    {view === 'data' && <DataView models={result} paperModelIds={paperIds} locale={locale} m={m} />}
    {view === 'timeline' && <TimelineView models={result} paperModelIds={paperIds} locale={locale} m={m} />}
