...
export default function ModelExplorer({ models, paperModelIds }: { models: AtlasModel[]; paperModelIds: string[] }) {
  ...
  const [filters, setFilters] = useState<ModelFilters>({ query: params.get('q') ?? '', vendor: params.get('vendor') ?? undefined, architecture: (params.get('architecture') as ModelFilters['architecture']) ?? undefined });
  const [sort, setSort] = useState<'release' | 'parameters' | 'name'>('release');
  const [showFilters, setShowFilters] = useState(false);
  const result = useMemo(() => sortModels(filterModels(models, filters), sort), [models, filters, sort]);
  ...
  const update = (key: keyof ModelFilters, value: string | boolean | undefined) => setFilters((current) => ({ ...current, [key]: value || undefined }));
