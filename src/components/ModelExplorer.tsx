...
export default function ModelExplorer({ models, papers, paperModelIds, locale = 'zh' }: { models: AtlasModel[]; papers: AtlasPaper[]; paperModelIds: string[]; locale?: Locale }) {
  ...
  const paperSet = useMemo(() => new Set(paperModelIds), [paperModelIds]);
  const result = useMemo(() => sortModels(filterModels(models, filters, paperSet), sort), [models, filters, paperSet, sort]);
  ...
  const generations = [...new Set(models.map((model) => model.generation))].sort();
  const specializations = [...new Set(models.flatMap((model) => model.checkpoint.specializations))].sort();
  ...
  const quickFilters: Array<{ key: BooleanFilterKey; label: string }> = [
    ...
    { key: 'lora', label: m.explorer.quickLora },
    { key: 'rl', label: m.explorer.quickRl },
    { key: 'baseCheckpoint', label: m.explorer.baseCheckpoint },
    { key: 'singleGpu', label: m.explorer.quickSingleGpu },
    { key: 'current', label: m.explorer.quickCurrent },
  ...
  useEffect(() => {
    ...
    setFilters({
      ...
      rl: booleanParam('rl'),
      lora: booleanParam('lora'),
      current: booleanParam('current'),
      baseCheckpoint: booleanParam('baseCheckpoint'),
      singleGpu: booleanParam('singleGpu'),
      toolUse: booleanParam('toolUse'),
      coding: booleanParam('coding'),
  ...
  const facetCount = (key: BooleanFilterKey) => filterModels(models, { ...filters, [key]: true }, paperSet).length;
  ...
  const decisionSections = useMemo<DecisionSection[]>(() => {
    const current = (model: AtlasModel) => isCurrentModel(model, familyCoverageRecords, models);
    const used = new Set<string>();
    const take = (predicate: (model: AtlasModel) => boolean) => result.filter((model) => !used.has(model.id) && predicate(model)).map((model) => { used.add(model.id); return model; });
    const sections: DecisionSection[] = [
      {
        id: 'current-open',
        title: locale === 'zh' ? '当前开放权重研究候选' : 'Current open-weight research candidates',
        body: locale === 'zh' ? '先看能下载权重、又仍处于当前模型空间的候选。' : 'Start with current models whose weights are available for local research.',
        models: take((model) => current(model) && model.openness.weights_available === true),
      },
      {
        id: 'current-api',
      ...
      {
        id: 'paper-used',
        title: locale === 'zh' ? '论文中已有采用证据' : 'Models with paper adoption evidence',
        body: locale === 'zh' ? '这些模型至少在收录论文中承担过明确角色，适合做可比性和复现起点。' : 'These models have explicit roles in recorded papers and are useful comparability or reproduction anchors.',
        models: take((model) => paperSet.has(model.id)),
      },
      {
        id: 'catalog',
        title: locale === 'zh' ? '其他目录模型' : 'Other catalog models',
        body: locale === 'zh' ? '完整目录仍保留，但它不再抢占研究决策的第一层。' : 'The complete catalog remains available without dominating the first decision layer.',
        models: take(() => true),
      },
    ];
    return sections.filter((section) => section.models.length > 0);
  }, [locale, models, paperSet, result]);

  const renderCard = (model: AtlasModel) => (
    <ModelDecisionCard
      key={model.id}
      model={model}
      fit={taskFits.get(model.id)}
      locale={locale}
      m={m}
      inCandidate={activeCandidates.includes(model.id)}
      inCompare={activeCompare.includes(model.id)}
      onToggleCandidate={() => activeCandidates.includes(model.id) ? removeCandidate(model.id) : addCandidate(model.id)}
      onToggleCompare={() => activeCompare.includes(model.id) ? removeFromCompare(model.id) : addToCompare(model.id)}
      onQuickView={() => openQuickView(model.id)}
      paperAdopted={paperSet.has(model.id)}
    />
  );

  return <section className="explorer-shell">
    <div className="explorer-toolbar">
      <label className="search-field"><span className="sr-only">{m.explorer.searchLabel}</span><input value={filters.query ?? ''} onChange={(event) => update('query', event.target.value)} placeholder={m.explorer.searchPlaceholder} /></label>
      <button className="button button-secondary filter-button" type="button" onClick={() => setShowFilters((value) => !value)} aria-expanded={showFilters}>{m.explorer.filter} {showFilters ? '↑' : '↓'}</button>
      <label className="sort-field"><span>{m.explorer.sort}</span><select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="release">{m.explorer.sortRelease}</option><option value="parameters">{m.explorer.sortParams}</option><option value="name">{m.explorer.sortName}</option></select></label>
    </div>
    ...
    {showFilters && <div className="filter-panel">
      <div className="filter-depth-toggle" role="group" aria-label={locale === 'zh' ? '筛选层级' : 'Filter depth'}>
        <button type="button" className={filterDepth === 'core' ? 'is-active' : ''} onClick={() => setFilterDepth('core')}>{locale === 'zh' ? '研究约束' : 'Research constraints'}</button>
        <button type="button" className={filterDepth === 'advanced' ? 'is-active' : ''} onClick={() => setFilterDepth('advanced')}>{locale === 'zh' ? '目录属性' : 'Catalog attributes'}</button>
      </div>
      ...
      {filterDepth === 'core' ? <>
        ...
        <Select label={m.explorer.lora} value={filters.lora === undefined ? '' : String(filters.lora)} onChange={(value) => update('lora', value === '' ? undefined : value === 'true')} options={yesNo} />
        ...
        <Select label={m.explorer.current} value={filters.current === undefined ? '' : String(filters.current)} onChange={(value) => update('current', value === '' ? undefined : value === 'true')} options={yesNo} />
        <Select label={m.explorer.baseCheckpoint} value={filters.baseCheckpoint === undefined ? '' : String(filters.baseCheckpoint)} onChange={(value) => update('baseCheckpoint', value === '' ? undefined : value === 'true')} options={yesNo} />
        <Select label={m.explorer.singleGpu} value={filters.singleGpu === undefined ? '' : String(filters.singleGpu)} onChange={(value) => update('singleGpu', value === '' ? undefined : value === 'true')} options={yesNo} />
        ...
        <Select label={m.explorer.toolUse} value={filters.toolUse === undefined ? '' : String(filters.toolUse)} onChange={(value) => update('toolUse', value === '' ? undefined : value === 'true')} options={yesNo} />
        <Select label={m.explorer.coding} value={filters.coding === undefined ? '' : String(filters.coding)} onChange={(value) => update('coding', value === '' ? undefined : value === 'true')} options={yesNo} />
      </> : <>
        ...
        <Select label={m.explorer.architecture} value={filters.architecture ?? ''} onChange={(value) => update('architecture', value)} options={[['', m.explorer.allArchitectures], ['dense', architectureLabel('dense', locale)], ['moe', architectureLabel('moe', locale)], ['other', architectureLabel('other', locale)]]} />
        <Select label={m.explorer.checkpoint} value={filters.checkpoint ?? ''} onChange={(value) => update('checkpoint', value)} options={[['', m.explorer.allCheckpoints], ['base', checkpointLabel('base', locale)], ['instruct', checkpointLabel('instruct', locale)], ['thinking', checkpointLabel('thinking', locale)], ['coder', checkpointLabel('coder', locale)], ['vision', checkpointLabel('vision', locale)]]} />
        ...
        <Select label={m.explorer.specialization} value={filters.specialization ?? ''} onChange={(value) => update('specialization', value)} options={[['', m.explorer.allSpecializations], ...specializations.map((value) => [value, specializationLabel(value, locale)])]} />
      </>}
    </div>}
    ...
    {view === 'decision' && <div className="decision-sections">{decisionSections.map((section) => <section className="decision-section" key={section.id} aria-labelledby={`decision-${section.id}`}><div className="decision-section-heading"><h2 id={`decision-${section.id}`}>{section.title}</h2><p>{section.body}</p></div><div className="model-grid">{section.models.map(renderCard)}</div></section>)}</div>}
    {view === 'data' && <DataView models={result} paperModelIds={paperSet} locale={locale} m={m} />}
    {view === 'timeline' && <TimelineView models={result} paperModelIds={paperSet} locale={locale} m={m} />}
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[][] }) {
  return <label className="field"><span>{label}</span><select value={value} onChange={(event) => onChange(event.target.value)}>{options.map(([option, text]) => <option value={option} key={option}>{text}</option>)}</select></label>;
}
