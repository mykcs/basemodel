...
function technicalCopy(kind: Kind, locale: Locale) {
  ...
  const copy: Record<Kind, string> = {
    ...
    seed: zh ? 'Stage 1 用外部 GLM-5.2 阅读完整 episode、生成 hindsight-skill 标注，再做 3-epoch SFT；Stage 2 才由当前 policy 同时承担 actor 与 analyzer，并用 OPD + GRPO 更新参数。GLM-5.2 不是 WebShop reward scorer。双概率条为机制示例，不代表真实 run 的 token probability。' : 'Stage 1 uses external GLM-5.2 to read completed episodes, generate hindsight-skill annotations, and run a 3-epoch SFT; only Stage 2 uses the current policy as both actor and analyzer and updates parameters with OPD + GRPO. GLM-5.2 is not the WebShop reward scorer. The dual probability bars are illustrative rather than measured token probabilities.',
...
export default function InteractiveResearchExplainer({ locale, kind, compact = false }: Props) {
  ...
  const configs: Record<Kind, { title: string; eyebrow: string; lede: string; steps: StepMeta[] }> = {
    ...
    seed: {
      ...
      lede: zh ? 'Stage 1 先让外部 GLM-5.2 把完整 episode 变成 hindsight skill，并通过 SFT 教会初始 policy 复盘；Stage 2 再由当前 policy 自己 rollout、自己分析，并把 OPD 与 GRPO 一起写回下一版参数。' : 'Stage 1 first lets external GLM-5.2 turn completed episodes into hindsight skills and uses SFT to teach the initial policy to analyze trajectories; Stage 2 then lets the current policy roll out and analyze its own experience before writing OPD plus GRPO back into the next parameter state.',
      steps: [
        { label: 'rollout', narration: zh ? 'Stage 2：当前 policy 做真实 on-policy interaction。' : 'Stage 2: the current policy performs real on-policy interaction.' },
