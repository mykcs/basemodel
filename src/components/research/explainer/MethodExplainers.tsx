...
export function OpenEvoExplainer({ locale, step, carrier, setCarrier, onStep }: { locale: Locale; step: number; carrier: Carrier; setCarrier: (carrier: Carrier) => void; onStep: (step: number) => void }) {
  ...
  const edges: EdgeSpec[] = [
    ...
    { id: 'next-loop', from: 'evo-next', to: 'evo-head', tone: 'persist', fromAnchor: 'right', toAnchor: 'top', shape: 'loop-top', label: zh ? '下一版本生效' : 'successor revision activates', active: step >= 6 },
  ];
  return (
    <>
      <div className="irx-diagram irx-openevo-scene" ref={sceneRef} data-ui-audit="contrast layout">
        <FlowNode id="evo-head" role={zh ? '当前智能体版本' : 'PROJECT HEAD'} title={zh ? '当前 agent revision' : 'current agent revision'} detail="Qwen2.5-3B-Instruct + current carrier state" moreLabel={moreLabel} more={zh ? 'Project Head 是任务开始时锁定的执行身份：base model、已激活 carrier 与 context revision 的组合。' : 'The Project Head is the execution identity pinned at task start: base model plus activated carriers and context revision.'} tone="state" active={step === 0} complete={step > 0} className="evo-head" />
        <FlowNode id="evo-task" role={zh ? '任务 N · WebShop' : 'TASK N · WEBSHOP'} title={zh ? '完成 WebShop Task N' : 'complete WebShop Task N'} detail="observation · action · outcome" moreLabel={moreLabel} more={zh ? '任务在冻结的 benchmark contract 下完成，产生可复现的 observation / action / outcome 记录。演化不会在这一 episode 中途发生。' : 'The task completes under one frozen benchmark contract and produces reproducible observation / action / outcome records. Evolution never happens mid-episode.'} tone="env" active={step === 0} complete={step > 0} className="evo-task" />
        <FlowNode id="evo-evidence" role={zh ? '任务完成边界' : 'TASK COMPLETION BOUNDARY'} title={zh ? '已封存证据' : 'SEALED EVIDENCE'} detail={zh ? 'trajectory + outcome + metadata 先冻结，再允许演化' : 'trajectory + outcome + metadata are frozen before evolution'} moreLabel={moreLabel} more={zh ? '封存包含 identity 与 digest，之后可以验证这份 evidence 没有被改写。' : 'Sealing records identity and digest, so later stages can verify the evidence was never rewritten.'} tone="experience" active={step === 1} complete={step > 1} className="evo-evidence">
          <DocStack label={zh ? "已封存证据" : "sealed evidence"} items={['trajectory', 'outcome', 'metadata']} strapped={step >= 1} tone="experience" />
        </FlowNode>
        <FlowNode id="evo-method" role={zh ? '演化方法' : 'EVOLUTION METHOD'} title={zh ? '反思 / 提炼 / 参数演变' : 'reflection / extraction / parametric evolution'} detail={zh ? '方法读取已封存 evidence，不改写刚完成的 Task N' : 'the method reads sealed evidence and does not rewrite the completed Task N'} moreLabel={moreLabel} more={zh ? '演化方法只读 evidence、输出声明类型的候选状态；它不直接在线改写正在服役的 policy。' : 'The evolution method only reads evidence and emits a candidate state of a declared type; it never rewrites the serving policy in place.'} tone="experience" active={step === 2} complete={step > 2} className="evo-method" />
        <FlowNode id="evo-adapter" role={zh ? '当前 WebShop 路线' : 'CURRENT WEBSHOP PATH'} title={zh ? '参数适配器' : 'PARAMETRIC ADAPTER'} detail={zh ? 'OpenEvo SD-LoRA · 把已封存经验写入参数化 carrier' : 'OpenEvo SD-LoRA · write sealed experience into a parametric carrier'} moreLabel={moreLabel} more={zh ? '这是当前 WebShop 实验的主路径：adapter 是被验证、持久化并由下一任务加载的具体 carrier。' : 'This is the current WebShop experiment main path: the adapter is the concrete carrier that is validated, persisted, and loaded by the next task.'} tone="state" active={step === 3} complete={step > 3} className="evo-adapter">
        ...
        <section className="irx-validation" data-flow-id="evo-validation" data-ui-audit-item data-active={step === 4}>
          <span>{zh ? '验证门' : 'VALIDATION GATE'}</span><strong>{zh ? '验证通过（PASS）后才能进入下一版本' : 'PASS is required before successor revision'}</strong>
        ...
        <div className="irx-carrier-lane" data-active={step === 3}>
          {carriers.map((item) => (
            <button key={item.id} type="button" className={`irx-carrier irx-carrier-${item.id} irx-tone-${item.tone}`} data-ui-audit-item data-selected={carrier === item.id} onClick={() => { setCarrier(item.id); onStep(Math.max(step, 3)); }} aria-pressed={carrier === item.id}>
              ...
              <span>{zh ? '载体' : 'CARRIER'}</span><strong>{item.title}</strong><small>{item.detail}</small>{item.id === 'adapter' && <em>{zh ? '当前 WebShop 实例' : 'current WebShop instance'}</em>}
            </button>
          ))}
        </div>
