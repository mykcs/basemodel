...
export function ResearchTaskBuilder({ models, papers, m, locale }: Props) {
  ...
  return (
    <section className="constraint-panel task-builder" aria-label={m.workspace.constraintTitle}>
      <div className="task-builder-header"><div><h2>{m.workspace.constraintTitle}</h2></div><button type="button" className="button button-quiet" onClick={() => { clearResearchTask(); setDraft(emptyTask); setStep(0); }}>{m.workspace.clearTask}</button></div>
      <div className="project-history" aria-label={locale === 'zh' ? '本机研究项目' : 'Local research projects'}>
        ...
        {projects.length > 0 && <div className="project-history-list"><label className="field"><span>{locale === 'zh' ? '恢复历史项目' : 'Restore a project'}</span><select value="" onChange={(event) => loadProject(event.target.value)}><option value="">{locale === 'zh' ? '选择一个已保存项目' : 'Choose a saved project'}</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name} · {project.updatedAt.slice(0, 10)}</option>)}</select></label><button type="button" className="text-button" onClick={() => { const id = projects[0]?.id; if (id) removeResearchProject(id); }}>{locale === 'zh' ? '删除最近项目' : 'Delete most recent'}</button></div>}
      </div>
