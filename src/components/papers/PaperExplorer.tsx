...
const emptyFilters: FilterState = { query: '', evolution: '', role: '', family: '', code: 'all', local: 'all', weight: 'all' };
...
function difficulty(paper: AtlasPaper, locale: Locale): string {
  const repro = paper.reproducibility;
  const missing = [repro?.code_status, repro?.checkpoint_status, repro?.config_status, repro?.environment_status].filter((value) => !value || value === 'unavailable' || value === 'not_reported' || value === 'not_verified').length;
...
function methodSummary(paper: AtlasPaper, locale: Locale): string {
  const rationale = paper.model_selection?.[0]?.rationale;
  ...
  return locale === 'zh' ? `${categories}方法，关注${targets}。` : `${categories} method focused on ${targets}.`;
}
...
export default function PaperExplorer({ papers, models, locale = 'zh' }: { papers: AtlasPaper[]; models: AtlasModel[]; locale?: Locale }) {
  ...
  const filtered = useMemo(() => papers.filter((paper) => {
    ...
    const searchable = [paper.title, paper.id, ...paper.category, ...paper.benchmarks, ...paper.evolution_targets].join(' ').toLowerCase();
    ...
    return (!needle || searchable.includes(needle))
      ...
      && (filters.code === 'all' || (filters.code === 'available' ? paper.code_url !== 'not_reported' && paper.code_url !== 'not_verified' && paper.code_url !== 'unavailable' : paper.code_url === 'not_reported' || paper.code_url === 'not_verified' || paper.code_url === 'unavailable'))
