...
function experimentBurden(paper: AtlasPaper, modelMap: Map<string, AtlasModel>, locale: Locale): { label: string; note: string } {
  ...
  return {
    ...
    note: locale === 'zh' ? 'Atlas 启发式：综合权重更新、多卡资源、API 依赖、环境与代码条件；不是论文实测成本。' : 'Atlas heuristic: combines weight updates, multi-GPU needs, API dependency, environment, and code conditions; it is not a measured paper cost.',
  };
}
...
export default function PaperExplorer({ papers, models, locale = 'zh' }: { papers: AtlasPaper[]; models: AtlasModel[]; locale?: Locale }) {
  ...
  const filtered = useMemo(() => papers.filter((paper) => {
    ...
    return (!needle || searchable.includes(needle)) && (!filters.evolution || paper.evolution_targets.includes(filters.evolution)) && (!filters.role || paperRoles.includes(filters.role as typeof paperRoles[number])) && (!filters.family || paperFamilies.includes(filters.family)) && (!filters.benchmark || paper.benchmarks.includes(filters.benchmark)) && (filters.code === 'all' || (filters.code === 'available' ? codeAvailable : !codeAvailable)) && (filters.checkpoint === 'all' || (filters.checkpoint === 'available' ? checkpointAvailable : !checkpointAvailable)) && (filters.local === 'all' || reproducibilityLevel(paper) === filters.local) && (filters.weight === 'all' || (filters.weight === 'updated' ? weightMatches : noWeightMatches));
  ...
  return <section className="paper-explorer" aria-labelledby="paper-explorer-title">
    ...
    {filtered.length ? <div className="paper-case-grid">{filtered.map((paper) => {
      ...
      return <article className="paper-case-card" key={paper.id}>
        ...
        <dl className="paper-case-facts"><div><dt>{m.evolution}</dt><dd>{paper.evolution_targets.map((value) => evolutionTargetLabel(value, locale)).join(locale === 'zh' ? '、' : ', ')}</dd></div><div><dt>{m.roles}</dt><dd>{rolesForPaper.join(locale === 'zh' ? '、' : ', ')}</dd></div><div><dt>{m.weight}</dt><dd>{weightState}</dd></div><div><dt>{m.burden}</dt><dd title={burden.note}>{burden.label} <span className="provenance-badge provenance-derived">{m.estimated}</span></dd></div></dl>
