...
export function ModelDecisionCard({ model, fit, locale, m, inCandidate, inCompare, paperAdopted, onToggleCandidate, onToggleCompare, onQuickView }: Props) {
  ...
  return <article className="model-card decision-card">
    ...
    <div className="card-footer decision-card-actions"><button className="button button-secondary" type="button" onClick={onQuickView}>{locale === 'zh' ? '一眼概览' : 'Quick view'}</button><button className="button button-secondary" type="button" onClick={onToggleCandidate}>{inCandidate ? m.workspace.removeCandidate : m.workspace.addCandidate}</button><button className="button button-secondary" type="button" onClick={onToggleCompare}>{inCompare ? m.workspace.removeFromCompare : m.workspace.addToCompare}</button></div>
  </article>;
}
