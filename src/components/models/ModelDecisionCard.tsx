...
export function ModelDecisionCard({ model, fit, locale, m, inCandidate, inCompare, paperAdopted, onToggleCandidate, onToggleCompare }: Props) {
  ...
  return <article className="model-card decision-card">
    ...
    <div className="tag-row">{model.checkpoint.modalities.map((value) => <span className="tag" key={value}>{modalityLabel(value, locale)}</span>)}<span className="tag">{checkpointLabel(model.checkpoint.type, locale)}</span>{model.openness.weights_available === true && <span className="tag tag-open">{m.explorer.openWeightsTag}</span>}</div>
    <div className="card-footer decision-card-actions"><button className="button button-secondary" type="button" onClick={onToggleCandidate}>{inCandidate ? m.workspace.removeCandidate : m.workspace.addCandidate}</button><button className="button button-secondary" type="button" onClick={onToggleCompare}>{inCompare ? m.workspace.removeFromCompare : m.workspace.addToCompare}</button><a className="card-compare" href={`${localePath(locale, '/compare/')}?models=${encodeURIComponent(model.id)}`} aria-label={m.explorer.compareAria.replace('{name}', model.name)}>{m.explorer.compare}</a></div>
  </article>;
}
