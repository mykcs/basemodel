...
export const HUMAN_PREFERENCE_MODEL: HumanPreferenceDimension[] = [
  ...
  },
  {
    ...
    statement: '科研叙事按前因 → 假设 → 实验 → 结果 → 下一步判断组织；每个转折要补足能解释“为什么此刻会问这个问题”的最小前因，工程 gate 只在决定可解释性、可识别性或因果归因时进入主线。',
    scopes: ['research-copy', 'briefing'],
    confidence: 'page-specific',
    priority: 4,
    retrievalTags: ['科研汇报', 'briefing', '研究问题', '前因', '背景', '为什么', '15', '30', '2048', '4096', '状态流水账', 'Ceiling', 'Mechanism', 'Control'],
    supportingCaseIds: ['CASE-082'],
    antiOvergeneralization: [
      '不是删除工程证据；当工程事实决定实验是否有效时，它就是科学叙事的一部分。',
      '补前因不是搬运完整历史；只补足让零上下文听众理解当前假设从哪里来的那一个观察或继承设置。',
...
export const HUMAN_FEEDBACK_GOLD_PAIRS: HumanFeedbackGoldPair[] = [
  ...
  {
    id: 'PAIR-082-LOCAL-CAUSAL-CONTEXT',
    caseId: 'CASE-082',
    preferenceIds: ['PREF-RESEARCH-JUDGMENT', 'PREF-OBJECT-FIRST'],
    scopes: ['briefing', 'research-copy'],
    rejected: '15 → 30 步：任务得分仍然为 0',
    accepted: '原来的 15 步来自沿用的 SEED WebShop trainer recipe；上一轮 96 / 96 条尝试有效但没有正向结果，所以才单独检验“是不是步数太少”。',
    reason: '结果页如果没有局部前因，零上下文听众无法知道变量从哪里来、为什么值得在此刻检验。',
    failureMechanisms: ['missing-local-causal-context', 'orphan-parameter-transition', 'author-context-required'],
    ownerStatus: 'accepted',
  },
  {
