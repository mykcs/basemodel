...
export const HUMAN_PREFERENCE_MODEL: HumanPreferenceDimension[] = [
  ...
  },
  {
    ...
    title: '同类实验图复用同一视觉语法',
    statement: '同一 briefing 内展示多条同类训练线时，Score、loss、candidate/accepted update 应保持稳定的图表位置、线型和标记含义；这也适用于后来新增的实验线，只要它展示的是相同训练证据，观众就不该因为“新实验”而重新学习一套图例。',
    scopes: ['research-ui', 'briefing'],
    confidence: 'repeated-explicit',
    priority: 4,
    retrievalTags: ['图表', 'checkpoint', 'loss', 'Score', 'SD-LoRA', '一致', '坐标轴', 'visual grammar', '曲线', '新实验', 'DirectApply', 'No-GDR'],
  ...
  },
  {
    ...
    confidence: 'repeated-explicit',
    priority: 4,
    retrievalTags: ['Preview', '预览', 'build', '构建', 'Vercel', '网页草稿', '审阅', '迭代', '快速', '等待', 'stale', '旧链接', '可见性', '曲线'],
    ...
    antiOvergeneralization: [
      ...
      '不是每次人审都跑全站 205 项浏览器矩阵；这里要求的是针对本轮声称改动的最小 hosted 可见性核对，例如打开目标 slide 确认两张曲线真的在。',
...

  {
    ...
    confidence: 'repeated-explicit',
    priority: 5,
    retrievalTags: ['Task Score', 'WebShop', '128题', '固定题', '最后20轮', 'final', '终评', '同题', '随机题', '题集选择', 'exact success', '能力分数', '指标解释'],
    supportingCaseIds: ['CASE-059', 'CASE-084', 'CASE-095'],
    antiOvergeneralization: [
      '不是要求所有结果都同时展示训练窗口均值和 final；只有两种证据都存在且回答不同问题时才并列。',
      '固定 panel 仍可用于严格同题比较；边界是不能把一套有限题集包装成模型的全面能力。',
      '最后 20 轮平均减少单轮抽样波动，但如果每轮任务不同，它仍是轨迹证据，不应被改写成固定题泛化测试。',
      '题集选择细节应说明到足以判断泄漏、同题性和 reward-blindness；不需要把无助于理解的内部路径或完整 manifest 粘到首屏。',
    ],
  },
...
export const HUMAN_FEEDBACK_GOLD_PAIRS: HumanFeedbackGoldPair[] = [  {
  ...
  },
  {
    ...
    rejected: '已有 7B / 1.7B / 3B 用 Score / loss 曲线，新实验却只放摘要数字卡片，或重新发明另一套 Score / loss / update 布局。',
    accepted: '只要新增实验展示的是同类训练证据，就沿用左侧 WebShop Score、右侧 SD-LoRA loss 和稳定的 candidate / accepted 标记；各实验坐标范围按真实数据调整。',
    reason: 'owner 先要求 7B / 1.7B / 3B 共享图表语法，随后对新增 No-GDR / DirectApply 再次明确“也得和其他实验一样的分数 loss 表”；可复用规则因此是按证据对象继承视觉语义，而不是按实验创建时间重开一套。',
  ...
  {
    id: 'PAIR-095-BENCHMARK-METRIC-CONTEXT',
    caseId: 'CASE-095',
    preferenceIds: ['PREF-BENCHMARK-METRIC-CONTEXT', 'PREF-SCIENTIFIC-BOUNDARY'],
    scopes: ['research-ui', 'research-copy', 'results', 'capability'],
    rejected: '首屏只写普通 OpenEVO 60.72、Bounded 45.98、+GDR 20.77 和 50/128、32/128、10/128；不解释 Task Score 怎么算、128 题怎么来的，也把这一套 final 当成整体能力。',
    accepted: '先说明 Task Score = 每个 WebShop 任务 terminal task_score 的平均值 ×100，partial credit 与 exact success 分开；说明 128 题在训练前从 held-out pool 以固定 reward-blind SHA256 规则选出并冻结，三组使用同一 ordered panel；同时把最后 20 轮均值标为训练轨迹，把固定 128 题标为同题终评，并明确两者都不是全面能力分数。',
    reason: 'owner 要求数字第一次出现就把“怎么算、题怎么选、是否同题、为什么片面”说清，并明确更关注最后 20 轮平均；这不是换一个主分数，而是把轨迹证据和固定题终评分层。',
    failureMechanisms: ['benchmark-number-without-measurement-context', 'fixed-panel-overread', 'trajectory-final-evidence-conflation'],
    ownerStatus: 'accepted',
  },
  {
    id: 'PAIR-096-NATIVE-WANDB-NO-DUPLICATE',
    caseId: 'CASE-096',
...

  {
    ...
    accepted: '正式结果之后按 loss → Frobenius 范数 → Task Vector 相关性 → 谱/方向 → 输出长度/任务步数 → action-family entropy 递进；每一节都先解释指标、再给本实验数字/图、最后写分析与边界，对应 W&B 图就放在该小节。',
    reason: 'owner 先要求从基本指标 loss 开始逐层进入参数和行为诊断；随后共同确定的最终顺序进一步细化为 loss → Frobenius 范数 → Task Vector 相关性 → 谱/方向 → 输出长度/步数 → entropy，并继续保持每个小 subsection 的“指标是什么 → 结果 → 分析”。',
