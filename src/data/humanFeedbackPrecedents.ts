...
export const HUMAN_FEEDBACK_PRECEDENTS: HumanFeedbackPrecedent[] = [
  ...
  },
  {
    ...
    principle: 'W&B 已经有同一指标的原生图时，网页优先解释指标、展示或回链原生证据；解释必须贴在对应图下面，让读者在看图时就知道它测什么、不能推出什么，而不是先做一张集中指标表让读者来回对应。如果需要的指标当时没有写进 W&B、但封存 run / receipt 里有真实数据，则从封存证据建立带来源哈希的 publication mirror，再让 W&B 与网页共用这一份衍生证据。',
    antiPatterns: ['W&B 已有 Task Score 曲线，网页又手画同一训练曲线或移动平均版本却没有新增科学问题', '把所有 W&B 指标解释集中到一张总表，图和解释分离，要求读者上下翻找对应关系', '图放进网页但不解释 Task Vector、cosine、steps、entropy 各自测什么', '网页自制图和 W&B 原生图形成两套视觉事实源'],
    positiveSignals: ['每张 W&B 图下面直接写这张图在测什么、怎样读、不能直接推出什么，并在同一图块回链原生 panel', '复用 W&B-derived snapshot / panel，并在附近解释指标定义和 claim boundary', '缺失 W&B 指标但封存证据可恢复时，新建明确标注 publication-mirror / scientific_authority=false 的可视化 run，并保存来源哈希；不改封存科学 run', '缺失更新轮保持缺失，不为了图连续而插值', '网页只新增能承担独立分析语义的图，不复制已有原生曲线', '静态公开阅读层继续回链 Report / Workspace / panel，科学 authority 留在 run / receipt / sealed JSON'],
  },
  {
    id: 'CASE-094',
...

  {
    ...
    positiveSignals: ['loss → Frobenius 范数 → Task Vector 相关性 → 谱/方向 → 输出长度/任务步数 → action-family entropy', '每节都有“① 指标是什么 → ② 这次实验的结果 → ③ 分析”', '对应 W&B 图直接跟在该指标的小节里', '越复杂的指标越晚出现，并明确它比前一层多回答了什么'],
  },
