...
export const OPEN_EVO_EXPERIMENTS: OpenEvoExperimentNavigationItem[] = [
  ...
  },
  {
    ...
    childLinks: [
      ...
      { role: 'analysis', label: { zh: '后继三组对比：普通 / Bounded State / + GDR', en: 'Three-way successor: baseline / Bounded State / + GDR' }, href: `${cap}/bounded-effective-state-gdr/` },
  ...
  },
  {
    ...
    number: '06',
    title: { zh: '1.7B · 普通 OpenEVO / Bounded State / + GDR', en: '1.7B · OpenEVO / Bounded State / + GDR' },
    summary: { zh: '我们比较了普通 OpenEVO、固定 rank128 历史的 Bounded State，以及在 Bounded State 上再加 GDR 的版本；三组最终模型使用同一冻结 128 题。', en: 'We compare ordinary OpenEVO, fixed-rank128 Bounded State, and Bounded State + GDR; all three final models use the same frozen 128-task panel.' },
    motivation: { zh: '普通 SD-LoRA 历史越积越多、参数更新越来越慢；先用 Bounded State 固定历史大小，再测试 GDR 能否更好地控制新经验写入。', en: 'Growing SD-LoRA history makes parameter updates slower; first bound the history with Bounded State, then test whether GDR better controls new writes.' },
    primaryHref: `${cap}/bounded-effective-state-gdr/`,
    status: 'completed',
    lineageNote: { zh: '普通 OpenEVO 是更早独立完成的一条线；Bounded State 与 Bounded State + GDR 才是这次预注册的匹配对照。三种最终模型使用同一冻结 128 题，因此 Final 可以做描述性同题比较。', en: 'Ordinary OpenEVO is an earlier independent run; Bounded State and Bounded State + GDR form the preregistered matched contrast. All three final models use the same frozen 128-task panel, so their finals can be compared descriptively.' },
    childLinks: [
      { role: 'result', label: { zh: '三组实验、方法与冻结终评', en: 'Three experiments, methods, and frozen final' }, href: `${cap}/bounded-effective-state-gdr/` },
      { role: 'analysis', label: { zh: 'Bounded State 如何固定 rank128', en: 'How Bounded State fixes rank128' }, href: `${cap}/sd-lora-bounded-state/` },
