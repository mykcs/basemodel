...
export const OPEN_EVO_EXPERIMENTS: OpenEvoExperimentNavigationItem[] = [
  ...
  },
  {
    ...
    childLinks: [
      ...
      { role: 'analysis', label: { zh: '后继三组对比：普通 / Bounded Online Recurrence / + GDR', en: 'Three-way successor: baseline / Bounded Online Recurrence / + GDR' }, href: `${cap}/bounded-effective-state-gdr/` },
  ...
  },
  {
    ...
    number: '06',
    title: { zh: '1.7B · 普通 OpenEVO / Bounded Online Recurrence / + GDR', en: '1.7B · OpenEVO / Bounded Online Recurrence / + GDR' },
    summary: { zh: '我们比较了普通 OpenEVO、把历史压成固定 rank128 State 的 Bounded Online Recurrence，以及再加 GDR 的版本；三组最终模型使用同一冻结 128 题。', en: 'We compare ordinary OpenEVO, fixed-rank128 Bounded Online Recurrence, and Bounded Online Recurrence + GDR; all three final models use the same frozen 128-task panel.' },
    motivation: { zh: '普通 SD-LoRA 历史越积越多、参数更新越来越慢；先用 Bounded Online Recurrence 把历史压成固定 rank128 State，再测试 GDR 能否更好地控制新经验写入。', en: 'Growing SD-LoRA history makes parameter updates slower; first compress history into a fixed rank128 State with Bounded Online Recurrence, then test whether GDR better controls new writes.' },
    primaryHref: `${cap}/bounded-effective-state-gdr/`,
    status: 'completed',
    lineageNote: { zh: '普通 OpenEVO 是更早独立完成的一条线；Bounded Online Recurrence 与 Bounded Online Recurrence + GDR 才是这次预注册的匹配对照。三种最终模型使用同一冻结 128 题，因此 Final 可以做描述性同题比较。', en: 'Ordinary OpenEVO is an earlier independent run; Bounded Online Recurrence and Bounded Online Recurrence + GDR form the preregistered matched contrast. All three final models use the same frozen 128-task panel, so their finals can be compared descriptively.' },
    childLinks: [
