...
export const OPEN_EVO_EXPERIMENTS: OpenEvoExperimentNavigationItem[] = [
  ...
  },
  {
    ...
    childLinks: [
      ...
      { role: 'analysis', label: { zh: '后继三组对比：普通 / Bounded / β-gating（α=1）', en: 'Three-way successor: baseline / Bounded / β-gating (α=1)' }, href: `${cap}/bounded-effective-state-gdr/` },
  ...
  },
  {
    ...
    status: 'completed',
    lineageNote: { zh: '普通 OpenEVO 是更早独立完成的一条线；Bounded Online Recurrence 与 β-gating（α=1）才是这次预注册的匹配对照。动态 α + 动态 β 尚未运行。三种已完成最终模型使用同一冻结 128 题，因此 Final 可以做描述性同题比较。', en: 'Ordinary OpenEVO is an earlier independent run; Bounded Online Recurrence and β-gating (α=1) form the preregistered matched contrast. Dynamic α + dynamic β has not been run. The three completed final models use the same frozen 128-task panel, so their finals can be compared descriptively.' },
    childLinks: [
      ...
      { role: 'mechanism', label: { zh: 'Gated Delta 从哪里来，以及本实验为什么只有 β-gating（α=1）', en: 'Where Gated Delta comes from, and why this study only uses β-gating (α=1)' }, href: `${cap}/bounded-effective-state-gdr/#method` },
    ],
