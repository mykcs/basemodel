...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按五次主要实验组织”的入口；05 下用 SD-LoRA 加速作为中性目录分组，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个分支', '五个主实验父项必须保持首层可见；SD-LoRA 加速只表示这两条加速路线的导航集合，不是 treatment 名称；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；历史 1.7B GDR-v1 同时属于 3B + 1.7B 后继实验', '选择一次实验；进入 05 时可直接进入 Stable Reduction 或 Bounded Online Recurrence', '.study-hero__lede', { maxInteractive: 7, maxHeadings: 1, maxTextChars: 660 }),
