...
export const SITE_READER_CONTRACTS = [
  ...
  c('flow-server', '/research/seed-openevo/flow/server/', '/research/seed-openevo/flow/server/', 'operational', '快速判断实验服务器当前存储状态，并选择健康扫描、实验收尾归档或空间回收提案', '先看到当前持久文件系统约有 0.99 TiB 可用，以及 Q17 这类大实验为什么需要资产治理', '静态快照不是实时监控；远端可恢复不等于本地可删除；删除必须单独批准 exact reclaim manifest', '选择健康扫描、实验收尾归档或空间回收提案', '.server-hero__status', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 820 }),
