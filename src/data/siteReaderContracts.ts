...
export const SITE_READER_CONTRACTS = [
  ...
  c('model-detail', '/models/[id]/', '/models/kimi-k2-thinking/', 'reference', '确认一个模型是什么、能否用于当前实验', '先认出模型身份和最影响实验决策的事实', '来源、许可与实验约束', '检查研究摘要，再按需展开引用和页内导航', '.model-detail-quickfacts', { maxInteractive: 4, maxHeadings: 2, maxTextChars: 720 }),
  ...
  c('flow', '/research/seed-openevo/flow/', '/research/seed-openevo/flow/', 'choice', '建立 OpenEvo、SEED、环境和训练流程的整体心智模型', '先知道 SEED 用任务轨迹继续训练、OpenEvo 把任务经验带到后续任务，再看到固定模型和公平比较条件', '流程设计不是实验结果；条件没对齐时分数差异不能归因于方法', '进入流程对象与训练设计', '.mission-hero__lede', { maxInteractive: 1, maxHeadings: 1, maxTextChars: 520 }),
  c('flow-server', '/research/seed-openevo/flow/server/', '/research/seed-openevo/flow/server/', 'operational', '用一个例行维护入口安全推进服务器健康扫描、实验收尾和空间回收提案', '先看到 2026-09-15 22:10 的静态快照约有 0.99 TiB 可用、当前不用扩容，以及下一步先归档和验证恢复', '静态快照不是实时监控；远端可恢复不等于本地可删除；删除与公开可见性变化都必须单独批准；未合并草稿不能替代正式规则', '复制例行维护指令；需要时再选择三个单步入口', '.server-hero__status', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 1050 }),
  ...
  c('flow-sd-lora', '/research/seed-openevo/flow/sd-lora/', '/research/seed-openevo/flow/sd-lora/', 'narrative', '理解筛选后的成功轨迹怎样训练成候选 LoRA 参数，以及训练和下一轮采用为什么是两件事', '先看到每题只取最早一条通过检查的成功，SD-LoRA 在两批任务之间训练候选参数，DirectApply / 历史 GDR-v1 只是后续准入规则', '当前 WebShop 实现每轮最多回放 64 条旧经验，因此不是原论文 rehearsal-free 协议；旧方向不被重写也不保证任务分数持续提升；候选训练与准入必须分开', '比较普通 LoRA 与 SD-LoRA，再沿 128 次任务尝试进入真实更新和准入流程', '.sdlora-intro__lede'),

  c('study-briefing', '/research/seed-openevo/study/briefing/', '/research/seed-openevo/study/briefing/', 'narrative', '用固定 16:9 HTML 演讲稿讲清 OpenEVO 暑期实验怎样从 7B 结果推进到更窄的科学问题', '先看到当前结果和 evaluation protocol 边界，再回到最早 7B，沿旧 gate、64-component、失败诊断、3B、Task Vector、GDR 与 DirectApply 追踪判断怎样改变', 'SEED 论文值与 OpenEVO 本地终评不是同协议胜负；3B 仍无同口径冻结终评；DirectApply 已完成自己冻结的 128 题 final，但历史 GDR final 不是同一题集', '沿时间线理解哪些小实验排除了错误解释，以及当前为什么转向更新准入和参数机制', '#briefing-title'),
  c('study-briefing-technical', '/research/seed-openevo/study/briefing/technical-notes/', '/research/seed-openevo/study/briefing/technical-notes/', 'reference', '按需查看汇报背后的完整技术推导与实验严谨性证据', '先知道主汇报只保留科研判断，本页负责展开负向实验、Task Vector、GDR/DirectApply 和复现边界', '技术页可以保留 SHA、重复性和完整公式，但这些默认不抢占主演讲；DirectApply final 已完成，但不同 final panel 的 GDR / DirectApply 结果仍不得写成算法胜负', '展开感兴趣的技术段落，或返回暑期考核汇报', '.technical-intro'),
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '理解六次 OpenEVO × WebShop 实验为什么连续发生，以及每一次怎样把研究问题推进到下一问，再选择一组进入结果和分析', '先看到六次主要实验不是六个平行项目：参数没更新 → 长周期更新 → 小模型接口 → GDR-v1 候选准入 → DirectApply 长期参数历史 → Bounded / β-gating 长期 State；05 继续保留“两条路线说明”以及 Stable Reduction / Bounded Online Recurrence 两个分支；桌面直接展开每组的当时问题、改动、结果边界和下一问', '六个主实验父项必须保持首层可扫描；手机不展示 child-link 墙，用首屏的单行研究链保留六次实验之间的问题演进；05 下 SD-LoRA 加速只是中性目录集合，“两条路线说明”是解释入口而不是第三个 treatment；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；06 的普通 OpenEVO 是历史独立前驱，Bounded 与 β-gating（α 固定为 1）才是预注册匹配对照；dynamic α + dynamic β 未运行，不能写结果', '沿研究演进选择一次实验；需要跨实验结论时再进入 Results，需要执行时进入复现实验', '.study-hero__lede', { maxInteractive: 9, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('result-four-arm', '/research/seed-openevo/study/results/four-arm-analysis/', '/research/seed-openevo/study/results/four-arm-analysis/', 'comparison', '比较 3B/7B × self/MiniMax 四组历史结果，并先确认哪些 final 真正存在', '先看到四组结果矩阵：3B/self、7B/self、7B/MiniMax 有封存 final；3B/MiniMax 明确未运行 final，缺失不能填成 0', '模型规模与 MiniMax 启动处理是不同变量；历史四臂不是纯 teacher causal 2×2；缺失 endpoint 不能补跑旧错误算法来填表', '先读结果矩阵，再看旧 Stage 2 为什么阻断参数更新、六个组间诊断和最终 claim guardrail'),
  ...
  c('capability-gated-delta', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', 'narrative', '理解 Gated Delta 怎样从 sequence State 的 online regression 推成 residual write，以及这些数学角色怎样映射到 OpenEVO 参数 State', '先看到公式边界：论文公式到 residual 形式是严格代数等价；sequence State 到 OpenEVO parameter State 是逐角色映射，不是 k/v 与 LoRA 张量的逐元素 1:1', '详细推导必须使用真实 LaTeX；论文内部等价与跨领域映射必须分开表述；早期四轮 event-write qualification 属于映射验证证据，不定义后续 rank128 Bounded + β-gating（α 固定为 1）的全部实现；Task Vector、WebShop 分数、固定 16 题检查和 final panel 不能控制在线参数写入', '按数学因果顺序读：linear write → online regression 目标 → State 梯度 → Delta residual → Gated Delta 的 alpha/beta → 等价边界 → OpenEVO 角色映射 → gauge invariance → 早期 factor 映射与四轮资格实验', '.gds-hero__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 900 }),
  { ...c('capability-vanilla-sd-lora', '/research/seed-openevo/study/capability-exploration/vanilla-sd-lora/', '/research/seed-openevo/study/capability-exploration/vanilla-sd-lora/', 'reference', '从旧能力探索地址进入 SD-LoRA 的流程机制图', '直接进入流程理解图中的 Vanilla SD-LoRA 页面', '旧 URL 只做兼容迁移；机制图由流程理解图维护一份', '继续查看 SD-LoRA 为什么存在、何时更新参数以及一轮怎样运行', '.sdlora-intro__lede'), redirectsTo: '/research/seed-openevo/flow/sd-lora/' },
  c('capability-sd-lora-history', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', 'choice', '先分清两条并行的 SD-LoRA 加速路线，再选择七个连续问题中的下一页', '先看到 Stable Reduction 与 Bounded Online Recurrence 改的是不同对象；Study 目录里的 SD-LoRA 加速只是这两个分支的中性导航分组', '2× 与约 37× 不是同一个实现的两次测速；目录分组不能替代两个 treatment 的 branch、commit、证据和 claim boundary', '进入 Stable Reduction、Bounded Online Recurrence，或按七个问题继续阅读', '.sdlora-series-hero h1', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 700 }),
  c('capability-sd-lora-equivalence', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', 'narrative', '理解 SD-LoRA v2 · Stable Reduction 改了什么、为什么它约快 2×，以及它与 Bounded Online Recurrence 为什么不是同一条线', '先看到 Stable Reduction 的身份和它仍逐个保留历史 component；后面的正文再解释约 2× 与 WebShop 资格，以及 Study 里的 SD-LoRA 加速只是中性目录分组', 'Study 目录里的 SD-LoRA 加速只是导航分组；SD-LoRA v2 这个正式名称只属于本页的 Stable Reduction treatment。16-task 非-final panel 只支持 bounded local qualification，不能升级成全局等价、统计显著更优或 bounded-state 结论', '查看 Stable Reduction 证据，或切到 Bounded Online Recurrence 看固定 rank128 的独立结果', '.sdlora-v2__chapter', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 900 }),
  ...
  c('capability-sd-lora-bounded-state', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-state/', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-state/', 'focus', '理解 Bounded Online Recurrence 是否真的能让 SD-LoRA 连续学习时保持固定大小历史状态', '先看到 R150–R159 连续更新保持 rank128、九轮 hard gate 全 PASS，并且同轮 Vanilla trainer 对比平均约 37×', '这只支持封存 R150–R159 历史窗口中的 bounded recurrence；它不是 SD-LoRA v2，也不证明逐 bit Vanilla 等价、protected-final 效果或所有 continual learning 都 O(1)', '查看四个行为 gate 与证据，再按需回到 Stable Reduction 对照', '.bounded__eyebrow', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 900 }),
  c('capability-sd-lora-bounded-acceleration', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-acceleration/', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-acceleration/', 'narrative', '理解我们为什么停止继续优化 Bounded 的 SVD / compression，并把下一问转向训练与 optimizer recurrence', '先看到 Bounded 的主要时间已经在真实训练：steady-state 1.7B 训练 4.26 秒、compression 0.49 秒；四卡 compression 虽然可以逐 tensor exact，但当前没有变快', '这些都是 GPU0–3 engineering smoke，formal B2/B3 仍未关闭；顺序反例只排除朴素重排/独立 chunk，不证明所有 full-state exact transfer 都不存在', '沿证据判断 B、Muon/SVD 与 naive A 分别为什么被降级，再进入 exact full-state transfer 的 A0/A1 问题', '.ba-accel__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 880 }),
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三条已完成 Qwen3-1.7B OpenEVO 参数演变路线分别解决什么问题，并能继续回答研究讨论中自然出现的实现追问：rank128 为什么这样选、β controller 从什么历史数据和 target 学出来、正式 160 轮怎样冻结并实际写入、Task Score / exact success / validity 如何区分、Carrier 到底由规则还是 LM 决定，以及当前证据还有哪些问题没回答', '第一屏先看到三组已完成路线和关键结果：参数更新 31.09h → 2.02h、同题终评 60.72 → 45.98 → 20.77；后文再进入完整技术层：rank128 与 rank95 8/7、Gated Delta 角色映射、β 离线训练与 runtime 代码、Carrier gate / generator / validator、指标语义与后验诊断；动态 α+β 保留为空白实验位但不是当前第一优先级', '固定 128 题只是同题窄切片；最后 20 轮是 20×128=2,560 次变化任务 rollout 的 round-level 均值，不是 retention test；Task Score、Exact Success 与 parser/action validity 是三个不同账本；普通 OpenEVO 是历史前序实验，不是正式匹配对照第三 arm；15.36× 只属于参数更新，整个 Stage2 约 1.33×；rank95 8/7 不是 rank8 能力保持证明；论文 S/k/v 到参数 State 是角色映射，runtime 不做“参数×observation→action”；β controller 离线训练后冻结、22 项来自局部 LoRA/gradient/Adam 状态；R159 cross-arm direction 不能替代 β 组上升期 vs 掉分期的 phase-conditioned 分析；所有后分析都不能升级成因果结论', '按研究链阅读：参数化长期记忆定位 → SD-LoRA 增长 → Bounded/rank128 → 后验 rank95 8/7 → Gated Delta 数学来源与实现边界 → β 的训练数据/target/22-feature MLP/qualification/runtime/实际范围 → OPSD Stage1 与 SEED SFT 区别 → Carrier deterministic gate→LM candidate→validator/repair→UPDATE/KEEP PRIOR → Task Score/Exact Success/validity → last20 与 fixed final → loss→norm→Task Vector→谱/方向→路径→entropy → 未回答的 phase-conditioned 问题和当前实验优先级；W&B 与 GitHub 源码/收据紧跟对应 claim', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 2100 }),
  c('capability-q17-directapply-analysis', '/research/seed-openevo/study/capability-exploration/q17-directapply-analysis/', '/research/seed-openevo/study/capability-exploration/q17-directapply-analysis/', 'narrative', '在一页里先确认 DirectApply 160 轮最终到了哪里，再用训练、载体、延迟和 D1 诊断解释这条轨迹', '先看到实验身份和唯一冻结 Final：60.72 / 100、50/128 exact success；随后才进入训练轨迹、可靠性、载体、SD-LoRA latency、参数几何和行为保持', 'R159=72.90 只是训练轮分数；R127→R128 只能支持局部能力重新分配而非全局遗忘；latency scaling 是机制诊断而非能力缺陷；Text Memory NOOP 是运行语义修复而非新算法成功；D1 不修改 final state；历史 GDR 37.60 使用不同 final panel，不能直接当方法差', '按 Final → 训练/loss → 行为诊断 → 载体/延迟 → D1 geometry → function preservation → synthesis 阅读，再按需展开原始证据', '.q17-analysis__hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1050 }),  c('capability-q17-frontier', '/research/seed-openevo/study/capability-exploration/q17-directapply-frontier/', '/research/seed-openevo/study/capability-exploration/q17-directapply-frontier/', 'focus', '看懂进入第 127 轮和第 128 轮的两个模型状态在同一组 32 道题上到底差多少', '先看到不同题训练轮从 76.1 掉到 48.9，而同题重测只有 63.58 → 59.41、完整做对 10 → 8，并明确这是局部诊断', '32 题来自训练/开发范围且与最终锁定 128 题不重合；95% 范围包含无差别；DirectApply 后来完成 160 轮和独立 final，本页不能外推整条轨迹或 final', '查看同题证据；需要时再展开历史排查计划，最终状态交给 DirectApply 完整曲线 / final 页面', '[data-q17-advisor-diagnostics] .lede', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 900 }),
  ...
  c('capability-stage2-analysis', '/research/seed-openevo/study/capability-exploration/stage2-7b-analysis/', '/research/seed-openevo/study/capability-exploration/stage2-7b-analysis/', 'focus', '理解 7B 为什么从零次更新走到真实参数学习，以及这些参数证据能不能说明新任务能力提高', '先看到直接答案：修订规则后真实参数更新发生；但参数变化和训练任务多样性都不能替代独立的新任务效果证据', '旧 Stage 2 的 0 次更新是程序门槛事实；Ceiling-1.0 的参数更新证明训练发生，不证明可靠迁移；更早 7B 对照属于独立历史实验', '先看直接答案，再看学习规则、Task Vector/容量诊断、训练任务多样性和原始证据'),
