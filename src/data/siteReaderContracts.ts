...
export const SITE_READER_CONTRACTS = [
  ...
  c('study-briefing', '/research/seed-openevo/study/briefing/', '/research/seed-openevo/study/briefing/', 'narrative', '用 HTML 演讲稿讲清 OpenEVO 暑期实验怎样从 SEED / WebShop 研究设置推进到更窄的科学问题', '先知道 OpenEVO 是什么、WebShop 在测什么、SEED 为什么是预算与结果参照，再进入 7B、1.7B / 3B、负向诊断、TaskVector、GDR 与 DirectApply', 'SEED 论文值与 OpenEVO 本地 128 题结果不是同协议胜负；3B 当前没有可直接并入冻结终评表的最终分数；DirectApply 尚无最终分数', '沿时间线理解每个假设从哪个前因产生、哪些小实验排除了错误解释，以及当前为什么转向更新准入和参数机制', '#briefing-title'),
