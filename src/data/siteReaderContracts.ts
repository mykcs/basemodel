...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按六次主要实验组织”的入口；05 下用 SD-LoRA 加速作为中性目录分组，先给一条“两条路线说明”，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个 treatment 分支；06 单列三组 1.7B 实验：普通 OpenEVO / Bounded State / Bounded State + GDR', '六个主实验父项必须保持首层可见；SD-LoRA 加速只表示导航集合，“两条路线说明”是解释入口而不是第三个 treatment；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；06 的普通 OpenEVO 是历史独立前驱，Bounded State 与 Bounded State + GDR 才是预注册匹配对照', '选择一次实验；进入 05 时可先看两条路线说明，再分别进入 Stable Reduction 或 Bounded Online Recurrence；进入 06 先看三行消融表，再读方法、结果与分析', '.study-hero__lede', { maxInteractive: 9, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三组 Qwen3-1.7B OpenEVO 实验为什么从普通 SD-LoRA 走到 Bounded State，再到 Bounded State + GDR，以及结果怎样', '先看到一张三行消融表：普通 OpenEVO 60.72、Bounded State 45.98、Bounded State + GDR 20.77；对勾直接说明每组增加了什么，再进入动机和方法', '三次 Final 同一冻结 128 题，但普通 OpenEVO 是更早独立实验，不是后两组预注册匹配对照的第三个 arm；13–15× 只属于参数更新阶段；后分析不能升级成因果结论', '沿论文式顺序读：SD-LoRA 变慢的动机 → Bounded State → GDR 从 linear attention 到参数 State 的数学映射 → 实验结果 → Task Vector / 范数 / 方向 / 步数 / entropy / adaptive 讨论', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1050 }),
