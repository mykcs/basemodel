...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三条已完成 Qwen3-1.7B OpenEVO 参数演变路线分别解决什么问题：原始 SD-LoRA 为什么变慢、Bounded 为什么先用 rank128、正式实验后为何出现 rank95 中位数 8 / 7、继续加入 β 组件实际怎样实现，以及这些结果如何形成当前卡点和下一实验优先级', '第一屏先看到三组已完成路线和关键结果：参数更新 31.09h → 2.02h、同题终评 60.72 → 45.98 → 20.77；随后解释 rank128 是保守容量选择而非理论最优，后验 rank95=8/7 只提示低维结构、不证明长期 rank8 足够；动态 α+β 继续保留为空白实验位，但不再冒充当前第一优先级', '固定 128 题是在训练前从 held-out goal_idx 0–499 中排除上一套 final 后按固定 SHA256 规则无视 reward/success 选出的同一套窄切片；普通 OpenEVO 是历史前序实验，不是后两组预注册匹配对照的第三个 treatment；15.36× 只属于参数更新部分，整个 Stage2 只有约 1.33×；rank95 8/7 是后验几何诊断，不是 rank8 能力保持证明；UPDATE/NOOP 不等于运行时使用或能力贡献；β controller 的 22 项输入来自局部 LoRA/gradient/Adam 状态，不是 rank128 State 的 22 维压缩；所有后分析都不能升级成因果结论', '按一条研究链阅读：原始 SD-LoRA 的增长问题 → Bounded 与 rank128 选择 → 后验 rank95 8/7 → Gated Delta 原式与 β 组件真实 22-feature frozen-controller 实现 → 三组实验与 Carrier 账本 → Task Score / final / compute → loss → norm → Task Vector → 谱与方向 → 路径长度 → entropy → 当前卡点；下一步先补 SFT/OPSD/SEED 基线，再做 rank 容量消融和简单 β scaling，动态 α+β 后置；W&B 与 GitHub 证据紧跟对应 claim', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 2100 }),
