...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-gated-delta', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', 'narrative', '理解当前 Gated-Delta SD-LoRA 怎样在每次学习时直接更新 LoRA 参数，以及这条机制现在已经证明到哪一步', '先分清两件事：旧 GDR-v1 是训练后再筛候选；现在的 Gated Delta 已经在一次真实 WebShop 学习里改过参数，但完整 Vanilla-vs-GDR 四轮配对实验还没完成', 'Task Vector、WebShop 分数和固定 16 题检查都不能控制在线参数写入；真实执行只能证明机制能运行，不能升级成 GDR 优于 Vanilla 或最终成绩提升', '沿参数更新机制理解 State 与 beta，再按需展开真实执行证据；旧 GDR-v1 / DirectApply 历史另有独立页面', '.gds-hero__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 760 }),
