...
export const SITE_READER_CONTRACTS = [
  ...
  c('flow-server', '/research/seed-openevo/flow/server/', '/research/seed-openevo/flow/server/', 'operational', '用一个例行维护入口安全推进服务器健康扫描、实验收尾和空间回收提案', '先看到当前持久文件系统约有 0.99 TiB 可用，以及默认入口会按 current canonical Passport / Registry authority 自动推进三个阶段', '静态快照不是实时监控；未合并 Draft 不是全项目 authority；远端可恢复不等于本地可删除；删除与公开可见性变化都必须单独批准', '默认复制例行服务器维护 Prompt；需要时再选择三个单步入口', '.server-hero__status', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 1180 }),
  ...
  c('flow-sd-lora', '/research/seed-openevo/flow/sd-lora/', '/research/seed-openevo/flow/sd-lora/', 'narrative', '先理解为什么 OpenEvo 需要一条参数写回路径、SD-LoRA 在一轮里的位置和时机，再进入真实更新机制', '先看到成功 rollout 本身不会自动改变下一轮参数；SD-LoRA 位于训练数据筛选之后、下一批任务之前，只负责参数更新这一条 carrier；随后再区分普通 LoRA 与 SD-LoRA', 'OpenEvo 不能等同于 SD-LoRA；当前语言 Agent 实现使用 bounded replay 且 paper_equivalent=false / rehearsal_free=false；SD-LoRA candidate 训练与 GDR/DirectApply 状态准入必须分开', '先比较普通 LoRA 与 SD-LoRA，再沿 128 次任务尝试进入一轮参数更新和 candidate 准入边界', '.sdlora-intro__lede'),
