...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三组 Qwen3-1.7B OpenEVO 实验为什么从普通 SD-LoRA 走到 Bounded Online Recurrence，再到 Bounded Online Recurrence + GDR，以及结果怎样', '先看到一张三行消融表：普通 OpenEVO 60.72、Bounded Online Recurrence 45.98、+GDR 20.77；对勾直接说明每组增加了什么，再进入动机和方法', '三次冻结终评使用同一份 128 题，但普通 OpenEVO 是更早独立实验，不是后两组预注册匹配对照的第三组；13–15× 只属于参数更新阶段；后分析不能升级成因果结论', '沿论文式顺序读：SD-LoRA 变慢的动机 → Bounded Online Recurrence → GDR 从 linear attention 到参数 State 的数学映射 → 实验结果 → Task Vector / 范数 / 方向 / 步数 / entropy / 自适应约束讨论', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1050 }),
