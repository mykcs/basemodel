...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-gated-delta', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', 'narrative', '理解当前 Gated-Delta SD-LoRA 怎样在每次学习时直接更新 LoRA 参数，以及四轮 Vanilla-vs-GDR 配对实验目前支持到哪一步', '先看到四轮 paired qualification 已全部封存：在冻结 schedule 上，GDR 的 pooled reward 与 exact success 都高于匹配 Vanilla 对照', 'Task Vector、WebShop 分数、固定 16 题检查和 final panel 都不能控制在线参数写入；4/4 qualification 只支持当前冻结 schedule 内的比较，不能升级成普遍优越、final 提升或单一子机制因果结论', '沿参数更新机制理解 State 与 beta，再核对四轮 pooled 结果、matched-cluster bootstrap 与 claim boundary', '.gds-hero__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三组 Qwen3-1.7B OpenEVO 实验分别改了什么，并分清训练后期 Task Score、固定 128 题终评与 W&B 诊断指标各自在测什么', '先看到三行消融表：每组一句话说明方法区别、Bounded/GDR 对勾、最后 20 轮平均 Task Score 67.07 / 64.33 / 54.28，以及同一固定 128 题终评 60.72 / 45.98 / 20.77；紧接着解释 Task Score 算法与题集来源', '固定 128 题是在训练前从 held-out goal_idx 0–499 中排除上一套 final 后按固定 SHA256 规则无视 reward/success 选出的同一套题，它只是一个窄切片；最后 20 轮每轮任务会变化，属于训练轨迹而非同题考试；普通 OpenEVO 是更早独立实验，不是后两组预注册匹配对照的第三组；13–15× 只属于参数更新阶段；后分析不能升级成因果结论', '先读三组方法和两种分数口径，再沿论文式顺序读：SD-LoRA 变慢的动机 → Bounded Online Recurrence → GDR 数学映射 → 训练后期 / 固定终评 → Task Vector / 范数 / 方向 / 步数 / entropy → W&B 原生曲线与指标说明；loss 图必须以累计 rollout 为横轴，明确每轮 128、总计 20,480，缺失更新轮不插值，并说明 loss 下降不等于 Task Score 或固定终评上升', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1800 }),
