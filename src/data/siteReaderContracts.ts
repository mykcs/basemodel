...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-sd-lora-history', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', 'choice', '理解这组 SD-LoRA 页面共同研究什么，并选择七个连续问题中的下一页', '先看到从“历史越来越重”到“能否形成固定大小学习状态”的完整问题链，而不是先看到一种预设方法', '后五页包含正在验证的科学问题；未完成实验不能显示成已证明结论', '按顺序从 Vanilla 机制开始，或直接进入自己关心的问题', '.sdlora-series-hero h1', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 650 }),
  c('capability-sd-lora-equivalence', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', 'narrative', '理解为什么简单并行化可能改变严格 Vanilla SD-LoRA 的训练轨迹', '先认出本页研究的是执行等价边界，而不是一种新学习算法', '数学公式相同不自动证明 BF16 backward 的有效数值语义相同；本页后续只填可追溯证据', '回到历史增长问题，判断工程困难是否指向更深的科学问题'),
