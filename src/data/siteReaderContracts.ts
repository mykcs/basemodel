...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-gdr', '/research/seed-openevo/study/capability-exploration/gdr-directapply/', '/research/seed-openevo/study/capability-exploration/gdr-directapply/', 'narrative', '理解历史本地 GDR-v1 怎样在 SD-LoRA candidate 已经训练完成后做 16 题放行判断，以及 DirectApply 怎样移除这层 veto', '先看到历史实验实际产生 44 个 candidate、只让 7 次更新进入下一轮，并明确这是 candidate admission 实验而不是原始 recurrent Gated Delta 机制', '44→7、16-task probe、DirectApply 对照与不同 frozen final panel 的边界必须保留；当前 Gated-Delta 推导由独立页面负责', '继续查看历史 DirectApply 证据，或进入当前 Gated-Delta SD-LoRA 推导'),
  c('capability-gated-delta', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', '/research/seed-openevo/study/capability-exploration/gated-delta-sd-lora/', 'narrative', '理解当前 Gated-Delta SD-LoRA 怎样把真实 learning event 变成 recurrent LoRA-factor state write，以及这条路径现在到底证明到哪一步', '先明确旧 GDR-v1 是 candidate admission；当前机制已经通过 Route S 在真实 WebShop learning opportunity 上完成参数写入，但完整 Vanilla-vs-GDR paired D1 仍未完成', 'Task Vector / score / probe 不进入 runtime control；Route S 只能支持 execution proof，不能升级成 GDR 优于 Vanilla 或 final 提升结论', '沿机制推导理解 factor state 与 beta，再查看 Route S execution seal；历史 admission 实验另见 GDR-v1 / DirectApply 页', '.gds-hero__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('capability-sd-lora-history', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', 'choice', '理解这组 SD-LoRA 页面共同研究什么，并选择七个连续问题中的下一页', '先看到从“历史越来越重”到“能否形成固定大小学习状态”的完整问题链，而不是先看到一种预设方法', '前 3 页已有封存的机制 / 扩展性 / 加速结果；后 4 页若仍未完成，就不能提前显示成已证明结论', '按顺序从 Vanilla 机制开始，或直接进入自己关心的问题', '.sdlora-series-hero h1', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 650 }),
  c('capability-sd-lora-equivalence', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', 'narrative', '理解严格等价加速为什么失败、为什么我们转而明确做 SD-LoRA v2，以及 v2 的加速和非-final WebShop 验证到底支持什么', '先看到 SD-LoRA v2 确实能更快训练；它不是原算法一模一样的复制品，但小规模真实 WebShop 检查没有发现它因为加速而明显变笨', 'SD-LoRA v2 是新的 numerical treatment，不是 Vanilla 的严格等价替代；16-task 非-final panel 只支持 bounded local qualification，不能升级成全局等价、统计显著更优、final-panel 结论或 GDR 比较', '把 v2 冻结成 fast baseline；等 GDR 独立封存后，再设计 Vanilla / v2 / GDR 的 prospective matched comparison', '.sdlora-v2__lede', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 900 }),
  ...
  c('capability-sd-lora-bounded-state', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-state/', '/research/seed-openevo/study/capability-exploration/sd-lora-bounded-state/', 'narrative', '在已有实验约束下理解固定大小学习状态是否值得设计，以及哪些方法只是候选工具', '先看到方法设计必须由前面关于必要历史信息的实验结果约束', 'GDR、WY、chunkwise 目前是候选方法或实现工具，不能在 S1–S4 结果前写成已验证答案', '依据前页证据再进入具体方法设计或回到工程加速线'),
