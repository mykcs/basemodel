...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按五次主要实验组织”的入口；05 下用 SD-LoRA v2 作为目录分组，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个分支', '五个主实验父项必须保持首层可见；SD-LoRA v2 在这里仅是 05 下的导航分组，两个分支的 treatment 身份与证据仍分别保留；历史 1.7B GDR-v1 同时属于 3B + 1.7B 后继实验', '选择一次实验；进入 05 时可直接进入 Stable Reduction 或 Bounded Online Recurrence', '.study-hero__lede', { maxInteractive: 7, maxHeadings: 1, maxTextChars: 660 }),
  ...
  c('capability-sd-lora-history', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', '/research/seed-openevo/study/capability-exploration/sd-lora-history/', 'choice', '先分清两条并行的 SD-LoRA 加速路线，再选择七个连续问题中的下一页', '先看到 Stable Reduction 与 Bounded Online Recurrence 改的是不同对象；Study 目录里的 SD-LoRA v2 只是这两个分支的导航分组', '2× 与约 37× 不是同一个实现的两次测速；目录分组不能替代两个 treatment 的 branch、commit、证据和 claim boundary', '进入 Stable Reduction、Bounded Online Recurrence，或按七个问题继续阅读', '.sdlora-series-hero h1', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 700 }),
  c('capability-sd-lora-equivalence', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', '/research/seed-openevo/study/capability-exploration/sd-lora-equivalence/', 'narrative', '理解 SD-LoRA v2 · Stable Reduction 改了什么、为什么它约快 2×，以及它与 Bounded Online Recurrence 为什么不是同一条线', '先看到 Stable Reduction 的身份和它仍逐个保留历史 component；后面的正文再解释约 2× 与 WebShop 资格，以及 Study 里的 SD-LoRA v2 只是目录分组', 'Study 目录里的裸 SD-LoRA v2 只是导航分组；本页 treatment 的完整名称是 SD-LoRA v2 · Stable Reduction。16-task 非-final panel 只支持 bounded local qualification，不能升级成全局等价、统计显著更优或 bounded-state 结论', '查看 Stable Reduction 证据，或切到 Bounded Online Recurrence 看固定 rank128 的独立结果', '.sdlora-v2__chapter', { maxInteractive: 4, maxHeadings: 1, maxTextChars: 900 }),
