...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按六次主要实验组织”的入口；05 下用 SD-LoRA 加速作为中性目录分组，先给一条“两条路线说明”，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个 treatment 分支；06 单列三组已完成 1.7B 实验：普通 OpenEVO / Bounded Online Recurrence / β-gating（α=1），另留 dynamic α + dynamic β 的未做空位', '六个主实验父项必须保持首层可见；SD-LoRA 加速只表示导航集合，“两条路线说明”是解释入口而不是第三个 treatment；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；06 的普通 OpenEVO 是历史独立前驱，Bounded Online Recurrence 与 β-gating（α=1）才是预注册匹配对照；dynamic α + dynamic β 未运行，不能写结果', '选择一次实验；进入 05 时可先看两条路线说明，再分别进入 Stable Reduction 或 Bounded Online Recurrence；进入 06 先看三组已完成实验与一个未做空位，再读方法、结果与分析', '.study-hero__lede', { maxInteractive: 9, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三组已完成 Qwen3-1.7B OpenEVO 实验分别改了什么，并分清 Gated Delta 的序列公式来源、实际只启用 β-gating（α=1）的实验边界、训练后期 Task Score、固定 128 题终评与 W&B 诊断指标', '先看到三组已完成实验和一个未做空位：方法列拆成 Bounded / 动态 β / 动态 α；已完成第三组明确 β ✓、α —，动态 α+β 行的结果保持为空；最后 20 轮平均 Task Score 67.07 / 64.33 / 54.28，同一固定 128 题终评 60.72 / 45.98 / 20.77', '固定 128 题是在训练前从 held-out goal_idx 0–499 中排除上一套 final 后按固定 SHA256 规则无视 reward/success 选出的同一套题，它只是一个窄切片；最后 20 轮每轮任务会变化，属于训练轨迹而非同题考试；普通 OpenEVO 是更早独立实验，不是后两组预注册匹配对照的第三组；13–15× 只属于参数更新阶段；sequence State → parameter State 不是 tensor 1:1；后分析不能升级成因果结论', '先读三组已完成方法、未做 dynamic α+β 空位和两种分数口径，再沿论文式顺序读：SD-LoRA 变慢的动机 → Bounded Online Recurrence → Gated Delta 简短推导 → 本实验 β-gating（α=1）边界 → 需要完整证明时进入 Gated-Delta 子页 → 训练后期 / 固定终评 → Task Vector / 范数 / 方向 / 步数 / entropy → W&B 原生曲线与指标说明；loss 图必须以累计 rollout 为横轴，明确每轮 128、总计 20,480，缺失更新轮不插值，并说明 loss 下降不等于 Task Score 或固定终评上升', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1900 }),
