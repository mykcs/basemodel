...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解固定 rank128 以后，为什么还要比较“直接写入新经验”和“由 Effective-State GDR 控制写入强度”', '先看到同一个 1.7B 从共同起点分成 OFF / ON 两条 160 轮正式实验，并明确实验正在运行、正式结果尚未产生', '第一代 Gated-Delta、Bounded recurrence 和 Effective-State 后继方法是三个不同科学对象；运行中分数、中途 W&B、单个 checkpoint、资格检查数字和资源调度状态都不能升级成正式效果结论', '先理解 OFF / ON 唯一差异；再看两条独立时间线如何在最终分析时重新配对；最后等待封存的正式结果', '.research-route-context [data-reader-purpose]', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 900 }),
