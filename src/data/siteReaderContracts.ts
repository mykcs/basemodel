...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按六次主要实验组织”的入口；05 下用 SD-LoRA 加速作为中性目录分组，先给一条“两条路线说明”，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个 treatment 分支；06 单列已经完成的 Bounded OFF / Effective-State GDR ON 正式实验', '六个主实验父项必须保持首层可见；SD-LoRA 加速只表示导航集合，“两条路线说明”是解释入口而不是第三个 treatment；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；06 是 DirectApply 之后的新正式实验，不是 DirectApply 的第三条 arm', '选择一次实验；进入 05 时可先看两条路线说明，再分别进入 Stable Reduction 或 Bounded Online Recurrence；进入 06 可直接看 OFF / ON 封存结果', '.study-hero__lede', { maxInteractive: 9, maxHeadings: 1, maxTextChars: 760 }),
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解固定 rank128 以后，Bounded OFF 与 Effective-State GDR ON 的正式 160 轮对照最终得到什么', '先看到同一冻结 128 题上的三个 Final：DirectApply 60.72、Bounded OFF 45.98、Bounded+GDR ON 20.77；再看 OFF/ON 训练期长期差值与参数/计算诊断', 'DirectApply 是历史前驱，不是 OFF/ON 预注册对照的第三条 arm；R1–R159 长期训练平均与冻结 Final 回答不同问题；trainer 约 13–15× 加速不能写成整个 Stage2 约 13–15× 加速', '先读同题 Final，再读 OFF/ON 长周期区间和计算效率；需要解释后期差异时，再看 Task Vector、更新方向、输出长度与 adaptive GDR 的只读后分析，最后展开证据边界', '.research-route-context [data-reader-purpose]', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 1050 }),
