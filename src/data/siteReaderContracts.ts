...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解为什么 Bounded 解决状态大小以后，还要把 GDR 的控制对象改成 effective state / effective write，以及新的正式 OFF/ON 实验比较什么', '先看到 rank128、OFF/ON 唯一处理差异、正式实验正在运行且两条臂独立推进，以及“正式结果仍 Pending”', 'first-generation Gated-Delta D1、Bounded recurrence 和 Effective-State successor 是三个不同科学对象；运行中 round、partial W&B、单 checkpoint、qualification-only 数字和资源调度状态都不能升级成 formal efficacy', '先理解为什么控制 effective state；再看两条独立 160 轮时间线如何在最终分析时重新配对；最后等待 sealed formal result', '.esg__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 900 }),
