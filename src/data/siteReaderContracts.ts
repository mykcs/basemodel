...
export const SITE_READER_CONTRACTS = [
  ...
  c('study', '/research/seed-openevo/study/', '/research/seed-openevo/study/', 'choice', '先选择自己要看的那一次 OpenEVO × WebShop 实验，再沿它的结果和分析继续阅读', '先看到“按五次主要实验组织”的入口；05 下用 SD-LoRA 加速作为中性目录分组，先给一条“两条路线说明”，再缩进 Stable Reduction 与 Bounded Online Recurrence 两个 treatment 分支', '五个主实验父项必须保持首层可见；SD-LoRA 加速只表示导航集合，“两条路线说明”是解释入口而不是第三个 treatment；SD-LoRA v2 只属于 Stable Reduction，Bounded Online Recurrence 保持独立；历史 1.7B GDR-v1 同时属于 3B + 1.7B 后继实验', '选择一次实验；进入 05 时可先看两条路线说明，再分别进入 Stable Reduction 或 Bounded Online Recurrence', '.study-hero__lede', { maxInteractive: 8, maxHeadings: 1, maxTextChars: 700 }),
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解为什么 Bounded 解决状态大小以后，还要把 GDR 的控制对象改成 effective state / effective write，以及新的正式配对实验比较什么', '先看到 rank128 bounded state、OFF/ON 正式设计和“正式结果尚未产生”；预启动工程已经完成，但 launch authority 仍为 false', 'first-generation Gated-Delta D1、Bounded recurrence 和 Effective-State successor 是三个不同科学对象；qualification-only 数字不能升级成 formal efficacy，partial W&B 也不是结果 authority', '沿 1.052 factor boundary、657.836 controller boundary、Effective-State sequential write 和 160-round matched design 阅读，再等待 sealed formal result', '.esg__lede', { maxInteractive: 2, maxHeadings: 1, maxTextChars: 900 }),
