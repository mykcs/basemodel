...
const c = (
  ...
  nextStep: string,
  firstViewportSelector?: string,
): SiteReaderContract => ({ id, sourceRoute, samplePath, attentionMode, audience, primaryTask, firstViewportGoal, mustStayVisible, nextStep, firstViewportSelector });

export const SITE_READER_CONTRACTS = [
  c('home', '/', '/', 'choice', '判断这个站点能帮助自己完成什么研究任务', '先认出 SEED × OpenEvo 研究主题和两个主要入口', '研究对象与两条主线的区别', '选择流程理解或实验研究入口'),
  ...
  c('models-index', '/models/', '/models/', 'choice', '从模型目录中找到适合继续查看的模型', '先知道这是模型目录以及如何筛选', '目录对象与筛选含义', '选择一个模型查看详情'),
  c('model-detail', '/models/[id]/', '/models/kimi-k2-thinking/', 'reference', '确认一个模型是什么、能否用于当前实验', '先认出模型身份和最影响实验决策的事实', '来源、许可与实验约束', '检查证据或加入比较'),
  ...
  c('papers-index', '/papers/', '/papers/', 'choice', '找到与实验问题相关的论文', '先知道论文目录覆盖什么研究证据', '论文报道与本地复现不是同一证据层', '选择论文查看方法与证据'),
  c('paper-detail', '/papers/[id]/', '/papers/metagpt/', 'reference', '理解一篇论文做了什么以及本项目能复现什么', '先认出论文主张、方法和本项目关系', '论文原始主张与本地验证边界', '检查模型角色、设置或复现路径'),
  c('guide', '/guide/', '/guide/', 'choice', '找到适合自己的复现入口', '先判断自己要做严格复现、方法复现还是现代化重跑', '不同复现等级不能混写', '进入对应复现步骤'),
  c('guide-today', '/guide/today/', '/guide/today/', 'operational', '完成当天需要执行的实验准备或运行步骤', '先看到现在该做什么和什么不能做', '当前授权与安全边界', '执行下一个明确步骤'),
  c('landscape', '/landscape/', '/landscape/', 'choice', '在模型与研究版图中找到相关对象', '先知道这张图按什么维度组织信息', '目录关系不等于能力排序', '选择对象继续查看'),
  c('methodology', '/methodology/', '/methodology/', 'reference', '理解站点如何区分事实、推断和未知', '先认出证据等级和比较边界', '未知必须保持未知', '按方法规则返回具体研究页面'),
  c('workspace', '/workspace/', '/workspace/', 'operational', '把模型、资源和实验约束组合成可执行决策', '先看到当前任务、候选和下一动作', '约束与证据不能被自动补猜', '保存、比较或修改实验条件'),
  c('data-status', '/data-status/', '/data-status/', 'reference', '判断站点数据的更新时间和可信范围', '先知道哪些数据已核验、哪些仍未知', '新鲜度和来源边界', '回到受影响的数据或页面'),
  c('lab', '/lab/', '/lab/', 'operational', '理解实验服务器资源与可安全操作范围', '先看到当前资源事实和操作边界', '共享硬件、授权和所有权边界', '选择安全的实验或存储动作'),

  c('flow', '/research/seed-openevo/flow/', '/research/seed-openevo/flow/', 'choice', '建立 OpenEvo、SEED、环境和训练流程的整体心智模型', '先看整体流程和各对象角色', '流程关系不等于实验结果', '选择一个对象或阶段深入理解'),
  c('flow-server', '/research/seed-openevo/flow/server/', '/research/seed-openevo/flow/server/', 'reference', '理解服务器在实验流程中的角色', '先知道服务器提供什么资源而不承担什么科学语义', '资源事实与实验授权分开', '返回流程或查看实验资源'),
  ...
  c('study-briefing', '/research/seed-openevo/study/briefing/', '/research/seed-openevo/study/briefing/', 'narrative', '向课题组快速汇报项目从问题定义到当前阻塞的研究进展', '先看到项目已经从“能跑”推进到“可信测量与单变量归因”，以及当前 Q17 仍暂停', '7B 完整结果、OpenEVO vs SEED 尚未完成的边界、Q17 PAUSED 与确定性 gate 必须可见', '按页查看探索路线、硬证据、质量体系和下一步', '[data-briefing-primary]'),
  ...
  c('capability-home', '/research/seed-openevo/study/capability-exploration/', '/research/seed-openevo/study/capability-exploration/', 'choice', '在能力探索的几条研究路径中选择要看的问题', '先知道每条路径回答哪个科学问题', '历史、后继设计和机制实验不能混成一个当前状态', '选择一条研究路径'),
  ...
  c('capability-gdr', '/research/seed-openevo/study/capability-exploration/gdr-directapply/', '/research/seed-openevo/study/capability-exploration/gdr-directapply/', 'narrative', '理解 GDR 到 DirectApply 的决策原因', '先看到 GDR 实际过滤了什么以及为什么需要消融', '机制改变与效果证明分开', '查看候选、应用更新与消融证据'),
