...
export const SITE_READER_CONTRACTS = [
  ...
  c('flow-sd-lora', '/research/seed-openevo/flow/sd-lora/', '/research/seed-openevo/flow/sd-lora/', 'narrative', '先理解为什么 OpenEvo 需要一条参数写回路径、SD-LoRA 在一轮里的位置和时机，再进入真实更新机制', '先看到成功 rollout 本身不会自动改变下一轮参数；SD-LoRA 位于训练数据筛选之后、下一批任务之前，只负责参数更新这一条 carrier；随后再区分普通 LoRA 与 SD-LoRA', 'OpenEvo 不能等同于 SD-LoRA；当前语言 Agent 实现使用 bounded replay 且 paper_equivalent=false / rehearsal_free=false；SD-LoRA candidate 训练与 GDR/DirectApply 状态准入必须分开', '先比较普通 LoRA 与 SD-LoRA，再沿 128 次任务尝试进入一轮参数更新和 candidate 准入边界', '.sdlora-intro'),
  ...
  { ...c('capability-vanilla-sd-lora', '/research/seed-openevo/study/capability-exploration/vanilla-sd-lora/', '/research/seed-openevo/study/capability-exploration/vanilla-sd-lora/', 'reference', '从旧能力探索地址进入 SD-LoRA 的流程机制图', '直接进入流程理解图中的 Vanilla SD-LoRA 页面', '旧 URL 只做兼容迁移；机制图由流程理解图维护一份', '继续查看 SD-LoRA 为什么存在、何时更新参数以及一轮怎样运行', '.sdlora-intro'), redirectsTo: '/research/seed-openevo/flow/sd-lora/' },
