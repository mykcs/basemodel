...
export const SITE_READER_CONTRACTS = [
  ...
  c('capability-bounded-effective-state-gdr', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', '/research/seed-openevo/study/capability-exploration/bounded-effective-state-gdr/', 'narrative', '理解三组已完成 Qwen3-1.7B OpenEVO 实验分别改了什么，并分清 Gated Delta 的序列公式来源、实际只启用 β-gating（α 固定为 1）的实验边界、四条 OpenEVO 学习状态在 160 轮里的实际更新频率、正式结果，以及从优化到行为逐层加深的诊断指标', '先看到三组已完成实验和一个未做空位：方法列拆成 Bounded / 动态 β / 动态 α；已完成第三组明确 β ✓、α —，动态 α+β 行的结果保持为空；正式结果后，分析严格按 loss → Task Vector → 参数范数/谱/方向 → 输出长度/任务步数 → action-family entropy 递进', '固定 128 题是在训练前从 held-out goal_idx 0–499 中排除上一套 final 后按固定 SHA256 规则无视 reward/success 选出的同一套题，它只是一个窄切片；普通 OpenEVO 是更早独立实验，不是后两组预注册匹配对照的第三组；13–15× 只属于参数更新阶段；UPDATE/NOOP 只表示终态是否改变，不等于运行时是否被使用或对能力贡献大小；sequence State → parameter State 不是 tensor 1:1；每个诊断 subsection 必须先解释指标是什么，再给本实验结果，最后给分析边界；后分析不能升级成因果结论', '先读三组方法，在正式结果前核对参数 / Text Memory / Skill / Agent System 的 160 轮 UPDATE/NOOP 账本，再按指标复杂度往下走：loss 先检查优化是否健康 → Task Vector 看单轮参数移动 → Frobenius / spectral norm / rank / cosine 看长期参数几何 → tokens / episode steps 看任务路径 → action-family entropy 看行为分布；每节固定“指标是什么 → 结果 → 分析”，对应 W&B 图跟着指标放，不再另设一个图集式 W&B 大节', '.paper-table-wrap--hero', { maxInteractive: 3, maxHeadings: 1, maxTextChars: 2100 }),
