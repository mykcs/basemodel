...
Browser-heavy acceptance runs **before merge on the repository-scoped self-hosted runner**. CPU-CI depth is now fail-closed: only proven Markdown-only PRs use the dependency-free documentation contract; code/CI/config/test/data/mixed or unproven diffs run the full deterministic/build path, and every `main` push is revalidated after merge. `scripts/ci-ui-gate.mjs` still reuses the existing `vercel-ui-plan.ts` browser scope policy. Playwright remains at one worker after a controlled 2026-09-02 two-worker A/B on PR #408 timed out a heavy layout case under concurrency despite no OOM. The 12-case Lab gate is added only for Lab/server-relevant diffs. Vercel Production remains `verify:deploy + astro build` only, while Cloudflare production-smoke independently checks the real Production origin every 30 minutes for HTTP, canonical, robots, sitemap and redirect health.
