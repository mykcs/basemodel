...
Browser-heavy **preflight** now runs first on public GitHub-hosted runners, while exact-head **final acceptance** still runs inside Vercel Pro before merge. Public GHA, the Vercel gate, and retained manual CircleCI fallback share `vercel-ui-plan.ts` skip/focused/full policy; first/unknown comparisons fail closed to complete Chromium coverage, and the 6-case active-Lab gate remains relevant where owned. The 2026-09-15 latency qualification refreshed the current suite to 197 Chromium identities and compared 4/6/8 independent shards on unchanged acceptance semantics. Runs `34986898313`, `34987545968`, and `34988052276` covered all 197 identities exactly once with no retries; the slowest browser steps were 138 s, 102 s, and 80 s respectively. The preregistered smallest-within-10%-of-fastest rule therefore selects 8 shards. Against the earlier ~191 s public-GHA tail this is about 58% faster, while Vercel remains the final authority. Vercel Production runs the deterministic + risk-based browser contract. Cloudflare production-smoke independently checks the real Production origin every 30 minutes for HTTP, canonical, robots, sitemap and redirect health.
