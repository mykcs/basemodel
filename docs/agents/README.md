...
- [`history/2026-09-14-sitewide-normalization-conversation-closeout.md`](history/2026-09-14-sitewide-normalization-conversation-closeout.md) — BaseModel sitewide-normalization closeout: evidence-driven rule transfer instead of mechanical templating, shared bilingual body-copy parity beyond localized metadata, one survivor normalization workline while preserving specialized semantic owners, automation/progress recovery from durable artifacts, and exact-head provider receipts that do not mutate the accepted release head.
