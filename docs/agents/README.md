...
For shared experiment-server artifact governance/reclaim — especially exact ownership, remote-reachable Git worktrees, post-approval liveness invalidation, `pgrep` self-matches, logical-vs-physical reclaim, cross-user attribution boundaries, BaseModel phase re-entry, required-CI authority, and stale route metadata — read [`history/2026-09-05-shared-server-artifact-governance-reclaim-retrospective.md`](history/2026-09-05-shared-server-artifact-governance-reclaim-retrospective.md) after the current server/deployment owners.
