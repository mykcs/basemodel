...
Status: **COMPLETE / HISTORICAL TASK CLOSEOUT**
...
- [x] Find sequences whose topology is invisible and expose the path.
- [x] Find detached provenance/controls and restore semantic adjacency.
- [x] Verify terminology consistency across nav, headings, diagrams, captions and evidence.
...
Phase-F visible-topology sweep witness — **2026-09-18**: audited current public sequence/branch owners and their responsive CSS for the failure mode where the desktop path existed but the narrow layout reduced it to nearby boxes or silently removed a connector. Existing valid owners were deliberately preserved, including the WebShop/ALFWorld benchmark diagrams, Vanilla SD-LoRA mobile branch/return diagram, Stage-1 evolution map, first-run lineage, Gated-Delta derivation and other components whose narrow layout already carries real connectors. Four high-confidence gaps were repaired without changing scientific semantics: Harness 2.0 now keeps the shared start visibly splitting into control and candidate arms on phone/tablet; the old Stage-2 genealogy is a semantic ordered list with a connected vertical spine below desktop instead of disconnected boxes; the Stage-1 responsibility path keeps every transition at tablet width; and the experiment selector no longer drops the Stage-1 → teacher transition at the tablet breakpoint. The rendered topology is now protected by dedicated geometry/connector assertions, not only by text presence. During validation, two old browser cases still treated archived `/en/**` sources as deployed pages; current repository authority says English is inert archive material, so those stale route assumptions were narrowed to the active Chinese surface while the explicit English-archive regression remained in place. No experiment result, treatment, lifecycle, evidence boundary or archived English source changed. Acceptance on the final product tree: focused structural/locale regressions **39/39 PASS**; focused Chromium topology/route acceptance **17/17 PASS** and focused WebKit **17/17 PASS**; Astro check **0 errors / 0 warnings** (two existing deprecation hints); CSS architecture PASS; **263-page** production build plus heading/brand audits PASS; full Chromium UI gate **198/198 PASS**; full Chromium + WebKit matrix **394/394 PASS**. This completes only the fourth Phase-F row. Phase F has no per-row weights, so the credited total remains **80 / 100** until all six Phase-F rows are complete.

Phase-F semantic-adjacency sweep witness — **2026-09-18**: refreshed current BaseModel `main`, PR #737 exact head/base/mergeability/check state, open scientific/content owners and the current expression/Reader Contract/UI/release authorities before editing. Claim-level evidence already sits beside its claims on the active result surfaces, while page-wide source notebooks and provenance footers legitimately describe the whole page; those were preserved instead of being mechanically pulled into every paragraph. The high-confidence detached-control defect was the true step-by-step Flow explainer transport: a duplicate CSS override made Previous / Next / Reset float at the bottom of the viewport immediately on page load, before the reader had entered the explainer. That conflicted with the current interaction-ownership contract and made the controls look detached from the diagram they operate. The transport now stays attached to its explainer by default and becomes a bottom dock only after explicit step/play interaction, focus, or active pointer use; Reset returns it to the local overview state. Passive scroll-linked synchronization can still update the highlighted step, but it cannot create the floating dock. The duplicate unconditional fixed-position rule was removed, and browser regressions now protect the semantic behavior rather than the old CSS token. No scientific result, treatment identity, lifecycle state, evidence link, claim boundary or caveat changed. Acceptance on the final product tree: focused structural/gate regressions **55/55 PASS**; focused Chromium explainer/transport acceptance **21/21 PASS** and focused WebKit **21/21 PASS**; Astro check **0 errors / 0 warnings** (two existing deprecation hints); CSS architecture PASS; **263-page** production build plus heading/brand audits PASS; deterministic deploy gate PASS; full Chromium + WebKit UI matrix **394/394 PASS**. This completes only the fifth Phase-F row. Phase F has no per-row weights, so the credited total remains **80 / 100** until the terminology-consistency row is complete.


Phase-F terminology-consistency sweep witness — **2026-09-18**: refreshed the live BaseModel source/PR state and current copy, Reader Contract, HPL, UI and release authorities, then scanned active research owners for the same technical object appearing under different reader-facing names. The high-confidence inconsistency was `TaskVector` versus `Task Vector`: the current Gated-Delta mechanism already uses `Task Vector`, while the briefing, technical notes, mechanism map, capability lobby and their Reader Contracts still mixed the joined spelling. Those public owners now use **Task Vector** consistently in headings, explanatory prose, equations/ARIA labels, metadata and evidence links. The sweep deliberately did **not** flatten semantically different names: general framework/reproduction contexts may still say `OpenEvo` while the experiment/project identity remains `OpenEVO`; historical `GDR-v1`, the original `Gated Delta Rule`, and current `Gated-Delta SD-LoRA` remain distinct objects; `SD-LoRA`, `DirectApply` and `WebShop` already had one stable spelling on active public surfaces. A new regression rejects a return of the joined `TaskVector` spelling in the affected public owners while also protecting the GDR identity split. The deterministic gate first exposed two stale briefing assertions that still required the retired joined spelling; only those expectations were updated, with no product/scientific assertion weakened. Final acceptance: focused terminology/Reader/briefing regressions **57/57 PASS**; joined `TaskVector` occurrences across active public research owners **0**; noncanonical `SD Lora` / `Direct Apply` / `Web Shop` variants **0**; deterministic deploy gate PASS; strict copy invariants **0 failures**; Reader Contract audit **66/66 public page patterns PASS**; human-feedback audit PASS; Astro check **0 errors / 0 warnings** (two existing deprecation hints); **263-page** production build plus heading/brand audits PASS; full Chromium + WebKit UI matrix **394/394 PASS**. No result value, treatment identity, lifecycle state, evidence link, scientific caveat or release authority changed. Phase F is therefore complete at **10 / 10 weighted points**, bringing the credited total to **90 / 100**.
...
- [x] `npm run check` / deterministic repository gates pass.
- [x] reader-contract / human-feedback / route-owner audits pass.
- [x] risk-appropriate Playwright/GHA browser evidence passes.
- [x] final current-base exact-head Vercel acceptance passes under existing release authority.
- [x] merge only after current-base refresh and overlap reconciliation.
- [x] Production readback verifies representative migrated routes on desktop + phone.
- [x] update canonical rules only for genuinely reusable lessons; archive task checklist when complete.
- [x] execute conversation/project closeout protocol and record repeated mistakes, if any.

Phase-H release witness — **2026-09-18**: the final #737 source head passed public PR CI and the required exact-head Vercel final gate before merge. Immediately before release closeout, live `main`, the merged PR state, open scientific/content owners and current release authority were refreshed; concurrent scientific owners remained separate and no claim/caveat was absorbed. The squash merge produced `main@371271c396dc1a8ecf82cfb9d15e124bd093b8dc`; its Git tree `44937ec67b4aba01e0e389347fd75fff2422fcb3` is identical to the accepted PR-head tree. The corresponding Vercel Production deployment reached **READY** after the full **198/198 Chromium** acceptance and **6/6 Lab** acceptance actually executed and passed. A fresh stable-Production readback then checked eight representative migrated routes in light and dark themes at **390×844** and **1280×633**: **32/32 PASS** for HTTP 200, expected Reader Contract identity, visible H1, zero page-level horizontal overflow, stable Production canonical identity and absence of `noindex`. No release, accessibility, Reader Contract or scientific-authority gate was weakened.

Closeout deliberately adds **no new current design policy**. The reusable mechanisms exposed by this work already have canonical owners: reference-product principles must be translated into reader cognition rather than surface imitation; first-screen budgets must not be passed by empty viewport-height spacers; comparison/sequence/evidence/control layout follows semantic shape; protected Preview login screens are not application evidence; and exact-head release evidence stays distinct from Production readback. The task therefore archives its execution record instead of creating another mutable visual standard. Phase H is complete at **10 / 10 weighted points**, bringing the final credited total to **100 / 100**.
...
## 13. Historical scheduled-continuation contract

This section records the automation contract used while the task was active. The checklist is now complete and the recurring continuation must remain disabled. While active, each scheduled run should:
...
## 16. Closeout lessons and coverage ledger

| Feedback / failure | Repeated? | Reusable lesson | Canonical destination / disposition |
| --- | --- | --- | --- |
| Apple screenshots and first-party guidance were requested as design references | Yes | Translate purpose, hierarchy, agency, simplicity and adaptivity into this reader task; resemblance is not a design reason | Already owned by current UI/HPL/design policy; no duplicate rule added |
| Full-screen whitespace initially kept dense catalog controls out of the first screen | Yes | Real hierarchy must come from useful content and semantic grouping, not viewport-height spacer tricks | Already owned by the Reader Attention Contract; this task added use-site regressions and kept the budget unchanged |
| Several migrations exposed stale browser/copy assertions after accepted UI semantics changed | Yes | Classify product regression vs stale contract; update only the retired assumption while keeping geometry/accessibility/scientific checks intact | Already owned by UI acceptance/release policy; repaired in-place during this task |
| Protected Preview requests could land on Vercel access UI rather than the product | Known class | Provider READY/HTTP success is not route readback; require a real product sentinel through an authorized session | Already owned by UI/release policy; authenticated product readback was used here |
| Parallel scientific PRs touched nearby Reader Contract/publication surfaces | Yes | Visual migration may change presentation, not silently absorb a scientific lifecycle/result owner | Existing multi-PR + scientific-publication policy remained authority; #728/#734/#740/#743 semantics stayed separate |
| PR narrative lagged live CI/final-gate state at release time | Yes | Refresh collaboration metadata without mutating an already accepted candidate tree | Existing exact-head/release closeout rule; PR body was refreshed metadata-only |
| Post-release owner handoff used the correct Production route but added an unsolicited tracking query | No | Owner-facing release links should preserve clean canonical identity: verify the live Production alias, then share the canonical domain + route without deployment hashes, share tokens, or tracking parameters unless the requested product behavior needs them | Added to the existing `deployment-policy.md` Production-release owner; this history records the incident without creating a second policy |
| Independent-review automation repeatedly opened temporary browser profiles and reached AI/account authentication surfaces on the owner's machine | No for this exact boundary; broader reviewer/evidence separation already existed | Browser automation may verify/capture the product, but it cannot manufacture reviewer identity. AI/account/login/sync surfaces mean wrong execution surface / `NOT_EXECUTED`; close the session and switch to an eligible reviewer route instead of creating more profiles | #737 added the verifier-only rule after the incident; current HPL/UI gate + scenario trigger now also encode the account/profile stop and dispatch witness |

Temporary ports, worktree paths, browser-session state, provider queue timing, deployment IDs and ephemeral access links were intentionally not promoted into standing policy. A temporary local browser-blocking shim used to stop the retry loop was incident containment only and is intentionally not a standing machine configuration. The post-release URL follow-up likewise keeps the stable canonical Production domain as the durable identity while leaving deployment-specific hostnames, share tokens and one-off query strings out of standing history. A temporary Production-readback harness initially expected the wrong Results Reader Contract id; the product rendered the current contract correctly, so the harness expectation was corrected and the same acceptance was rerun to 32/32. That incident reinforces the existing product-vs-stale-test classification rule and does not create a new policy.

### Future-Agent test

A new Agent starting from root `AGENTS.md` reaches the current design/copy/Reader Contract/UI acceptance/release owners before this historical record. It should therefore begin by asking what thought structure the page must expose, preserve claim-changing caveats, avoid empty-space/card-template shortcuts, run repository-owned browser acceptance, and keep exact-head Preview evidence separate from Production verification. When independent review is explicitly activated, it should also bind the requested evidence class to an actually eligible reviewer and keep reviewer identity separate from the owner's browser/account state; an auth/login/sync page is a stop signal, not an invitation to try another profile. The active task path is now a navigation-only completion stub, so a future Agent cannot mistake this finished checklist for current mutable authority.
