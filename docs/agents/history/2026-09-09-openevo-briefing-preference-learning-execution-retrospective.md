...
## 2.1 Coverage ledger — significant owner corrections and escaped failures

This ledger is intentionally about **durable lessons**, not the final PR/Preview/run state. Truly duplicate feedback is merged by failure family; temporary SHAs, URLs, ports, provider queue state, and live experiment progress remain excluded.

| Feedback / failure | Repeated? | Reusable lesson | Canonical destination | Why there |
| --- | --- | --- | --- | --- |
| Meaningless English eyebrows / internal labels returned after earlier webpage corrections | **Yes** | remove attention/decode tax, not English or technical vocabulary in general | `human-preference-learning-system.md`, `humanPreferenceModel.ts`, CASE-027/063/082 | HPL must reject the family before owner review |
| Dense card wall was rejected; later over-minimal HTML also lost useful structure | **Yes** | optimize competing visual centers, not “less information at all costs” | Preference Trajectory + Silver/Rejected visual set | pairwise direction prevents over-learning one correction |
| Decorative pastel bubbles reappeared after “soft slides” received positive feedback | **Yes** | positive style signal does not authorize non-informational decoration | HPL visual events + CASE-082 | keeps “soft” separate from “decorative noise” |
| “HTML, not PPT” was misread as “redesign this as a normal webpage” | **Yes** | delivery medium and presentation composition are separate decisions | CASE-082 + briefing device-scope preference + scenario trigger | the use site must classify artifact role before styling |
| SEED and OpenEVO values were visually aligned before the non-comparability caveat | No, but high impact | a table visually asserts comparability; protocol boundary must be visible before/beside it | scientific-boundary preference + research presentation owner | protects scientific interpretation, not just wording |
| 3B experiment line was omitted from a supposedly comprehensive result story | **Yes, owner had to remind** | website visibility is not experiment inventory authority; inventory upstream families before writing the table | scenario-trigger briefing specialization + this history case | makes the check happen before the first draft |
| Engineering rigor / SHA / fixed-GPU / “trustworthy and fast” repeatedly became a main slide | **Yes** | engineering validity is prerequisite; mainline only when it changes the scientific conclusion | research-presentation progressive disclosure + HPL research-judgment rule + scenario trigger | stops “proof of diligence” from replacing research contribution |
| “这里可以更硬核一点” was exposed to the audience | No | technical depth should be carried by formulas/data, not self-evaluative meta copy | `PREF-TECHNICAL-DEPTH-WITHOUT-META` | project preference is precise and scoped |
| `44 → 7` was initially too compressed | **Yes** | name candidate generation, GDR decision, and persistent application as distinct objects | inline-terminology + concrete-mechanism HPL rules | prevents a lifecycle count from being misread as training failure |
| Status/module taxonomy obscured the actual research reasoning | **Yes** | advisor story follows question → hypothesis → test → result → next question | `PREF-RESEARCH-JUDGMENT`, CASE-082/084/090 | makes scientific judgment the primary organizing principle |
| Negative 15→30 and capacity tests risked being treated as failed engineering | No | a clean null is a research result when it rules out an explanation | current research-copy preference + this project history | preserves diagnostic value without overclaiming benchmark gain |
| Generic reviewer suggested deleting owner-required Agenda/TLDR/final choice | **Yes within review loop** | explicit current owner instruction outranks generic reviewer taste | HPL owner-override rule | reviewer is evidence, not task authority |
| Screenshot / preference receipt stayed “green” after later content or CSS changed | **Yes** | visual evidence is exact-artifact evidence and invalidates on material geometry/science changes | HPL candidate receipts + current release/visual acceptance owners | prevents stale evidence from certifying a new tree |
| Main/PR moved while the conversation assumed a stable candidate; unrelated nav work entered the briefing PR | **Yes** | refresh remote head/scope before rewrite; preserve unknown work before separation | root `AGENTS.md` + project operating principles | shared branch is not a private lock |
| Local push/tool timeout was interpreted too quickly as failure | **Yes across project work** | timeout after dispatch has unknown side effects until durable state is read back | root/bootstrap + project operating principles | prevents duplicate writes and false blockers |
| Capability discovery / action selection drifted into unintended writes during closeout | **Yes — repeated four times inside closeout** | discover with schema/list/search/fetch/read; bind target→action before writes; if mismatch recurs after the witness, abandon that mutation surface and switch to an explicit CLI/API action | retrospective scenario trigger + project operating principles | a prose reminder alone did not stop dispatch mismatch; the workflow now has a surface-switch escalation |
| Focused tests passed before full Gate caught dead code / CSS architecture debt | **Yes** | focused PASS is diagnostic; completion uses the exact current repository/provider contract | release closeout + deployment policy | protects cross-cutting invariants without weakening tests |
| Repeated owner “都完成了吗？继续解决” prompts followed near-complete reports | **Yes** | report implementation, local tests, PR, hosted acceptance, merge, Production, and owner review as separate states; do not call “one thing left” before the real acceptance boundary is known | project operating principles → bounded progress reports | completion language must match evidence stage |
| Full Vercel final acceptance was used too eagerly during iterative human review | **Yes** | owner-review iterations use lightweight review Preview; exact-head Vercel final gate only when merge-ready | root `AGENTS.md`, deployment policy, `PREF-FAST-REVIEW-PREVIEW` | reduces waiting/cost without weakening final authority |
| The original case library recorded corrections but future first drafts still repeated them | **Yes — core system failure** | feedback must bind both generation-time context and evaluation-time rejection | Human Preference Learning V2 | fixes the knowledge-system failure rather than adding another reminder |
...
**What happened.** During this retrospective closeout itself, the Agent first called a GitHub `create_file` action on a deliberately nonexistent branch while trying to confirm the available mutation surface. GitHub returned 404 and created nothing. Later, the Agent used `create_branch` merely to “confirm” a branch action, creating a temporary shadow-check branch; it was immediately read back and deleted with deletion verified. A third escape happened while intending to open the closeout PR: the Agent selected `create_branch` again and created a temporary `-copy` branch instead of calling `create_pull_request`; that exact branch was also immediately deleted and deletion verified. Even after writing an explicit `open PR → create_pull_request` witness, a fourth dispatch selected `create_branch` and created a temporary `-pr-check` branch; it too was read back, deleted, and deletion verified. At that point the correct response was no longer another reminder or another connector retry: the write surface was classified as action-selection-ambiguous for this task, and the PR mutation was moved to an explicit `gh pr create` command path. None of the three accidental branches remained.
...
**Defensive rule.** Never use a write endpoint merely to test whether a connector/tool works. Before a legitimate write, bind the intended object and action explicitly (`open PR → create_pull_request`, `move ref → update_ref`) and stop if the selected action targets a different object. If the mismatch recurs after that witness, stop using the ambiguous mutation surface and switch to an explicit programmatic path instead of retrying. A failed or cleaned-up write with no lasting residue is still a workflow defect and should be disclosed. Root `AGENTS.md` / project operating principles remain the authority; this section is historical evidence of recurrence and the scenario trigger now carries both the use-site witness and the surface-switch escalation.
