...
Status: **pre-registered implementation / benchmark**
...
- trigger: prior release work already established that merge calls must not be used as probes for missing checks -> owner: release-closeout protocol -> checked: exact live required context is currently only `Vercel`; `public-ci-gate` is GitHub Actions App id 15368 and passed on #763 -> allowed: qualify code/config first, then change branch protection only after the transition commit is merged and Production is healthy -> invalidate on any branch-protection/ruleset drift.
...
- [ ] implement script/config/test/doc changes locally;
- [ ] local deterministic tests and build pass;
- [ ] push one coherent PR and let existing Public PR CI qualify the unchanged acceptance population;
- [ ] request exact-head Vercel final gate only after `public-ci-gate` is green;
- [ ] record exact-head Vercel duration and compare with 507.7 s control;
- [ ] if exact-head correctness is green and the primary Vercel latency rule passes, update live `main` branch protection **before merge** to require both `public-ci-gate` (GitHub Actions app 15368) and `Vercel` (app 8329); keep strict current-base semantics and preserve all unrelated protection;
- [ ] read branch protection back and prove strict mode + both exact contexts; this prevents any no-full-acceptance window during the cutover;
- [ ] merge only under the new dual-required authority;
- [ ] wait for merged Production READY and record duration vs 633.3 s control;
- [ ] update current CI/deployment docs with the adopted architecture and measured result.
