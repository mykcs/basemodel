...
The deterministic job remains pinned to the qualified Debian 12 / Node 24 base container. Browser validation uses the public digest-pinned `ghcr.io/mykcs/basemodel-ci-browser@sha256:8db8a291f50b9ac3dc00e1e02477924e816b09ec5904bf731efb64f268d2712a` runtime, built from `.circleci/browser-runtime.Dockerfile` on GitHub Actions run `34020048408` at source head `d2f611ed55d0e7106fbf0e4851817c2222ef1765`. That image keeps the same Node 24 / Debian Bookworm base and pre-installs Git/SSH plus the Playwright 1.62.1 Chromium system dependencies and browser payload; it contains no repository source or CI secret. Full browser acceptance still uses two independent CircleCI `medium` shards with one Playwright worker each and retries=0. `scripts/ci-ui-test-list.mjs` enumerates the current canonical Chromium suite and assigns individual test cases by the exact `202609061200` one-worker timing receipt; unknown or renamed tests remain included with a conservative weight. Focused browser work is owned by shard 1 and the other shards exit before browser work. Documentation-only PRs exit before npm/browser work after the documentation contract passes.
