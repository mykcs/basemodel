...
A cold-read/preference study and a visual-regression acceptance gate answer different questions. The former can study first-time comprehension or preference; the latter proves that the product renders and behaves correctly under the repository contract. Never silently turn the optional former into a dependency of the mandatory latter.
