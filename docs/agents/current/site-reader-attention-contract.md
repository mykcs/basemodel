...
`firstViewportBudget` 的目标是发现**同时竞争注意力的内容**，不是要求第一认知层占满整个物理 viewport。一个页面可以通过 `min-height: 100vh/100svh`、超大 top padding、空 spacer 或垂直居中把下一节推到折叠线以下，让计数变小；如果这些空白本身没有 reader-task 含义，这种 PASS 反而增加了读者到达真实内容的时间，不能算 attention-first。

因此：

- 不得为了过首屏预算而新增满屏 `min-height`、空占位或无语义大留白；如果删掉空白后只是让下一节在屏幕底部自然露出一小段，先判断它是否真的形成同权重视觉中心，而不是机械把它全部算成认知竞争；
- `data-reader-primary` 表示页面声明的第一认知 owner，不是逃逸预算的包裹器。任何基于它的 Gate 都必须先证明 owner 唯一、范围准确，不能把整个页面包进去来制造假低密度；
- `100vh/100svh`、大标题、留白、serif、圆角和 full-screen slide 都不是禁用项；只有当它们服务当前 reader task、分组、节奏或真实 presentation 介质时才成立；
- 如果“同样的内容，只加空白就从 FAIL 变 PASS”，把它当成 **Gate proxy / 产品层级共同需要检查的信号**。先做 zero-context cold read，再决定是页面真的过载，还是 Gate 的几何代理测错了对象。
