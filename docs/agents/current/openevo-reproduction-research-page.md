...
Use the manual mindset only, not GDKVM-specific project content. Executable subsections normally read: **explanation → command/action → expected result/acceptance → local Troubleshooting**.

The original dual-line requirement is integrated: current success path owns the numbered flow; failure knowledge appears beside the relevant step and in the final diagnostic index. Do not turn the page back into a dashboard, research-card gallery, architecture pitch, or chronological incident diary.
...
- Data path: pinned AgentGym copy → GitHub API → Mac cache → validation → rsync RTX6.
