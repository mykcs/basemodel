...
This is an execution-first academic reproduction manual. The owner supplied the GDKVM reproduction page as the structural template. Preserve this order:
...
Use its manual mindset only, not GDKVM-specific project content. Executable subsections should normally read: **explanation → command/action → expected result/acceptance → local Troubleshooting**.

The original dual-line requirement is integrated:
- current success path owns the numbered main flow;
- failure knowledge appears beside the relevant step and in the final diagnostic index.

Do not turn this back into a dashboard, research-card gallery, architecture pitch, or chronological incident diary.

## Scientific boundary

Historical 2026-07-29 WEIYING/Kaggle/Colab evidence is a real core-path reference: real WebShop, Qwen2.5-3B-Instruct, Transformers, CUDA, real model actions, real reflector, evolution injection, gen0→gen1. `0.0` is valid execution evidence; `0.0→0.0` is not improvement. A historical OpenEvo `COMPLETED` stub path is orchestration evidence only.
...
- Curated stack includes `gym==0.24.0`, `env==0.1.0`, `train==0.0.5`, `pyserini==0.17.0`, `faiss-cpu`, spaCy model support; validate sparse/Lucene with `LuceneSearcher`.
...
Use a narrow numbered academic-document flow, monospaced commands, local Troubleshooting, compact final failure index, mobile-safe code/tables, shared bilingual component, and the `/guide/reproduction-c/` hierarchy. Never treat setup/stubs/mocks/fallback-only/clean exit/`COMPLETED` as P0.
