...
Required Public PR CI is the ordinary deterministic/browser acceptance path; Vercel Pro is the separately required exact-head provider build/deploy path and Production host. Chromium/Lab acceptance runs once in Public PR CI rather than being repeated inside Vercel. CircleCI automatic PR/main workflows are disabled; only explicit API-triggered manual fallback remains. The repository-scoped Mac/OrbStack workflow is also manual recovery only. Cloudflare Pages/build helpers and Workers shadow tooling remain legacy rollback/provider-specific surfaces, while `cloudflare/production-smoke/` is the active monitoring-only exception and does not build or publish the site.
