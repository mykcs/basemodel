...
For ordinary local iteration on a **small, machine-proven route surface**, use `npm run verify:fast` instead of paying the full repository typecheck after every edit. The executable planner may admit either existing non-dynamic `src/pages/**/*.astro` leaf pages (currently at most four) or a very small set of Astro components (currently at most two) only when repository-wide source scanning proves that every runtime importer is one of at most four concrete non-dynamic Astro pages. Docs and source-test companions do not widen runtime scope. The fast path checks the changed runtime sources plus every proven page consumer with scoped Astro diagnostics and ESLint, then preserves the complete Vitest suite and the CSS/copy/Reader/human-expression audits; independent read-only checks may run in parallel to shorten feedback without changing their assertions or population.

The boundary is fail-closed: indirect or non-page component consumers, global components, data/content, dynamic routes, deletes/renames, unknown or unscanned references, package/config changes, and any fast-gate/planner owner must fall back to `npm run verify:deploy`. `npm run check:watch` remains useful for shared-code iteration when the component graph cannot be conservatively bounded, but it is not release evidence. `verify:fast` and `check:watch` are development feedback only; required Public PR CI keeps the full deterministic/browser contract, while final-candidate Vercel and Production own the static provider build/deploy contract on the accepted tree. Measurement evidence lives in [`../history/2026-09-13-fast-local-gate-experiment.md`](../history/2026-09-13-fast-local-gate-experiment.md) and the bounded-component extension in [`../history/2026-09-13-bounded-component-fast-gate-experiment.md`](../history/2026-09-13-bounded-component-fast-gate-experiment.md).
...
- Required Public PR CI's hosted browser gate is Chromium-only; Vercel no longer reruns browser acceptance after that exact-head check is green.
...
Before claiming that browser/review evidence belongs to a commit SHA, check the candidate worktree state. A dirty working tree means the rendered artifact is `HEAD + local delta`, not the named commit. Treat that run as uncommitted preflight evidence unless you bind it to a distinct content/tree fingerprint. If the dirty files can affect the tested surface, commit the intended delta first and rerun the affected render/review/browser acceptance; do not carry the green result forward and relabel it as exact-head evidence. A later commit does not retroactively change what tree the earlier browser actually rendered.
