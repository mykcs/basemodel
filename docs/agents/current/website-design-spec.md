...
标题也不要为了制造叙事感，给普通实验差异强行套上 `分岔 / 路线 / 地图 / 楼层 / 关卡` 等隐喻。若事实只是分别测试 3B 和 7B，就直接命名 `3B 和 7B 的第一轮购物实验`；如果因果设计确实从同一 checkpoint 建立两组，优先写 `从同一 checkpoint 分成两组`。只有这个关系本身影响科学解释时，才值得让关系词占用标题或首屏注意力。见 [CASE-067](website-copy-cases.md#case-067-科研标题不要为了叙事感发明分岔)。
...
**Eyebrow / kicker 不是必填字段。** 中文页面尤其不要用 `HISTORICAL MAP · FIRST RUN` 这类纯英文 eyebrow / kicker 表达“这是历史页 / 这是第一轮”等已经由标题、导航或正文说清的内部分类。没有增加科研信息时直接删除；确实承担复现类别、日期、阶段、状态或 provenance 时，先用自然中文表达，例如 `复现 · 原始基线`。见 [CASE-067](website-copy-cases.md#case-067-科研标题不要为了叙事感发明分岔)。
