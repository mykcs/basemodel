...
标题也不要为了制造叙事感，给普通实验差异强行套上 `分岔 / 路线 / 地图` 等空间隐喻。若事实只是分别测试 3B 和 7B，就直接命名 `3B 和 7B 的第一轮购物实验`；只有真实分支拓扑本身是研究对象时，才需要这类词。见 [CASE-067](website-copy-cases.md#case-067-科研标题不要为了叙事感发明分岔)。
...
中文页面尤其不要用 `HISTORICAL MAP · FIRST RUN` 这类纯英文 eyebrow / kicker 表达“这是历史页 / 这是第一轮”等已经由标题、导航或正文说清的内部分类。它没有增加科研信息时应直接删除；确有新增信息时先用自然中文表达。见 [CASE-067](website-copy-cases.md#case-067-科研标题不要为了叙事感发明分岔)。
