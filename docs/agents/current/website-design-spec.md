...
**网站的阅读对象首先是人类。** 公开页面的结构、文案、加粗、字号、卡片和视觉中心都先服务于减少人的**注意力切换**与**上下文恢复成本**。不能为了“解释完整”要求读者在主线、术语表、代号表和解释层之间来回跳；每一次视觉强调也都在消耗读者的注意力预算，必须对应真正重要的对象、结论或决定性数字。
...
同理，`先分清两种“新任务”` 的问题不只是 `先` 字，而是把作者的教学动作做成全区视觉中心。标题、加粗和大字号会强迫读者优先处理这句话，所以它们必须指向真实主题本身，例如 `训练范围内未见任务与 SEED 验证任务`。**视觉中心就是注意力分配，不是装饰。** 见 [CASE-068](website-copy-cases.md#case-068-视觉中心必须给真实主题)。
...
### 6.1 专业名词和代号必须在第一次出现的位置就让人懂

术语处理默认按这个顺序：

1. **先判断能不能不用代号。** 如果 `Track A / H1.38B / WB1 / goal_idx` 对理解当前句子没有必要，就把它留在证据或 provenance 层。
2. **需要保留时，先给人类名称或全称。** 例如先写 `SEED 留作验证的任务（0–499）`，再在必要处保留内部字段；不要让内部字段成为主语。
3. **解释它在这里具体做什么。** 不能把一个陌生词换成另一个陌生词就算解释完成。
4. **解释放在术语第一次出现的局部上下文。** 读者不应该离开当前段落去查一个集中术语表，再回来恢复刚才的阅读位置。

**集中术语表只能是参考工具，不是主阅读路径。** 如果一个页面只有先查表才能读懂正文，说明正文还没有完成 first-use explanation。定义列表或附录可以保留给复查和精确检索，但不能承担“读者自己去找解释”的责任。见 [CASE-069](website-copy-cases.md#case-069-术语和代号必须就地解释)。
...
因此：**分层是信息深度，不是视觉标签模板。** 任何 `专业解释` 小标题、虚线分隔、额外卡片或加粗前缀都必须证明它让读者更容易理解，而不是只让页面“看起来像有层级”。见 [CASE-070](website-copy-cases.md#case-070-解释深度不是可见-ui-模板)。
...
3. 用户给出真人网页反馈、指出重复理解问题，或再次纠正同一类阅读困难时，不只找一个相似句子：读取一个**案例簇**，至少包含当前最接近案例 + 2 个同类/相邻案例；新反馈优先于较老案例。
4. 先把案例簇总结成一句可复用规则，并同时写清“这条规则不意味着什么”。例如 CASE-061–070 的共同规则不是“删除英文、禁止关系词或不要技术深度”，而是“零上下文首层先展示真实对象、双方、动作与结果；视觉中心服从真实主题；术语就地解释；内部代号、方法学术语和非必要叙事包装后置；信息深度不机械变成 UI 模板”。
...
8. 用“语义位置 + 案例里的词语线索”扫描其他 production copy owner、共享组件和 sibling routes。先判断是不是同一个失败机制，再分类为高置信同类 / 不确定 / 有意例外；同一任务里直接修高置信同类，不对不确定项做机械替换。
...
| 视觉中心只强调真实主题、结论和决定性数字，不强调“先怎么读” | [CASE-068](website-copy-cases.md#case-068-视觉中心必须给真实主题) |
| 专业名词和内部代号在第一次出现处就地解释，不能要求读者来回查表 | [CASE-069](website-copy-cases.md#case-069-术语和代号必须就地解释) |
| 信息深度不等于固定的“一句话 / 专业解释 / 证据”可见 UI 模板 | [CASE-070](website-copy-cases.md#case-070-解释深度不是可见-ui-模板) |
