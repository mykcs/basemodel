...
默认读者可以懂 ML / 系统 /科研概念，但不知道本项目内部历史。第一次出现项目术语时，先说它在这里具体做什么，再给精确术语。
...
### 7.1 First-time-reader completeness：任何醒目数字、状态或内部术语都必须自解释

对公开研究页面，默认读者**懂机器学习，但没跟过这个项目的聊天、实验编号和运行日志**。以下内容只要进入标题、卡片、表格、图注、状态条或正文主线，就不能裸露：

- 数字：`16 / 1,440`、`80 / 80`、`7 / 8`、`59 / 80 + 171`、`0`；
- 状态：`BLOCKED`、`NOOP`、`Pending`、`sealed`、`superseded`、`measurement invalid`、`fail-closed`；
- 项目词：`gate`、`block`、`identity`、`carrier`、`successor revision`、`receipt`、`evidence boundary`、`checkpoint`。

公开表达必须能让第一次来的读者仅靠当前页面回答四个问题：
...
> Stage 2 共做了 20,480 次完整任务尝试，其中 797 条是可用于训练的完整成功轨迹。旧程序每 256 次尝试单独检查一次：只有同一块里至少 8 个不同任务都能重复完整成功，才允许更新参数。实际最好的一块只有 7 个，因此这次历史运行没有触发 Stage-2 参数更新。这里的 7 和 8 都是“不同任务数”，不是模型分数；0 次更新也不等于实验没跑或没有成功数据。

反面写法：

> 20,480 rollouts / 797 qualified positives / max identity 7 / gate 8 / 0 updates.

同一规则也适用于“没有发生”的结果。`final 没运行`、`测量无效`、`0 次更新`、`0 / 128 完整成功`必须分别写清，不能都压成一个 `0 / failed / pending`。

### 7.2 否定词和失败状态必须附带“剩下什么仍然有效”

`设计错误 / 未通过 / 无效 / 已取代 / 阻塞 / 没有更新`都可能是准确事实，但不能单独承担解释。公开页面应优先写成：

> **发生了什么 → 为什么这样判断 → 哪部分仍然有效 → 下一步是什么（若有）**

例如 `历史 Stage 2 已取代` 后应继续说明：历史 rollout、成功轨迹和 checkpoint 仍是有效证据；被取代的是“逐块硬门槛作为长期学习算法”的解释，而不是整个实验数据作废。

### 7.3 内部编号降级为 provenance，不让编号承担语义

`H1.40 / H1.41 / A1 / C1 / T2 / Phase G` 可以保留用于追溯，但标题和主句先写自然语言：

- 推荐：`第二代经验整合没有同时保住新旧能力（H1.40）`
- 反面：`H1.40 upstream gates failed`

编号是索引，不是解释。读者不应该先知道项目历史，才能理解一句话。
...
| 数字、状态、否定词、项目术语和内部编号都要让第一次来的读者自解释 | [CASE-051–058](website-copy-cases.md#case-051-数字先解释数的是什么) |
