...
首屏不是信息库存，也不是把所有“正确字段”同时展示出来。默认假设第一次来的读者几乎没有项目上下文，也不会主动分配很多注意力给页面。设计要先替读者完成取舍：**几秒内认出对象，并抓住一个最重要的事实。**
...
保真指的是**对象、因果顺序、语气和可复述性**，不是逐字复制整段聊天。临时进度、PR/Preview 状态、过期数字、对话轮次提示和重复寒暄仍应删除；科学数字与边界仍要重新对照当前 authority。能直接复用 owner 已认可的人话时，先复用，再做最小编辑；不要从 Agent 自己的“正式写作”默认风格重新起稿。
