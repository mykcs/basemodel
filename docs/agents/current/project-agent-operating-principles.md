...
3. bind the target object to the exact mutation action/schema before dispatch (for example, `open PR -> create_pull_request`); if the selected action targets a different object, stop rather than treating a nearby write capability as equivalent;
4. do not create probe files, comments, branches, deployments, or other mutations merely to test whether a tool works;
5. when several files form one coherent change, prepare them before the first provider-triggering update and prefer one atomic multi-file commit when practical;
...
9. before the first substantial implementation write on a multi-Agent task, search live open PRs/branches/worktrees by **semantic target and reader problem**, not only by overlapping filenames. If another workline already owns the same outcome, cold-read/adopt or reconcile it instead of creating a second implementation path.
...
- Bind manual browser acceptance to the **task-owned server that actually launched**, not to the port you asked for. Read the dev/preview server stdout and use its emitted URL; if the framework auto-increments because the requested port is busy, the requested URL is invalid evidence. Before screenshots or DOM assertions, confirm a task-specific route/sentinel on that server so an older worktree's process cannot be mistaken for the current candidate. Prefer a unique free port or fail-on-busy behavior when the runner supports it.
...
- Treat shell dialect as part of the execution environment. When syntax relies on Bash (`VAR=value`, `set -euo pipefail`, compound loops, arrays, heredocs, process substitution), request `/bin/bash` when supported or invoke a standalone Bash/Python script. **Verify the interpreter actually launched from the tool/process result before relying on that request**: a shell option can be ignored or normalized by the execution surface. If the execution API reveals the interpreter only *after* starting the same command, make that first probe dialect-neutral (`pwd`, `printf`, `git status`, etc.); do not put process substitution, arrays, heredocs, or other Bash-only syntax in the probe that is supposed to tell you which shell actually ran. If the reported outer shell is still Fish or another dialect, do not send compound Bash syntax through it; use a simple explicit interpreter handoff when quoting is trivial, otherwise write and syntax-check a standalone script. Prefer an explicit remote interpreter (`ssh host 'bash -s'`, `python3 -`, or an uploaded script) over multi-layer quoting. An inner `bash -lc` does not rescue malformed quoting already parsed by an outer Fish shell. A parser error before mutation is `NOT_EXECUTED`, not repository/server corruption.
