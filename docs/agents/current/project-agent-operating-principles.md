...
- **Return authority to the canonical cloud controls.** Local Git/test/build evidence is preflight only. GitHub remains canonical source/PR state and required exact-head/current-base `public-ci-gate` owns merge acceptance; Vercel separately owns Production and optional provider Preview evidence. Never substitute a green local build for the required GitHub Actions gate or for real Production verification.
...
- **Avoid provider churn.** When a local loop can cheaply catch a UI/multi-file defect before public GHA, use it. Do not request a Vercel Preview merely to duplicate a green required CI result; request one only for real provider/human-review evidence. Conversely, do not run an expensive local matrix that adds no useful evidence for a trivial docs/content edit.
