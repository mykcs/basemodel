...
7. **Missing exact-head hosted acceptance is actionable topology, not passive pending.** First require exact-head `public-ci-gate=success`; a missing or red Public PR CI result is a repository/browser acceptance blocker, not permission to spend Vercel. Once that check is green and the candidate is still current with `main`, run `node scripts/request-vercel-final-gate.mjs <PR_NUMBER>` if no exact-head Vercel provider object exists yet. Only poll after the persistent final-gate request has actually produced that provider object.
...
5. **Name the boundary of the conclusion.** A successful exact-SHA Vercel status proves provider build/deploy acceptance only when the provider object is the intended real execution and reached the required success state. Merge readiness also requires exact-head `public-ci-gate=success`; full Production acceptance still requires the stable alias / representative route checks in Section 9 when the task asks for them.
