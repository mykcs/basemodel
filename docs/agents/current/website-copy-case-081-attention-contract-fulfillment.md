# CASE-081 — 有 Reader Contract 也可能认知失败：首屏必须只有一个主理解中心
...
- **CASE-068** 已经规定“一个首屏只承担一个主要理解任务”；CASE-081 证明这条规则不能只靠 prose contract 或 H1 几何检查执行。
- **CASE-069** 说明字号、加粗和标题是在分配注意力；CASE-081 把这个结论推广到按钮、导航、卡片和固定控件。
- **CASE-079** 说明即使一行里有部分信息有用，也要继续审查剩余重复层；CASE-081 同样要求逐个判断首屏元素是否真的需要同时存在。
