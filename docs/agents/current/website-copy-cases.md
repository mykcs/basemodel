...
**认可案例**：`7B 与 3B 的第一轮购物实验`；首段直接写 `我们分别用 7B 和 3B 模型做了 WebShop 实验。`。如果实验设计确实从同一个 frozen checkpoint 分成两个 treatment arm，也优先写 `从同一 checkpoint 分成两组`，让动作本身承担含义，而不是把 `fork / 分叉` 当叙事风格。
...
### 案例簇总结：CASE-061–067 — 零上下文读者先看到现实对象

这七个连续真人反馈不是七条互不相关的禁词。它们共同指出同一个失败机制：**页面第一层要求读者先解码网站作者的关系式、背景课、阅读姿势、内部路线名、统计术语或叙事隐喻，真正的实验对象、比较双方与实验事实反而后出现。**

从 CASE-061–067 抽象出的当前规则是：**零项目上下文的读者先看到现实对象、对象类型、比较双方、共同任务、动作和结果；背景知识链接到 canonical explainer；内部 route / phase / provenance、方法学术语与非必要叙事包装放第二层或删除。** 这不意味着删除专业词、英文、统计方法或所有关系词；它们在读者已经知道“谁和谁做了什么”之后，或关系本身确实影响科学解释时，仍然应该精确保留。
