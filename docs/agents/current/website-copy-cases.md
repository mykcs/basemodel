...
`CASE-###` 是这份共享案例库的稳定身份，不是某个分支自己的行号。新增案例前先看 exact current base 和相关并行 PR；`当前文件最大编号 + 1` 在并发写入期间只能视为临时编号。集成前必须在组合树上检查 CASE ID 与锚点唯一性。发生碰撞时保留两边事实，只重编号较晚/成组的记录并同步索引与引用；不能为了“解决冲突”删掉另一个真人反馈案例。机器保护见 `src/lib/websiteCopyCaseIdGovernance.test.ts`。
...
| 训练设计与局部设计语言 | 页面归属、导航、字体、动效和共享组件都服从读者理解路径 | CASE-073–080 |
...
### 案例簇总结：CASE-061–068 — 零上下文读者先看到现实对象与最重要事实

这八个连续真人反馈不是八条互不相关的禁词。它们共同指出同一个失败机制：**页面第一层要求读者先解码网站作者的关系式、背景课、阅读姿势、内部路线名、统计术语、叙事隐喻或一整张等权重信息清单，真正的实验对象、比较双方与最重要事实反而不够突出。**

从 CASE-061–068 抽象出的当前规则是：**零项目上下文的读者先看到现实对象、对象类型、比较双方和一个最重要的事实；首屏只承担一个主要理解任务。共同任务、必要状态与会改变结论的边界保持可见；背景知识链接到 canonical explainer；内部 route / phase / provenance、方法学术语、重复 orientation 与非必要叙事包装放第二层、渐进披露或删除。** 这不意味着删除专业词、英文、统计方法、所有关系词或所有细节；它们在读者已经知道“谁和谁做了什么”之后，或它们本身确实改变科学解释时，仍然应该精确保留。
...
### CASE-073 — 页面归属按读者理解路径决定，不按实现模块决定
...
### CASE-075 — 动效的验收标准是理解收益，不是“页面用了 HTML 动画”
**PREFERENCE · 2026-09-07 · direct human feedback**
...
### CASE-078 — 共享组件里的装饰层会把一个错误复制到整组页面
...
### CASE-079 — 不要用英文或双语小标签把已经说清的标题再说一遍
**PREFERENCE · 2026-09-07 · derived from direct human feedback**
...
### CASE-080 — 序号可以保留，序号后面的主持人标签仍要审查
**PREFERENCE · 2026-09-07 · derived from direct human feedback**
