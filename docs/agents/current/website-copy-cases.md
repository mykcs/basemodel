...
**2026-09-09 Owner 二次校正（覆盖上一条中的手机 reflow 结论）**：用户实际复看 Preview 后明确要求“手机上还是 16:9”。因此本汇报页的最新规则是：**桌面和手机都保持同一张固定 1280×720、16:9 的 slide 构图；窄屏通过受控的横向滚动 / 浏览器缩放查看整张 slide，不把内部两列、表格、公式或版式拆成纵向网页。** 这里保留上一条记录，是为了保留反馈演化轨迹；生成与验收时必须以后面的 owner 校正为准。
