...
| 叙事隐喻与装饰层级 | 实验对象和事实状态优先；分岔/路线/关卡等故事包装与无信息英文 eyebrow 删除 | CASE-067 |
...
### 案例簇总结：CASE-061–066 — 零上下文读者先看到现实对象

这六个连续真人反馈不是六条互不相关的禁词。它们共同指出同一个失败机制：**页面第一层要求读者先解码网站作者的关系式、背景课、阅读姿势、内部路线名或统计术语，真正的实验对象与比较双方反而后出现。**

从 CASE-061–066 抽象出的当前规则是：**零项目上下文的读者先看到现实对象、对象类型、比较双方、共同任务、动作和结果；背景知识链接到 canonical explainer；内部 route / phase / provenance 与方法学术语放第二层。** 这不意味着删除专业词、英文或统计方法；它们在读者已经知道“谁和谁做了什么”之后仍然应该精确保留。
...
<a id="case-067-科研标题不要为了叙事感发明分岔"></a>
### CASE-067 — 科研标题不要为了“叙事感”发明分岔
**PREFERENCE · 认可案例 + 反面案例 · 2026-09-07 · owner live-page feedback**

**反面案例**：`第一轮购物学习：7B 与 3B 的分岔`、`路径分叉`。这里真正发生的事情只是分别用 3B 和 7B 做实验，随后两组实验出现不同结果。“分岔”没有增加科学信息，却会让读者额外猜“从哪里岔开、为什么岔开、是不是存在某个共同分支点”。同样，`关卡说明` 会把普通实验节点包装成游戏关卡，让读者去理解作者的叙事风格，而不是实验本身。

**认可案例**：H1 直接写 `3B 和 7B 的第一轮购物实验`；首段写 `我们分别用 Qwen2.5-3B 和 Qwen2.5-7B 做 WebShop 实验`；状态直接写 `7B 继续；3B 停止检查接口`；节点按钮写 `实验说明`。如果实验确实从同一个 checkpoint 建立两组 treatment，也优先写 `从同一 checkpoint 分成两组`，让实际操作承担含义，而不是用 `fork / 分岔` 制造故事感。

**第二个反面案例**：中文页面 H1 上方出现 `HISTORICAL MAP · FIRST RUN`、`ROGUELIKE RESEARCH MAP · EXPLORATION`、`THE RESEARCH EVOLVED WITH THE QUESTION`，或者 section 上方出现 `STAGE 1 → STAGE 2 → OUTCOME`。这些短语没有提供日期、状态、实验对象或 provenance，只是为了“看起来有一层设计”。

**认可处理**：没有新增信息就删除 eyebrow / kicker；如果它确实承担复现类别、日期、阶段、状态或 provenance，就先用自然中文表达，例如 `复现 · 原始基线`、`复现 · 密钥配置`，而不是为了版式保留全英文大写标签。

**同类扫描**：本轮还发现 `Stage 1 设计分岔`、`Core fork question`、`FLOOR 1 → FLOOR 6` 与问句式 `四组实验怎样一起回答问题？`。分别改成实际设计选择、实际比较问题、历史阶段和直接命名比较对象的标题；同类能力探索页面里的装饰性英文 eyebrow 也一起删除或翻成有信息的中文标签。

规律：**科研页面第一层先说实验对象、实际比较、实验操作和事实状态。不要为了“有故事感”把普通差异包装成分岔、路线、地图、楼层、关卡等隐喻，也不要用只表达内部页面分类的英文 eyebrow 占用读者注意力。关系词只有在真实关系本身影响科学解释时才值得出现；eyebrow / kicker 是可选的信息层，不是必须填满的 UI 插槽。**
