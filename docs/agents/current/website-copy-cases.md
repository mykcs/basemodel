...
owner 明确要求从最基础的 **loss** 开始，再逐步进入 **Task Vector → 参数谱/范数 → 输出长度/任务步数 → entropy**，让指标复杂度逐层增加。更重要的是，每一个小 subsection 都必须用同一个顺序：
...
- Task Vector 再回答“一轮参数动了多少 / 往哪动”；
- Frobenius norm、spectral norm、rank、cosine 再进入长期参数 State 的几何；
...
规律：**实验页的后分析应该像从显微镜低倍逐渐换到高倍：先确认训练是否健康，再看参数变化，再看参数几何，最后看行为。每换一层都先定义“现在在看什么”，再展示“看到了什么”，最后才讨论“这意味着什么”。**
