...
**2026-09-09 局部前因补充**：即使整套汇报已经按真实时间顺序排列，单页仍可能让第一次听的人“掉线”。`15 → 30`、`2048 → 4096` 这类实验转折不能只给变量和结果；必须先交代**上一层观察 / 继承设置是什么、为什么因此提出这个假设**。例如：15 步来自最初沿用的 SEED WebShop trainer recipe；前面的 fresh-task 诊断持续没有完成购买，才值得单独检验“是不是动作预算太短”。Text Memory 的 2048 是 A1 在实验前冻结的单次输出硬上限；3B 在同 20 条记录的 primary 与 repair 上都撞到这个上限，才提出“一次生成做长到 4096 会不会够”的 A1.1；4096 在 3B / 1.7B 都失败后，才有理由转向有边界的 10+10 结构。

规律：**科研 slide 的最小完整单元不是“实验 → 结果”，而是“前因 → 假设 → 单变量实验 → 结果 → 下一步判断”。** “补背景”也不是把项目历史整段搬上来；只补足能解释“为什么此刻会问这个问题”的那一个前因。第一次出现整个研究对象时同样适用：先交代 OpenEVO 是什么、WebShop 在测什么、SEED 为什么成为参照，再给结果表。
