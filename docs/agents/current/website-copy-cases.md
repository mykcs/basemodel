...
| 首次读者完整性 | 数字、状态、否定词、内部编号和项目术语都必须自解释 | CASE-051–058 |
...
<a id="case-051-数字先解释数的是什么"></a>
### CASE-051 — `7 / 8` 先解释“数的是什么”，再谈结果
**PREFERENCE · 2026-08-31 · 用户直接反馈**
反面：`单个 block 的 qualifying identity 最大只有 7，而 gate 需要 8。`
认可：`旧程序每 256 次任务尝试单独检查：同一块里至少要有 8 个不同任务各自重复完整成功，才允许更新参数；实际最好的一块只有 7 个。这里的 7 和 8 都是“不同任务数”，不是模型分数。`
规律：**数字不能只给作者熟悉的计数名；第一次来的读者必须知道分子、分母、单位和对象。**

<a id="case-052-零必须说明是哪一种零"></a>
### CASE-052 — `0` 不是统一状态
**PREFERENCE · 2026-08-31 · 用户直接反馈 + PR #341 延伸**
反面：`0 update / 0 score / final pending` 混在同一组状态里。
认可：`0 次 Stage-2 参数更新` 明确解释为“实验跑了并收集了经验，但旧规则没有允许修改参数”；`0 / 128 完整成功` 明确解释为“128 个有效评测任务里完整成功 0 个”；`final 未运行` 则直接写“没有最终评测结果”。
规律：**任何醒目的 0 都必须写出它是什么 0，以及它不是什么。**

<a id="case-053-状态词不能裸露"></a>
### CASE-053 — `BLOCKED / NOOP / Pending / sealed` 不能直接当读者文案
**PREFERENCE · 2026-08-31 · 用户直接反馈**
反面：`Stage1 / BLOCKED`、`carrier NOOP`、`final sealed`。
认可：`Stage 1 已完成，但还没有满足进入下一阶段的条件，因此停在这里等待下一步决策（内部状态：BLOCKED）`；`这一轮没有足够证据支持修改该模块，因此保持原状态（NOOP）`。
规律：**状态码可以保留作审计名，但主句先说人能理解的状态。**

<a id="case-054-否定结论保留有效部分"></a>
### CASE-054 — `设计错误 / 已取代` 后要说明什么仍然有效
**PREFERENCE · 2026-08-31 · 用户直接反馈**
反面：`历史 / 设计错误 / 已取代` 作为唯一定位。
认可：`旧 Stage 2 的逐块硬门槛设计已被取代；历史 rollout、成功轨迹和 checkpoint 仍保留为有效运行证据。`
规律：**否定一个设计，不等于否定整批数据；公开文案必须把“作废的是什么”和“仍有效的是什么”分开。**

<a id="case-055-项目术语首次出现先给中文含义"></a>
### CASE-055 — `carrier / successor revision / evidence boundary` 首次出现先给含义
**PREFERENCE · 2026-08-31 · 用户直接反馈延伸**
反面：`4 bounded carrier decisions → compose once → State_(k+1)`。
认可：`同一轮证据分别交给四种可继承状态（carrier）判断是否更新，最后只合并一次，形成下一轮使用的新状态（successor revision）。`
规律：**专业词可以保留，但不能要求读者先解码项目英语。**

<a id="case-056-内部编号降级为索引"></a>
### CASE-056 — `H1.40 / T2 / C1` 是索引，不是解释
**PREFERENCE · 2026-08-31 · 用户直接反馈延伸**
反面：`H1.40 upstream gates failed; T2 unopened.`
认可：`第二代经验整合没有同时通过“新知识获取、旧能力保持、已有收益保持”三项检查，因此预留的新任务迁移评测没有启动（实验编号 H1.40，评测代号 T2）。`
规律：**标题和主句先说自然语言；实验编号放括号、证据层或链接里。**

<a id="case-057-没有发生不能写成效果差"></a>
### CASE-057 — “没运行”不能写成“失败”
**PREFERENCE · 2026-08-31 · 用户直接反馈延伸**
反面：`T2 failed`、`final = 0`，但对应评测实际上没有启动。
认可：`T2 没有运行，因此没有 T2 分数，也不能判断它是成功还是失败。`
规律：**absence of measurement 不是 negative measurement。**

<a id="case-058-first-reader-completeness-test"></a>
### CASE-058 — 一句话必须经得起“第一次来的研究生”测试
**PREFERENCE · 2026-08-31 · 用户直接反馈总结**
反面：作者看到内部名词就能补全上下文，因此觉得 `7/8 gate`、`59/80 + 171 partial`、`PREPARED` 已经够清楚。
认可：删除聊天历史和内部记忆后，页面本身仍能回答“发生了什么、数字数什么、为什么这样、能推出什么、不能推出什么”。
规律：**聪明读者不等于拥有项目上下文。first-time-reader completeness 是公开科研文案的验收标准。**
