...
| 科研标题的叙事包装 | 实验对象和事实状态优先；装饰性分岔/路线/地图和无信息英文 eyebrow 删除 | CASE-067 |
...
**反面案例**：`第一轮购物学习：7B 与 3B 的分岔`、`HISTORICAL MAP · FIRST RUN`、`路径分叉`。这里真正发生的事情只是分别用 3B 和 7B 做实验，随后两组实验出现不同结果。“分岔”额外制造了空间/故事隐喻，读者会自然追问“为什么在这里分岔”；纯英文 eyebrow 又要求中文读者先解码页面内部分类，却没有增加科学信息。

**认可案例**：H1 直接写 `3B 和 7B 的第一轮购物实验`；首段写 `我们分别用 Qwen2.5-3B 和 Qwen2.5-7B 做 WebShop 实验`；状态直接写 `7B 继续，3B 停止`。当 H1 / H2 已经说明实验对象和阶段时，删除 `HISTORICAL MAP · FIRST RUN`、`STAGE 1 → STAGE 2 → OUTCOME` 这类不增加信息的英文 eyebrow / kicker。

**同类扫描**：本轮还发现 `Stage 1 设计分岔` 与问句式 `四组实验怎样一起回答问题？`。前者改成 `Stage 1 的两项设计选择`，后者改成直接命名比较对象的 `3B / 7B × self / MiniMax 的四组比较`。

规律：**科研页面第一层先说实验对象、实际比较和事实状态。不要为了“有故事感”把普通差异包装成分岔、路线、地图等空间隐喻，也不要用只表达内部页面分类的英文 eyebrow 占用中文读者注意力。只有当真实分支拓扑本身就是研究对象、且这个词能减少解释成本时，才保留这类表达。**
