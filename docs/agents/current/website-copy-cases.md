...
| 注意力与视觉中心 | 标题、加粗和大字号只强调真实主题/结论，不强调作者的教学动作 | CASE-068 |
| 术语与代号的就地解释 | 先不用代号；必须保留时先给人类名称，并在第一次出现的位置解释 | CASE-069 |
| 解释层与可见 UI | 深浅层次是信息深度，不机械渲染“一句话看懂 / 专业解释”标签 | CASE-070 |
...
<a id="case-068-视觉中心必须给真实主题"></a>
### CASE-068 — `先分清两种“新任务”` 不应该成为视觉中心
**PREFERENCE · 认可案例 + 反面案例 · 2026-09-07 · owner live-page feedback**
...
<a id="case-069-术语和代号必须就地解释"></a>
### CASE-069 — 术语表不能要求读者离开当前句子去“查字典”
**PREFERENCE · 认可案例 + 反面案例 · 2026-09-07 · owner live-page feedback**
...
<a id="case-070-解释深度不是可见-ui-模板"></a>
### CASE-070 — `一句话看懂 / 专业解释 / 实验依据` 是深度概念，不是必须显示三层标签
**PREFERENCE · 认可案例 + 反面案例 · 2026-09-07 · owner live-page feedback**
...
### 案例簇总结：CASE-061–070 — 零上下文读者先看到现实对象

这十个连续真人反馈不是十条互不相关的禁词。它们共同指出同一个失败机制：**页面第一层要求读者先解码网站作者的关系式、背景课、阅读姿势、内部路线名、统计术语、叙事隐喻、远处术语表或额外解释层，真正的实验对象、比较双方与实验事实反而后出现。**

从 CASE-061–070 抽象出的当前规则是：**零项目上下文的读者先看到现实对象、对象类型、比较双方、共同任务、动作和结果；视觉中心只强调这些真实主题与结论；术语在第一次出现的位置就地解释；内部 route / phase / provenance、方法学术语与非必要叙事包装放第二层或删除；信息深度不能机械变成新的可见 UI 模板。** 这不意味着删除专业词、英文、统计方法、证据层或所有关系词；它们在读者已经知道“谁和谁做了什么”之后，或关系本身确实影响科学解释时，仍然应该精确保留。
