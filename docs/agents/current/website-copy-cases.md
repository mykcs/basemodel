...
**2026-09-21 follow-up · results page**：owner 指出三组 160 轮 OpenEVO 实验的公共结果页长期只把参数更新讲细，却没有直接回答 Text Memory、Skill Bundle、Agent System 在这 160 轮里到底有没有实际 UPDATE。修正方向不是再补一张“模块介绍”图，而是在实验设置附近给出四条状态通道的真实终态 UPDATE / NOOP 账本，并明确 **NOOP ≠ 没有被运行时使用**。例如普通 OpenEVO 的 Skill Bundle 只 UPDATE 1 次，却仍在 42,270 个已记录环境 step 中被 selected；因此“内容是否改变”和“是否参与上下文”必须拆开陈述。规律扩展为：**当结果解释依赖一个会共同形成下一轮状态的多载体系统时，局部自包含不仅要说这些载体是什么，还要交代它们在本次实验里实际发生了什么。**
...
**2026-09-11 · Q17 结果页复发**：owner 直接反馈 `但是你创造的这个网页里的语言，请说人话`。当时首屏虽然科学边界正确，但用了 `任务能力发生重排`、`95% CI 跨 0`、`operational decoding entropy`、`adjacent cosine` 等研究者内部表达。修订后，首屏先说 `R128 比 R127 稍差，但没有训练曲线看起来那么严重`，直接给 `63.58 → 59.41`、`10 题 → 8 题` 和 `现在还不能说模型整体真的退步了`；统计术语和 update cosine 下沉到解释/实验细节层。

补充规律：**科学边界可以严谨，但第一层语言必须先让普通读者知道“发生了什么、差多少、现在能不能下结论”。统计名词、内部缩写和训练诊断量只有在它们改变判断时才进入主线，并要就地翻译成人话。**
...
<a id="case-097-数学等价与跨领域映射要分层讲"></a>
### CASE-097 — 数学等价与跨领域映射要分层讲
**PREFERENCE · RESEARCH COPY + MATH · 2026-09-20 · direct owner feedback**

owner 指出“三个 OpenEVO 实验”页链接出去的 Gated-Delta 页面内容过于混杂：主实验页应该用很短的篇幅交代公式怎样从序列建模里的 State 更新推出来，以及迁到 OpenEVO 时哪些东西是严格等价、哪些只是对应；真正完整的数学推导则下沉到独立子页。owner 同时提醒，HPL 已经要求公式使用网页 LaTeX 排版，但这条规则没有完整辐射到旧 Gated-Delta 子页。

认可方向：
- 主实验页只保留理解三组实验所需的推导桥梁：online regression / Delta residual 的来源、Gated Delta 的 α / β，以及 sequence State → parameter State 的角色对应；
- 详细子页从目标函数、梯度、SGD 更新、residual 展开一直推到 Gated Delta，并明确张量 shape / 外积方向；
- “论文公式 → residual 形式”可以叫严格代数等价；“sequence State → OpenEVO 参数 State”只能叫逐角色映射，除非确有张量级等式证明；
- 旧资格实验、失败映射和历史证据留在详细页后半段，不和主推导抢第一层注意力；
- 所有承担数学论证的公式统一用现有 KaTeX / MathML 组件，不再用 HTML sub/sup 或 code 文本伪装公式。

反面机制：把“等价”和“映射”混成一句 1:1；总览页塞完整附录；详细页一开头先讲资格实验结果而不是公式来源；同一站点一页用 KaTeX、一页继续手拼上下标。

规律：**跨领域借公式时先把“原领域内部的严格等价”推清楚，再单独说明“迁移到本项目后的角色对应”。详细推导可以很深，但深度应该下沉到专页；总览页只承担让读者知道公式从哪里来、哪些对应关系可信。**


<a id="case-098-方法名不能把未运行的机制写成已验证"></a>
### CASE-098 — 方法名不能把未运行的机制写成已验证
**PREFERENCE · RESEARCH COPY + SCIENTIFIC BOUNDARY · 2026-09-21 · direct owner correction**

owner 在检查 `bounded-effective-state-gdr` 页面时追问：“这里的 GDR 实验没有用 alpha 吗？”重新核对实现与页面后确认：这次正式长期实验固定 `α=1`，真正动态启用的是 `β`。因此把第三组一直简称成“+ GDR”会让零上下文读者自然理解成完整的动态 α + 动态 β Gated Delta 已经被测试，这超出了真实实验。

owner 随即要求：“把网页里所有有关的表述都改了，没做的实验就暂时空着。”

认可方向：
- 已完成第三组统一叫 **Bounded Online Recurrence + β-gating（α=1）**；
- 首屏消融表把“动态 β”和“动态 α”拆成两列：已完成第三组是 β ✓、α —；
- **动态 α + 动态 β** 可以作为下一实验保留一行，但必须明确写“未做”，训练轨迹、终评和统计结果全部保持 `—`；
- Gated Delta Networks 原论文及其 α/β 推导仍可准确称为 Gated Delta / GDR；需要纠正的是“本次已完成 treatment”的公共名字，不是删除理论来源；
- `EFFECTIVE_STATE_GDR_LORA_V1`、`BOUNDED_OFF`、证据路径等历史审计身份保持原样，不能为了公共文案一致而重命名封存实验。

反面机制：
- 因为内部 experiment ID 含 GDR，就在标题、图例、结果和 Analysis 都写“+ GDR”；
- 用一个“GDR ✓”把动态 β 与动态 α 合并，掩盖 α 实际固定为 1；
- 为了让消融表完整，给尚未运行的 α+β 版本填推测分数、预期趋势或结论；
- 反过来把论文里的 Gated Delta 也改成“β-gating”，混淆理论原式与当前 treatment。

规律：**公共方法名描述“这次真的做了什么”，历史 ID 描述“这条资产当时叫什么”。两者可以不同。一个完整方法有多个自由度时，未启用的自由度必须在首层显式露出来；未运行实验宁可留空，也不能靠名称暗示已经验证。**


<a id="case-099-实验指标按复杂度递进每节固定定义结果分析"></a>
### CASE-099 — 实验指标按复杂度递进，每节固定“定义 → 结果 → 分析”
**PREFERENCE · RESEARCH COPY + DATA NARRATIVE · 2026-09-21 · direct owner correction**

owner 在审阅 `bounded-effective-state-gdr` 页面时指出，实验分析虽然堆了很多真实指标，但叙事顺序很差：Task Vector、参数范数、方向、长度、entropy 与 W&B 图分散在不同地方，读者必须自己判断哪些是基础指标、哪些是更复杂的诊断。

owner 先明确要求从最基础的 **loss** 开始，再逐步进入参数与行为诊断；随后在共同确定页面结构时，进一步把最终顺序细化为 **loss → Frobenius 范数 → Task Vector 相关性 → 谱与方向 → 输出长度/任务步数 → entropy**。更重要的是，每一个小 subsection 都必须用同一个顺序：

1. **指标是什么**：先让没做过实验的人知道这个量在测什么；
2. **这次实验的结果**：再给图、表和实际数字；
3. **分析**：最后解释它支持什么判断、又不能推出什么。

认可方向：
- loss 先回答“优化器有没有正常拟合”，而不是一上来就讲参数几何；
- Frobenius norm 先回答“参数总体改了多大”，先把最简单的大小问题讲清楚；
- Task Vector 相关性再问“更新大小和 Task Score 是否真的一起变化”；
- spectral norm、rank、cosine 再进入更细的参数结构与方向；
- tokens / episode steps 把视角从参数层推进到任务路径；
- action-family entropy 最后讨论策略行为分布是否越来越集中，并明确它不是 token entropy；
- W&B 图跟着对应指标放，不再单独做一个“所有图都在这里”的图集区。

反面机制：
- 按研究者事后想到问题的顺序写 Analysis，导致基础指标反而比复杂指标晚出现；
- 同一节混放 Task Vector、norm、cosine 和因果猜测，读者不知道哪个数在回答哪个问题；
- 把所有图放到独立 W&B 大节，正文和图表需要来回翻；
- 只报数，不解释指标；或只解释指标，不把本实验结果紧跟其后。

规律：**实验页的后分析应该像从显微镜低倍逐渐换到高倍：先确认训练是否健康，再看参数变化“有多大”，再问这种变化是否与任务分数相关，然后才看更细的谱与方向，最后进入行为。每换一层都先定义“现在在看什么”，再展示“看到了什么”，最后才讨论“这意味着什么”。**

<a id="case-100-seed论文参考与消融表说明要放对位置"></a>
### CASE-100 — SEED 论文参考与消融表说明要放对位置
**PREFERENCE · RESEARCH UI + RESULTS · 2026-09-21 · direct owner correction**

owner 在审阅 `bounded-effective-state-gdr` 页顶部消融表时连续指出两件事。第一，“seed”指的是 **SEED 论文在 Qwen3-1.7B / WebShop 上报告的结果**，不是 RNG seed / random seed；表格应把 SEED 作为外部论文参考行，给出原论文 Score 87.1、Success 77.3%。第二，消融表各行“这一组到底做了什么”的解释不应该塞进 table cell，而应像 CVPR / ICLR 论文一样放进统一 caption，让表格主体只保留实验名、机制列和结果。

本次同时核对了可比性边界：SEED 论文的 87.1 / 77.3% 来自论文自己的 128 个 WebShop validation tasks，不是我们三条 OpenEVO 最终模型共用的同一冻结 128 题。因此它可以作为“论文参考基准”，但不能把 `87.1 - 60.72` 之类的数值差直接写成严格同题收益或损失。

认可方向：
- 顶部表格可加入 `SEED（论文，Qwen3-1.7B）` 参考行，展示论文原始 Score / Success；
- SEED 行不在 Bounded / 动态 β / 动态 α 列打勾，因为它不是这组 OpenEVO 机制消融的一条 arm；
- 三条已完成 OpenEVO 与动态 α+β 未做空位继续使用自己的机制列和结果；
- table cell 只保留实验名、机制标记和指标；每一行“做了什么”统一放到 `表 1. / Table 1.` caption；
- caption 同时说明 SEED 与 OpenEVO final 题集不同，避免把外部论文参考误读成 strict same-panel ablation；
- “seed”如果语义可能指 SEED 论文或随机种子，优先根据当前科研对象解歧，并在 UI 里写完整 `SEED（论文）`，不要让读者猜。

反面机制：
- 把 owner 说的 “seed” 直接解释成 RNG seed，并给表格新增 `seed base = ...`；
- 把 SEED 87.1 / 77.3% 放进表却不说明题集不同；
- 表格第一列塞两三行方法解释，导致结果表像内部 dashboard 而不是论文表；
- 为了表格整齐，把未做的动态 α+β 填成推测结果或把 SEED 行错误打上本地机制对勾。

规律：**论文式消融表先让单元格承担“谁 / 加了什么 / 得到什么”，让 caption 承担“每行是什么意思 / 哪些能严格比较 / 哪些只是外部参考”。遇到 `SEED` 这类同时可能被误解成普通英文词的专名时，公共页面直接写完整对象身份。**

边界：不是所有外部论文都要塞进消融表。只有它是读者理解本实验相对位置的核心参考时才加入；加入后必须保留数据集、题集、模型和评测口径差异，不能把 contextual baseline 包装成 matched arm。

<a id="case-101-非线性流程的主角是关系不是步骤卡片"></a>
### CASE-101 — 非线性流程的主角是关系，不是步骤卡片
**PREFERENCE · RESEARCH UI / FLOW DIAGRAM · 2026-09-12 · repeated direct owner feedback**

Vanilla SD-LoRA 机制页最初已经有正确事实、编号步骤、16:9 Slide、Archify 候选和验证，但 owner 仍直接指出：**“完全没法看出来流程流动，是我的指令不对吗？”** 这说明问题不是少一个箭头，也不是 Archify 有没有被调用，而是最终页面把节点/卡片做成了视觉主角，关系只剩很弱的提示。

反面：
- 六个并列步骤块之间只放很小的 `→`，读者要先读完文字才能自己还原顺序；
- 真实流程被放到 Slide 下方的第二张图，首屏仍然是卡片摘要；
- `Sₜ₊₁ → 下一轮` 只写成一句“循环继续”或一个 `↺` 字符，没有真正 return edge；
- desktop 上支路正确，手机却把 failure / replay 全部排到流程末尾，因果 attachment 被破坏；
- Archify 9/9 或构建/浏览器全绿被当成“流程已经表达清楚”的替代证据。

认可方向不是某一张具体截图，而是一条稳定的视觉语法：**当语义包含 side input、branch/join 或 feedback loop 时，edge / routing 必须先于卡片成立。** 先画一条不读段落也能走完的 main path，再把 failure、replay 等支路挂到它真正离开/汇入的节点；回环必须是真实 connector。节点负责命名“站点”，关系负责让人看见“东西怎样流动”。手机可以换布局，但不能改变这些因果 attachment。

边界：
- 简单线性步骤仍可以用 `<ol>`，不是所有流程都要 SVG。
- 箭头不应成为装饰；只有真实顺序、依赖、分支、汇入或回环才需要 connector。
- 动画只能强化已经清楚的静态路径，不能承担唯一的方向信息。
- 这条反馈没有授权把后来的 routed-flow 具体视觉升级为 Golden / Silver；owner 明确拒绝的是旧版关系表达，后继视觉仍需按具体任务验收。

规律：**“用了流程图工具”不等于“最终产品让人看见了流程”。工具验证拓扑，最终网页还要让关系本身成为第一等视觉对象。**
