...
1. Before the first public-page mutation, read `human-thinking-web-expression-contract.md`, `site-reader-attention-contract.md`, `research-site-presentation-contract.md`, the central human-expression preferences, and the relevant BaseModel-specific learned experience. Register/update the Reader Contract before choosing components. Do not build a dashboard first and only later ask what the owner actually prefers.
...
8. If this task creates or materially refines a reusable reader/preference rule, preserve the owner feedback through the central closeout and test transfer on at least one likely sibling plus one unrelated positive-control or route-role exception before calling the rule generalized. A valid Reader Contract or green copy/reader audit is structural coverage, not proof that older sibling pages already embody the same mental model.
...
5. Register/update the Reader Contract and follow the exact-artifact evidence rule: material visual/content changes invalidate earlier screenshots/review evidence. Do not reuse an old render as proof for the new page.
6. When the conversation already contains an owner-requested ELI5/plain-language explanation that materially resolved a comprehension gap, run a **plain-language coverage pass** before release. For each major technical section, preserve an equivalent `what this means` bridge before/alongside the professional metric. A correct table or threshold list is not proof that the ELI5 layer survived. If the owner explicitly says the current conversation wording itself is already good enough to publish or reuse, treat that wording as the accepted copy baseline: structure and deduplicate it, but do not “webify” it into more academic, managerial, or jargon-heavy prose. Do not paste every chat sentence: keep durable comprehension bridges, keep scientific boundaries, re-verify current facts, and exclude temporary state.
...
10. During iterative human review, use the fast review-only lane rather than the final gate. Before moving the final gate, classify whether the owner is still reviewing/correcting the surface or the candidate is actually merge-ready; active review stays on the review-only lane. A review handoff is complete only after opening the **hosted target route / anchor / slide** and confirming the specific claimed change is visible there. **Every owner-facing webpage delivery handoff — review or released — must include a clickable URL for the exact surface just verified:** the current-candidate Preview URL during review, or the canonical Production URL after merge/release. Reporting only PR / SHA / `READY` / merged state is false-complete for a page-delivery task. If the current build failed, do not reuse an older successful static output or old Preview URL as if it represented the new candidate; rebuild fresh generated output first. This target-specific check is not permission to rerun the unrelated full 205-case final matrix on every edit.
...
15. Apply `REPEAT-CORRECTION` and the correction-to-action witness in `project-agent-operating-principles.md` before the next affected action. Apply §10.10 of `human-thinking-web-expression-contract.md` to five-question orientation, actual dependency topology, readable text size, and desktop-versus-mobile scope. When the owner reports repeated comprehension failure despite prior prose standards, do not answer with another wording guide alone. Audit the shared semantic owner and sibling routes; establish typed required fields/state when practical; add negative cases and rendered reader-journey checks to the normal Gate; and keep explicit coverage labels so contextualized pages are not reported as fully rebuilt. **For material copy/layout work, always run the normal rendered visual/Reader Contract/browser acceptance that applies.** If the owner explicitly requests an independent comprehension study, bind the reviewer and evidence class directly to the changed surface; do not route that review through the retired local HPL preference/judge machinery. Structural/visual PASS and measured human-comprehension evidence remain separate receipts; do not turn reviewer availability into an ordinary UI-release dependency. **Before dispatching any independent reviewer, bind `requested evidence class -> eligible reviewer kind -> transport -> forbidden account surfaces`. A human-required receipt stays human. Browser automation may render/capture the target artifact, but it must not source reviewer identity by opening AI/chat or account/login/sync pages; such a hit is `NOT_EXECUTED`, so close the session and do not retry with fresh profiles.**
16. **Advisor briefing / slides / projected-deck specialization:** before the first substantial screen, classify the artifact role and load the central human-expression plus BaseModel-specific learned evidence. Write the scientific story as observable question/evidence pivots before arranging slides. Before a result table or chronology claims to cover the experiment family, inventory model/arm identities and terminal metrics from the live experiment authority; the current website is publication state, not experiment-inventory authority.
...
18. `HTML` is an implementation medium, not a visual-style reset. For an explicit projected deck, preserve the owner-learned presentation composition from current project authority and central learned evidence instead of silently converting it into a generic long-form webpage or reusing a stale fixed-phone canvas rule.
19. **Central learning traceability:** when the task itself is about真人反馈、RAW、案例归纳或 preference learning, preserve the owner wording centrally first and trace any BaseModel rule/guard back to the central evidence. Do not recreate a local retrieval model merely to make evidence easier to query.
20. **Bind independent review to the surface actually varied.** For a migration or multi-route feature, choose the candidate/cold-read Reader Contract from the page whose visible copy/layout changed between candidates, not from a linked destination or broader feature name. Split review scopes when multiple surfaces materially change. In Phase B separate candidate-caused regressions from pre-existing/independent page debt; do not expand a narrow migration merely to make unrelated old debt satisfy a borrowed contract.
21. **Bind local visual evidence to the actual tree and server.** Before calling screenshots/browser/review evidence exact-head, confirm the worktree is clean for that SHA (or label the run dirty-tree preflight), read the dev/preview server's emitted URL, and verify a task-specific route/sentinel before capture. A requested port that auto-falls forward is not server identity.
22. **Treat navigation-entry removal as a shared-shell scan, not capability deletion.** If the owner says to remove `模型 / Model / Models` (or another object) from navigation, inspect the desktop global/header resource menu, mobile navigation, contextual child navigation, and their visible summary/meta copy together. Remove the direct destination consistently, but preserve the underlying route, search, comparison/task actions, and contextual links unless the owner explicitly includes them. Verify both absence of the unwanted nav destination and survival of at least one intended positive-control path.
23. **Blind cold read starts from the true default state.** Phase A first-attention evidence must use the route's actual default/persisted entry state. Do not open menus/details/filters/dialogs or scroll merely to expose the changed control and then treat that screenshot as the default-page verdict. Review expanded states separately when relevant.
