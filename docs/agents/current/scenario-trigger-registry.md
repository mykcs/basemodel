...
6. For the BaseModel public server page, label the site-owner account only as the relative `我 / Me` when authorized private context or the current user instruction resolves it reliably; never publish the owner's system username. Exclude the owner, then regenerate temporary anonymous rankings (`用户一 / 用户二 / …`) for everyone else by attributable total. Persist no other-user identity mapping, username, home path, container identity, SSH detail, IP, or GPU UUID. Treat attribution as storage responsibility, not literal proof of who authored each file.
