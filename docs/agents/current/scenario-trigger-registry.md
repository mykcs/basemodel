...
When the owner says `吸收真人反馈`, `偏好 closeout`, `Human Preference Learning closeout`, or equivalent after the page work reaches a stable point, stop ordinary polishing and load `human-feedback-ingestion-closeout.md`. Build the complete coverage ledger **before** mutating preference assets, preserve non-binary verdicts and supersession, then run `npm run feedback:ingestion:verify -- <INGESTION-ID>` plus the required preference/Reader Contract gates. Do not ask the owner to restate feedback that is already in the current conversation.
