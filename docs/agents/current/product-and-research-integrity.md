...
Risk-based Playwright acceptance runs in Vercel Pro before merge and again on Vercel Production. CircleCI is retained as non-blocking shadow/fallback evidence during cutover, and the repository-scoped Mac/OrbStack runner is manual fallback only. Deterministic source/data/unit/V2 audits remain deployment blockers; Cloudflare production-smoke only observes the deployed Vercel origin.
...
7. Use branch -> PR -> automatic exact-head Vercel Pro deterministic + risk-based browser acceptance -> merge for deployment-sensitive work; treat CircleCI only as non-blocking shadow evidence during cutover.
