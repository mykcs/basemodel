...
Full Playwright runs in CircleCI risk-based acceptance so browser downloads and execution stay out of Vercel Production. The repository-scoped Mac/OrbStack runner is manual fallback only. Deterministic source/data/unit/V2 audits remain appropriate deployment blockers; Cloudflare production-smoke only observes the deployed Vercel origin.
...
7. Use branch -> PR -> CircleCI risk-based CI -> optional exact-head Vercel Preview -> merge for deployment-sensitive work.
