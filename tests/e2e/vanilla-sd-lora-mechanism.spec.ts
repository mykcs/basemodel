...
test('Vanilla SD-LoRA page exposes the real round mechanism and scientific boundary', async ({ page }) => {
  ...
  await expect(page.locator('h1')).toContainText('SD-LoRA：把成功轨迹训练成下一轮候选参数');
  await expect(body).toContainText('LoRA 是冻结基础模型、只训练少量适配参数的方法');
  await expect(body).toContainText('文字记忆（Text Memory）、可复用技能（Skill Bundle）和全局行为规则（Agent System）');
...
for (const viewport of [
  ...
  test(`${viewport.name}: the first viewport stays on one parameter-write task`, async ({ page }) => {
    ...
    await expect(page.locator('.sdlora-intro__steps')).toContainText('当前 DirectApply 是检查通过就进入下一轮');
    ...
    expect(visibleMainHeadings).toEqual(['SD-LoRA：把成功轨迹训练成下一轮候选参数']);
