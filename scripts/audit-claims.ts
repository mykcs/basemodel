...
for (const entry of files) {
  ...
  if (entry.type === 'model') {
    ...
    if (data.openness?.custom_license === true && !sources.some((source: any) => source.type === 'official_license')) findings.push({ severity: 'warning', record, field: 'openness.license_name', kind: 'custom-license-without-license-source', action: 'Attach the official license text.' });
