...
export function checkStrictAudienceCopyInvariants(root = process.cwd()): CopyFinding[] {
  ...
  requireText(hero, 'COPY-SUBJECT-TITLE-001', "t('SEED 与 OpenEvo：同模型同任务的学习对照实验', 'SEED and OpenEvo: a same-model, same-task learning comparison')", 'The first-screen heading must name the durable research subject rather than an editorial instruction.');
