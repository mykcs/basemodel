...
export function checkStrictAudienceCopyInvariants(root = process.cwd()): CopyFinding[] {
  ...
  requireText(hero, 'COPY-SUBJECT-TITLE-001', "t('SEED 与 OpenEvo：同一个 Qwen、同一批任务', 'SEED and OpenEvo: the same Qwen and the same tasks')", 'The first-screen heading must name the durable research subject rather than an editorial instruction.');
