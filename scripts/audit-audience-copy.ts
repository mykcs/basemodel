...
export function checkStrictAudienceCopyInvariants(root = process.cwd()): CopyFinding[] {
  ...
  requireText(hero, 'COPY-SUBJECT-TITLE-001', "t('同一个 Qwen、同一批任务，只比较怎么学习', 'Same Qwen, same tasks: only the learning method changes')", 'The first-screen heading must name the durable research subject rather than an editorial instruction.');
