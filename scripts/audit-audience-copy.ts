...
export function checkStrictAudienceCopyInvariants(root = process.cwd()): CopyFinding[] {
  ...
  for (const required of ['3B 和 7B 的第一轮购物实验', '我们分别用 Qwen2.5-3B 和 Qwen2.5-7B 做 WebShop 实验。', '7B 继续；3B 停止检查接口', '实验说明']) requireText(firstRun, 'COPY-FIRST-RUN-HUMAN-001', required, 'The first-run page must describe the two model experiments directly in ordinary language.');
  for (const forbidden of ['第一轮购物学习：7B 与 3B 的分岔', 'HISTORICAL MAP · FIRST RUN', 'STAGE 1 → STAGE 2 → OUTCOME', '路径分叉', '两条模型线', '两条路线', '关卡说明']) ban(firstRun, 'COPY-FIRST-RUN-HUMAN-002', forbidden, 'Rejected narrative, decorative, or gamified first-run wording must not return.');
