...
export function checkStrictAudienceCopyInvariants(root = process.cwd()): CopyFinding[] {
  ...
  requireText(hero, 'COPY-SUBJECT-TITLE-001', "t('SEED 与 OpenEvo 的 ALFWorld / WebShop 对照实验', 'SEED and OpenEvo on matched ALFWorld / WebShop tasks')", 'The first-screen heading must name the durable research subject rather than an editorial instruction.');
