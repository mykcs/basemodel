...
const paperRationale = read('src/components/papers/PaperSelectionRationale.astro');
...
const detailTools = read('src/components/models/detail/ModelDetailTools.tsx');
...
const familyTimeline = read('src/components/FamilyTimeline.tsx');
const zhGuidePage = read('src/pages/guide.astro');
const enGuidePage = read('docs/archive/site-en/src/pages/en/guide.astro.archive');

pass('AV-ROUTES', ['src/pages/index.astro', 'docs/archive/site-en/src/pages/en/index.astro.archive', 'src/pages/workspace/index.astro', 'docs/archive/site-en/src/pages/en/workspace/index.astro.archive', 'src/pages/models/index.astro', 'docs/archive/site-en/src/pages/en/models/index.astro.archive', 'src/pages/papers/index.astro', 'docs/archive/site-en/src/pages/en/papers/index.astro.archive', 'src/pages/landscape/index.astro', 'docs/archive/site-en/src/pages/en/landscape/index.astro.archive', 'src/pages/guide.astro', 'docs/archive/site-en/src/pages/en/guide.astro.archive', 'src/pages/methodology.astro', 'docs/archive/site-en/src/pages/en/methodology.astro.archive', 'src/pages/data-status.astro', 'docs/archive/site-en/src/pages/en/data-status.astro.archive'].every(exists), 'active Chinese routes and archived English counterparts are present');
...
pass('AV-EXPLORER-BASE', read('src/pages/_bodies/models-index.astro').includes('baseCheckpoint=true') && explorer.includes('baseCheckpoint'), 'Base checkpoint has a real quick entry and remains part of explorer state');
...
pass('AV-PAPERS-UNKNOWN', paperRationale.includes('paper.models') && paperRationale.includes('Not recorded') && read('src/pages/_bodies/paper-detail.astro').includes('PaperSelectionRationale'), 'paper detail shows model roles and explicit unknown selection rationale even when no rationale records exist');
pass('AV-PAPER-METHOD-BOUNDARY', !paperExplorer.includes('model_selection?.[0]?.rationale') && paperExplorer.includes('methodSummary') && paperExplorer.includes('methodPending') && paperExplorer.includes('researchScope'), 'paper method summary is separate from model-selection rationale and missing summaries remain explicit');
pass('AV-PAPER-FILTERS', paperExplorer.includes('benchmark') && paperExplorer.includes('checkpoint') && paperExplorer.includes('reproducibility?.checkpoint_status'), 'paper explorer exposes dedicated benchmark and checkpoint filters');
pass('AV-GUIDE-COVERAGE', ['checkpoint', 'open-weight-source', 'architecture-scale', 'adaptation-evolution', 'reproducibility-fields', 'research-paths'].every((id) => read('src/content/guides/research-workbench.json').includes(`"id": "${id}"`)) && read('src/content/guides/research-workbench.json').toLowerCase().includes('external memory') && read('src/content/guides/research-workbench.json').includes('weight-updating self-evolution'), 'novice concepts include checkpoint, access, architecture, evolution, and reproduction boundaries');
pass('AV-GUIDE-HARDWARE-TRUTH', zhGuidePage.includes('启发式规划估算') && zhGuidePage.includes('不是硬件实测') && enGuidePage.includes('heuristic planning estimate') && enGuidePage.includes('not a measured hardware result'), 'guide distinguishes catalog tiers, heuristic VRAM planning, and measured hardware facts');
...
pass('AV-REPLACEMENT-THREE-MODES', substitute.includes("['strict', 'method', 'modern']") && substitute.includes('replacementVerdict') && substitute.includes('replacement-mode-verdicts'), 'replacement candidates expose strict, method, and modern-rerun verdicts simultaneously');
pass('AV-PAPER-WORKFLOW', paperWorkflow.includes('paper.workflow') && paperWorkflow.includes('unsupported relations remain blank') && paperWorkflow.includes('does not guess'), 'paper detail uses recorded workflow data and leaves unsupported relations blank without guessing');
pass('AV-PAPER-MODEL-LINKS', paperWorkflow.includes('openQuickView') && paperWorkflow.includes("/models/${model.id}/") && read('src/pages/_bodies/paper-detail.astro').includes("/models/${model.id}/"), 'paper model roles can open quick view and full model details');
...
pass('AV-MODEL-UNKNOWN-HUMAN', modelDetail.includes('UnresolvedFieldList') && read('src/components/models/detail/UnresolvedFieldList.astro').includes('fieldCatalog') && read('src/components/models/detail/UnresolvedFieldList.astro').includes('Unknown does not mean'), 'model detail names unresolved facts in human language and explains their research impact');
pass('AV-MODEL-DETAIL-NAV', modelDetail.includes('ModelDetailNav') && read('src/components/models/detail/ModelDetailNav.astro').includes('paper-adoption') && read('src/styles/v2-closeout.css').includes('position: sticky'), 'model detail has a sticky research-oriented section navigation');
pass('AV-REVISION-TRUTH', detailTools.includes("supports?.includes(MODEL_REVISION_FIELD)") && detailTools.includes('Model revision is not pinned') && !detailTools.includes("model.id}@${model.release_date}"), 'pinned revision requires an explicitly supporting source; release dates are never fabricated into revisions');
pass('AV-REPORT-ISSUE', detailTools.includes('issues/new') && detailTools.includes('Current source') && detailTools.includes('window.open'), 'data-issue action opens a prefilled GitHub issue instead of only copying a draft');
pass('AV-GLOBAL-QUICK-VIEW', exists('src/components/models/GlobalModelQuickView.tsx') && exists('src/pages/model-data/[id].json.ts') && appLayout.includes('GlobalModelQuickView') && familyTimeline.includes('openQuickView') && paperWorkflow.includes('openQuickView'), 'quick view is available across public research pages and loads one model on demand');
pass('AV-MOBILE-COMPARE', compare.includes('comparison-mobile-cards') && compare.includes('comparison-desktop-table') && read('src/styles/v2-closeout.css').includes('.comparison-mobile-cards { display: block'), 'mobile comparison has dedicated cards instead of relying only on a wide table');
pass('AV-FAMILY-HISTORY', familyTimeline.includes('!isCurrentModel') && familyTimeline.includes('papersForModel(papers, model.id).length > 0'), 'historical-paper note is based on non-current models actually used by papers');
...
const papersData = paperFiles.map((file) => JSON.parse(read(path.join('src/content/papers', file))) as { model_selection?: unknown[]; sources?: Array<{ id?: string; evidence_note?: string }> });
