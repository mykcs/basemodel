...
plutil -insert 'ProgramArguments.0' -string "$libexec_dir/mac-orbstack-reconcile.sh" "$pending_plist"
plutil -insert EnvironmentVariables -dictionary "$pending_plist"
plutil -insert 'EnvironmentVariables.PATH' -string \
  '/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin' "$pending_plist"
