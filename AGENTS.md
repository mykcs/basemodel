...
13. [`docs/agents/current/ci-provider-decision.md`](docs/agents/current/ci-provider-decision.md), [`docs/agents/current/hosting-architecture.md`](docs/agents/current/hosting-architecture.md), and [`docs/agents/current/deployment-policy.md`](docs/agents/current/deployment-policy.md) — BaseModel CI provider rationale plus current Vercel Pro authority, final-candidate-only spend model, CircleCI/manual fallback boundaries, Preview + Production workflow, and release closeout.
...
- **Provider writes are noun-bound at dispatch.** Immediately before any non-read connector/API call, bind `intended object -> exact action -> exact target`. For example, `open PR -> create_pull_request`; a file/ref/branch mutation is not an acceptable substitute for a PR mutation. If the selected tool/action names a different object, do not dispatch it. After an accidental write, clean only that object, verify the resulting changed files/refs, and change execution surface after a repeated mismatch. Detailed ownership remains in `project-agent-operating-principles.md` and the closeout trigger in `scenario-trigger-registry.md`.
- **PR acceptance is split across two exact-head owners; branch names are semantic.** Ordinary working PR refs do **not** enter Vercel. Public PR CI owns deterministic/browser acceptance and must finish with exact-head `public-ci-gate=success`; only then may `node scripts/request-vercel-final-gate.mjs <PR_NUMBER>` pin non-deploy `ci/vercel-gate-base` to live `main` and move persistent `ci/vercel-gate-final` to that exact PR head SHA. Vercel then owns provider build/deploy acceptance on the same SHA. Do not use `[vercel-preview]` or a freshly created alias ref as a spend/acceptance gate.
...
- **Iterative human-review Preview is lightweight by default.** While the owner is still reviewing repeated UI/copy/slide revisions, do not move `ci/vercel-gate-final` or pay for the complete Vercel acceptance build after every edit. Use the manual GitHub-hosted Fast Review workflow to build the exact PR head without provider secrets, then publish only its prebuilt artifact to the dedicated non-Git-connected review Preview surface (or equivalent non-authoritative static Preview). This fast Preview exists only to let the owner inspect the page quickly: it is not merge evidence, may omit `verify:deploy` and the full Chromium/Lab matrices, must remain non-Production/non-indexed, and must never replace exact-head final acceptance. When the candidate is actually ready to merge, return to public-GHA preflight plus the persistent exact-head Vercel final gate.
- **HPL control-plane changes are deploy-detached while proven unreachable from runtime UI.** The exact allowlist in `scripts/hpl-control-plane.mjs` may skip browser acceptance and `main` Production rebuilds only while its detachment scan proves no ordinary runtime `src` module imports those HPL modules. A new runtime importer, an unknown HPL path, or any gate-owner change restores fail-closed browser/deploy behavior automatically.
- **Name and verify the shell when syntax matters.** If a command depends on Bash semantics (`VAR=value`, `set -euo pipefail`, loops, arrays, heredocs, process substitution), request `/bin/bash` or use a standalone Bash/Python script. The requested shell is not execution proof: read the execution tool's reported launched interpreter before sending compound syntax. If it still reports `fish` or another dialect, use a simple explicit interpreter handoff such as `exec /bin/bash -lc <quoted-command>` only when quoting is trivial; otherwise write and syntax-check a standalone script. Do not assume an inner `bash -lc` rescues text already parsed by an outer `fish`. A wrong-shell parser failure is `NOT_EXECUTED`, not repository/server failure.
...
working PR / development branch
  ...
  -> full work uses 8 independent Chromium shards; bounded work uses one focused runner and non-UI work starts no browser runner
  -> when the owner needs a page to inspect now: manual GitHub-hosted Fast Review build -> prebuilt non-Git-connected review Preview
...
final non-draft current-base candidate
  ...
  -> exact-head Public PR CI must already be green (`public-ci-gate`)
  -> Vercel Preview runs the static production build and proves provider deployability on the same SHA
  -> required GitHub statuses: `public-ci-gate` + `Vercel`
...
-> keep ordinary PR pushes outside Vercel; only when the exact candidate is ready, run `node scripts/request-vercel-final-gate.mjs <PR_NUMBER>` so `ci/vercel-gate-base` records live `main` before persistent `ci/vercel-gate-final` moves to that PR head SHA and treat the resulting Preview as real acceptance
