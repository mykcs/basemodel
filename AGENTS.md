...
- **CI performance decisions require a complete experiment ledger.** Before continuing, adopting, rejecting, or reverting an optimization, read the [current CI performance experiment runbook](docs/agents/current/ci-performance-experiment-runbook.md). Recover related closed PRs and comments; bind the executed base/head tree and workflow/job/attempt; separate correctness, observed latency, causal evidence, and adoption. Green checks, an unchanged timeout literal, or a selected fast control do not establish performance equivalence. Preserve receipts and append corrections.
