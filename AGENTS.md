...
- **CI status is not benchmark evidence.** A green external check proves only that the checked SHA passed its acceptance gate; it does not prove speed, test-set identity, runtime identity, or log availability. Before reporting a CI experiment result, bind the claim to the exact provider, exact SHA, comparable workload, and provider timing receipt. If the provider log path is unavailable, report measurement as blocked rather than asking the owner to shuttle routine status or inventing a performance conclusion.
