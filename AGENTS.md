...
- **Admin mergeability is not acceptance authority.** Before merge, read the current required-check contract (`deployment-policy.md`, workflow/ruleset evidence, and live check state). `mergeable=true`, an admin-capable API, a green Vercel deployment, or a locally green suite does not authorize bypassing a required queued/failed CI check. If the required self-hosted runner is intentionally offline, hold the merge unless the owner explicitly invokes the documented emergency-recovery boundary.
- **Server-governance work re-enters the website release contract before the first BaseModel branch.** Completing inventory/reclaim does not carry deployment knowledge forward implicitly. Re-read root `AGENTS.md`, `branch-and-pr-conventions.md`, and `deployment-policy.md`; decide Preview eligibility and `[vercel-preview]` intent before naming the branch.
